...
c            iaCldBot(iIn) = iaCldBot(iIn) + 1
c            iaCldTop(iIn) = iaCldTop(iIn) + 1
...
c if the sun is ON and is in the FOV of the instrument, then the BC of 5700K
...
c this is if you want a funky dunky phase function
      REAL raPhasePoints(MaxPhase),raComputedPhase(MaxPhase)
...
c ------------------------ END OF DISORT ---------------------------------
     ...
     $                 raInten)
        CALL wrtout(iIOUN,caOutName,raFreq,raInten)
c        iF = 1
c        print *,iii,iF,raIntenStep(iF),raNoScatterStep(iF),raInten(iF)
      END DO
...
c     $         raUseEmissivity(iF),raFreq(iF),raNoScatterAll(iF),-1,
...
c these are to flip the temperature, abs profiles if instr looks up
      ...
      iiDiv = 0
 555  CONTINUE
      IF (iiDiv*kProfLayer .LT. iaRadLayer(3)) THEN
        iiDiv = iiDiv + 1
        GOTO 555
      END IF
      iiDiv = iiDiv - 1
...
c      Losch = (kAtm2mb*100/kBoltzmann/273 * 1e-6)    ! (p0/k T0)
...
c     $          raFreq,raNoScatterAll,raInten)
 ...
 1010          FORMAT('ERROR! linear SPLINT: bad XA array.',/,
     ...
     $       ', XA(KHI) = ',E12.5,',X = ',E12.5,', numpts = ',I4,'. Quitting.')
        ...
        ELSEIF (iMethod .EQ. 3) THEN     
          IF (ya(KHI) .GT. 0 .AND. CA(KHI) .GT. 0 .AND.  ya(KLO) .GT. 0 
     $         .AND. CA(KLO) .GT. 0) THEN
            !METHOD 3 
