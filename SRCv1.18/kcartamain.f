...
c      IF ((iakSolar(1) .GE. 0) .AND. (rFreqStart .GE. 5000.0) .AND. 
c     $     (raKsolarAngle(1) .LE. 90) .AND. (raKsolarAngle(1) .GE. 0)) THEN
c        kWhichScatterCode = 6
c        iNclouds_RTP = 1
c        iNclouds     = 1
c        iaaScatTable(1,1)   = 1
c        caaaScatTable(1,1)  = 'junk'
c        raScatterDME(1)     = 1.0
c        raScatterIWP(1)     = 1.0
c        rDummy1       = raaPrBdry(1,1) - 200.0
c        rDummy2      = raaPrBdry(1,1) - 10.0
c        iaCloudNumLayers(1)    = 0
c        iL_low = iaaRadlayer(1,1)
c 765    CONTINUE
c        IF (raPressLevels(iL_low) .GE. rDummy1 .AND. 
c     $      raPressLevels(iL_low) .LE. rDummy2) THEN
c          iaCloudNumLayers(1)    =  iaCloudNumLayers(1) + 1
c        END IF
c        IF (iL_low .LT.  iaaRadlayer(1,iaNumLayer(1))) THEN
c          iL_low = iL_Low + 1
c          GOTO 765
c        END IF  
c        iaCloudNumAtm(1)       = 1
