...
! local variables
      INTEGER iI,iJ,iaNeed(kMaxGas),iNewRead,iFound,iFoundX,iIDgas,iLay,iG0
...
! local variables : all copied from ftest1.f (Howard Motteler's example)
      ...
      print *,'booboo ! ',cfrac12,cfrac1,cfrac2,cngwat1,cngwat2,ctype1,ctype2,iNclouds_RTP
...
! output variables
      INTEGER iFnd,iGasIndex       !is RTP gas in the MOL/XS! gas list?
...
! <----------------------------------------------------------------------->
      ...
      !now see if there is a cloud to be used with this atmosphere
      IF (prof%cfra! .gt. 0.0) THEN
        cfra!   =  1.0            !assume total cloud cover
        cfra!   =  prof%cfra!     !be more honest
        cfrac1  =  prof%cfra!     !be more honest
        !!!first cloud is easy to do
        DO i = 1,1
          ctype1    = prof%ctype
          iaCtype(i) =  prof%ctype       !cloud type 1=cirrus 2=water etc
...
!          raCpsize(i) = prof%udef(12)    !in microns
        ...
        END DO
       ELSE
        cfra!  =  0.0            !assume clear sky, use dummy values
        cfrac1 =  0.0
        cfrac2 =  0.0
        ctype1 = -101
        ctype2 = -101
