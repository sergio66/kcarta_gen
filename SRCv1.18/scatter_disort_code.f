c this only differs by "SUBROUTINE GETMOM" from Dave Turners LBLDIS
c which is in scatter_disort_misc.f
c ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
c the same as the f77 version picked up from Warren Wiscombe ftp site on
c climate.gsfc.nasa.gov in August 20000, except for one or two modifications
c indicated by cccccccccccc sergio
c and every REAL variable --> DOUBLE PRECISION AFTER compilation
c this means that R1MACH has been replaced by D1MACH

c functions/subroutines isamx, sasum, sscal, sswap,saxpy, sdot have been 
c renamed to isamx_dis, sasum_dis, sscal_dis, sswap_dis,saxpy_dis, sdot_dis
c so that there is nbo conflict with the BLAS library

c rewrote plkavg so that it does not do any integrating, but just computes
c Plank radiance (if you use the integration feature, divide out by 
c delta(wavenumber)
c ie DISORT plkavg ---> plkavg_orig
c           plkavg ---> ttorad(w,T)

c also rewrote write( * ----> write( kStdWarn
c this meant that a few fcns/subroutine which needed include 'scatter.param'
c have to be modified so that maxmom, maxulv are not sent in as arguments 
c and reused inside of scatter.param

c by Frank Evans sugggestion, we turn DELTAM = .FALSE. 
c First, you don't need to un-delta scale.  Just run DISORT with its
c delta scaling flag off, but using the delta scaled sscatmie output.  
c This would match rtspec better.
...
c since they are in  include '../INCLUDE/scatter.param'
c got rid of MAXCLY, MAXULV, MAXUMU, MAXPHI, MAXMOM
c      INTEGER   MXCLY, MXULV, MXCMU, MXUMU, MXPHI, MI, MI9M2, NNLYRI,
c     &          MXSQT
c      PARAMETER ( MXCLY = maxcly, MXULV = maxcly + 1, MXCMU = maxcmu,
c     &            MXUMU = maxumu, MXPHI = maxphi, 
c     &            MI = MXCMU / 2, MI9M2 = 9*MI - 2,
c     &            NNLYRI = MXCMU*MXCLY, MXSQT = maxsqt )
...
cccccccccc sergio modified this a little
      IMPLICIT NONE
...
c got rid of MAXCLY, MAXULV, MAXUMU, MAXPHI, MAXMOM by using scatter.param

      CHARACTER HEADER*127
      LOGICAL   LAMBER, ONLYFL, PLANK, USRANG, USRTAU
ccccccccc sergio
cccccccc      INTEGER   IBCND, MAXCLY, MAXMOM, MAXPHI, MAXULV, MAXUMU, NLYR,
cccccccc    &          NMOM, NPHI, NSTR, NTAU, NUMU
...
csergio comment out this header as it puts too much stuff into warning.msg
c      IF( .NOT.PASS1 .AND. LEN( HEADER ).NE.0 )
c     &    WRITE( KSTDWARN,'(//,1X,100(''*''),/,A,/,1X,100(''*''))' )
c     &    ' DISORT: '//HEADER
...
c                                  ** Compute upward and downward fluxes
         ...
         IF( MAZIM.EQ.0 )
csergio comment out maxulv
c     &       CALL FLUXES( CMU, CWT, FBEAM, GC, KK, LAYRU, LL, LYRCUT,
c     &                    MAXULV, MXCMU, MXULV, NCUT, NN, NSTR, NTAU,
c     &                    PI, PRNT, PRNTU0( 1 ), SSALB, TAUCPR, UMU0,
c     &                    UTAU, UTAUPR, XR0, XR1, ZZ, ZPLK0, ZPLK1,
c     &                    DFDT, FLUP, FLDN, FLDIR, RFLDIR, RFLDN, UAVG,
c     &                    U0C )
...
csergio comment out maxulv,maxumu
...
csergio comment out maxulv
c      INTEGER   MAXULV, MXCMU, MXULV, NCUT, NN, NSTR, NTAU
...
c************************************************************************
csergio this has been usurped; however, call this for the SLFTST
c      REAL FUNCTION PLKAVG( WNUMLO, WNUMHI, T )
...
c************************************************************************
csergio got rid of maxmom
c      SUBROUTINE PRTINP( NLYR, DTAUC, DTAUCP, SSALB, NMOM, PMOM, TEMPER,
c     &                   WVNMLO, WVNMHI, NTAU, UTAU, NSTR, NUMU, UMU,
c     &                   NPHI, PHI, IBCND, FBEAM, UMU0, PHI0, FISOT,
c     &                   LAMBER, ALBEDO, BTEMP, TTEMP, TEMIS, DELTAM,
c     &                   PLANK, ONLYFL, CORINT, ACCUR, FLYR, LYRCUT,
c     &                   OPRIM, TAUC, TAUCPR, MAXMOM, PRTMOM )
...
csergio got rid of maxmom
c      INTEGER   IBCND, MAXMOM, NLYR, NMOM, NPHI, NSTR, NTAU, NUMU
      INTEGER   IBCND, NLYR, NMOM, NPHI, NSTR, NTAU, NUMU
      REAL      ACCUR, ALBEDO, BTEMP, FBEAM, FISOT, PHI0, TEMIS, TTEMP,
     &          UMU0, WVNMHI, WVNMLO
c     ..
...
c     ..
     ...
     &       '     No. computational layers =', NLYR
      ...
      IF( IBCND.NE.1 ) WRITE( KSTDWARN, '(I4,A,10F10.4,/,(26X,10F10.4))' )
...
c************************************************************************
csergio comment out maxulv,maxumu
c      SUBROUTINE PRTINT( UU, UTAU, NTAU, UMU, NUMU, PHI, NPHI, MAXULV,
c     &                   MAXUMU )
...
csergio comment out maxulv,maxumu
c      INTEGER   MAXULV, MAXUMU, NPHI, NTAU, NUMU
     ...
     &   '    Depth   Cosine'
      ...
      DO 30 LU = 1, NTAU
         ...
         DO 20 NP = 1, NPASS
            ...
            IF( NP.EQ.1 ) WRITE( KSTDWARN, '(F10.4,F8.4,1P,10E11.3)' )
     &             UTAU(LU), UMU(1), (UU(1, LU, J), J = JMIN, JMAX)
            IF( NP.GT.1 ) WRITE( KSTDWARN, '(10X,F8.4,1P,10E11.3)' )
...
c      print *,   COMPAR, CORINT, DELTAM, LAMBER, ONLYFL, PLANK, USRANG,
c     &           USRTAU
c      print *,   IBCND, NLYR, NMOM, NPHI, NSTR, NTAU, NUMU
c      print *,   ACCUR, ALBEDO, BTEMP, DTAUC, FBEAM, FISOT, FLUP, PHI
c      print *,    PHI0, RFLDIR, RFLDN, SSALB, TEMIS, TTEMP, UMU, UMU0
c      print *,    UTAU, UU, WVNMHI, WVNMLO
c         stop
...
c                                 ** solve L*V = Y
      ...
      DO 70 K = 1, N
         ...
         IF( K.LT.N ) CALL SAXPY_DIS( N - K, T, A( K + 1,K ), 1, Z( K + 1 ),
     &                            1 )
...
c  OUTPUT-- SASUM_DIS   Sum from 0 to N-1 of  ABS(SX(1+I*INCX))
c ----------------------------------------------------------
...
c OUTPUT--
c     SDOT_DIS   Sum for I = 0 to N-1 of  SX(LX+I*INCX) * SY(LY+I*INCY),
c            where  LX = 1          if INCX .GE. 0,
c                      = (-INCX)*N  if INCX .LT. 0,
c            and LY is defined analogously using INCY.
c ------------------------------------------------------------------
...
c OUTPUT-- ISAMAX_DIS   First I, I = 1 to N, to maximize
c                         ABS(SX(1+(I-1)*INCX))
c ---------------------------------------------------------------------
...
c************************************************************************
cccccccc sergio shorter faster version 
c     COMPUTES PLANCK FUNCTION  
      ...
      IF (T .LT. 1.0e-4) THEN
        PLKAVG = 0.0
      ELSEIF (wnumhi-wnumlo .lt. 0) THEN
        write(*,*) 'what!!!! wnumhi < wnmlo!!!!'
        stop
      ELSEIF (wnumhi-wnumlo .gt. 1.0) THEN
...
c************************************************************************ 
c slftest : when working
c in disort nana  0.000000000000000  50000.0000000000
c  T T T T F T T T
c 0 1 4 1 4 1 1
c 1.000E-004  0.700  300.  1.00  3.1415926500  1.00  152.585283866739  90.0
c 0.000  1.52728600717066  28.3722254400388  0.900  0.800  100.  0.500  0.866
c 0.500  47.8655714383180  5.00  0.000
