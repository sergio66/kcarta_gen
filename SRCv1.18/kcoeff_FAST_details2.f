...
c************************************************************************
c  this is for iSplineType = 2  --- FASTER as the klayers pressure levels
c                                 are same as kCompressed database levels
c************************************************************************
...
c          print *,iI,iK,iaP1(iK),iaT11(iK),iaQ11(iK)
          daaKpro(iI,iK) = 
...
c pAvgUse,pProf in mb
     ...
     $     (abs(kLongOrShort) .LE. 1)) THEN
      ...
      IF ((iGasID .EQ .kNewGasLo) .OR. (iGasID .EQ .kNewGasHi)) THEN
        ...
        CALL FindReferenceName(caFName,1,iLowerOrUpper)  
      ELSEIF (abs(iLowerOrUpper) .EQ. 1) THEN
     ...
     $         raOrig100P,raOrig100PP,iE)
      ELSEIF (iLowerOrUpper .EQ. 2) THEN
        ...
        CALL DoSTOP
c        CALL GetUS_Std_UA(raUpperPress_Std,raUpperMixRatio_Std,
c     $         raUpperDZ_Std,raUpperCO2Amt_Std,raUpperTemp_Std,iUpperStd_Num)
      ...
      ELSEIF (iLowerOrUpper .EQ. 2) THEN
        ...
        CALL DoStop
c        DO iI = 1,kMaxLayer
c          daOrig100A(iI) = 0.0d0
c        END DO
c        DO iI = 1,iUpperStd_Num
c          daOrig100A(iI) = raUpperCO2Amt_Std(iI) * 1.0d0
c        END DO
      END IF
      ...
      ELSEIF ((iLowerOrUpper .EQ. +1) .OR. (iLowerOrUpper .EQ. +2) .OR. (iLowerOrUpper .EQ. +3)) THEN
        ...
        ELSEIF ((iGasID .EQ. 2) .AND. (iLowerOrUpper .EQ. +1)) THEN
          ...
          CALL ua_avg_databasepressures(pAvgUse,raPP2,raT2,raA2)
c        ELSEIF ((iGasID .EQ. 2) .AND. (iLowerOrUpper .EQ. +2)) THEN
c          write(kStdWarn,*) 'when uncompressing the Compressed UA NLTE database, replacing '
c          write(kStdWarn,*) 'usual database pressures with UA'
c          DO iI = 1,kMaxLayer
c            pAvgUse(iI) = 0.0
c            raRTemp(iI) = 0.0
c          END DO
c          !! pProf and pAvgUse have same units (mb)
c          DO iI = 1,iUpperStd_Num
c            pAvgUse(iI) = raUpperPress_Std(iI)
c            raRTemp(iI) = raUpperTemp_Std(iI)
c          END DO
      ...
      IF (abs(iLowerOrUpper) .EQ. 1) THEN
        iLowest  = kProfLayer - iProfileLayers + 1
c        iHighest = kProfLayer
      ELSEIF (iLowerOrUpper .EQ. 2) THEN
        iLowest  = 1
c        iHighest = iUpperStd_Num
      ELSEIF (iLowerOrUpper .EQ. 3) THEN
        iLowest  = kProfLayer - iProfileLayers + 1
c        iHighest = kProfLayer
      END IF
...
c this calls stuff to do pressure, temperature interpolations, then
c does the uncompression
c this is for CKD gases : there is NO gasAMT involved in the compression
...
c************************************************************************
c this routine does the interpolation of a compressed matrix
c note we do NOT worry about raAmt as CKD tables only have coefficients and not optical depths
c   daaaKx(kMaxK,kMaxTemp,kMaxLayer) ---> daaaKxNew(kMaxK,kMaxTemp,kProfLayer)
c then temperature interpolation
c   daaaKxNew(kMaxK,kMaxTemp,kProfLayer) --> daaKpro(kMaxk,kProfLayer)
...
c      !!!l no need to change k 
c      DO iI=1,iNk
c        DO iJ=1,iKm
c          DO iE=1,kMaxLayer   
c            daaaKxNew(iI,iJ,iE) = daaaKx(iI,iJ,iE)
c            END DO
c          END DO
c        END DO

c use daaKx directly, instead of daaaKxNew
C     now do the spline Interpolation of the K vectors in TEMPERATURE
      DO iI=1,iNk                  !Loop over the K vectors
        DO iK=iLowest,kProfLayer   !Loop over the layers
...
c      !!!l change k (optical depth, dimenstionless) --> k (abs coeff, cm2 mol-1)
c      DO iI=1,iNk
c        DO iJ=1,iKm
c          DO iE=1,kMaxLayer   
c            daaaKxNew(iI,iJ,iE) = daaaKx(iI,iJ,iE)/daQ(iE)
c            END DO
c          END DO
c        END DO
