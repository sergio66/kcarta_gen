...
c to put in arbitrary CO2 amounts, have to be a little more careful!!!!!!!
c   load in the LTE weak+strong database       (at multiplier = 1)
c   compute weak backgnd LTE, lower atm        (at multiplier = rX)
c   compute strong NLTE lower atm              (at multiplier = rX)
c   compute strong NLTE radiance enhance, LA   (at multiplier = rX)
c   compute weak backgnd LTE, upper atm        (at multiplier = rX)
c   compute strong NLTE upper atm              (at multiplier = rX)
c   compute strong NLTE radiance enhance, UA   (at multiplier = rX)
c   combine and output everything              (at multiplier = rX)
c   divide the OD output by multiplier, so that the raaMix code recorrects
c                                       the OD

c note this is separate from  raNLTEstrength(1) :
c   raNLTEstrength(1) = +1.0 : says use a weight of 1.0 for all the
c                          upper layer optical depths computed for this gas.\\
c                          Together with parameter kCousin_CO2path, this lets 
c                          kCARTA do cousin lineshape computations via the
c                          following fudge : if raNLTEstrength(X) = -1.1212 and
c                          iaNLTEGasID(X) = 2, kCARTA not do NLTE but instead 
c                          swaps the linemix uncompressed spectra for cousin 
c                          uncompressed spectra in the layers above that 
c                          specified by raNONLTEstart(X)\\
c                          Otherwise raNLTEstrength is really not used

c************************************************************************
c************************************************************************
c************************************************************************
c  UA pressure levels are from running KCARTADATA/NLTE/UA/read_glatm.m
c************************************************************************
c************************************************************************
c************************************************************************
c* defaults used when producing the NLTE database in Dec 2004
c*                  for NLTE
c*  knonlte.f           iTalk = -1              short summary of NLTE info
c*                                                /band/layer
c*  klineshapes.f       iTalk = -1              short summary of NLTE info
c*                                                /band/layer
c*  kvoigt_cousin.f     iTestGenln2 = -1        use line mix for strong bands
c*  knonlte.f           iNoPressureShiftCO2 = 1 no pressure shift in CO2 lines
c*  klineshapes.f       iQtips_H98 = +1         QTIPS values for partition fcn
c*  kbloat.f            iLinearOrSpline = -1    Use spline to go from 0.0025 to
c*                                                0.0005 spacing, for LTE stuff
c*                                                in BloatCoeffs()
c*
c************************************************************************
c************************************************************************
c        !!! (a) all weak LTE lines have been read in from the compressed 
c        !!!     database, computed using a Cousin lineshape
c        !!!     CALL lte_spectra(...)
c        !!!     CALL read_std_optdepths_new(iGasID,raFreq,iStart, 
c        !!! (b) these medium LTE lines will have a Cousin lineshape
c        !!!     CALL lte_spectra(...)
c        !!! (c) the strong NLTE lines will have a Cousin lineshape deep in the
c        !!!     4 um band, or a linemix lineshape in the R branchhead
c        !!!     CALL nonlte_spectra(...)
c this means there is a discontinuity at 2355 cm-1, as the strongest sigsig
c band is switched from Cousin to line mix (about a 1 K step)
c************************************************************************
c Ref : Edwards et al : NON LTE Studies, JGR vol 98, pgs 14955 August 1993
...
c Ref : KOPRA documentation 
        !! http://www-imk.fzk.de/imk2/ame/publications/kopra_docu/
c see pg 277,435 of Lopez-Puertas book for the NLTE bands in the 4.3 um
...
c Ref : HITRAN bd_vibs.for tells you the HITRAN spectroscopic notations
c see SUBROUTINE NLTEBandMapper for all this
        !!so v_lower == 1, v_upper ==  9 for strong SigSig band
        !!          00001         00011
        !!/asl/data/hitran/HITRAN2k/BD_Vibs.for

c /home/sergio/KCARTA/SRC/NONLTE2/sergio/NONLTE_MATLAB/make_lbl2kc_weakco2.m
c has topts.linemixloop = -1 SO WE NEED TO ADD on all the linemix bnds!
c which I define as 2310,2311,  2320,2321,2322,   2350,2351,2352,2353,2354,2355
c Pretty much bands 2350,2351,2352,2355,  2320,2321,  2310,2311 are in NLTE
c So we need to include the effects of bands 2322, 2353 and 2354!!!!!!!
c These are linemix bands, so just use Cousin for them!

c NOTE (1) : 
c      INTEGER iEdwards  !!!**** choose how to do things!!!!!! for the 
c                        !!!**** abs coeff and Planck coeff factors!!!!
c this is following Eqn 15,16 of the NLTE paper by Edwards
c Ref : Edwards et al : NON LTE Studies, JGR vol 98, pgs 14955 August 1993
c      iEdwards = +1
c this is following the TES algorithm
c      iEdwards = -1

c places I can see where it could differ from GENLN are in computing downwell
c background radiation, kCARTA does NOT use raaPlanckCoeff!

c MAGIC NUMBERS
c    iOneCmFine = 400                 !!!! 1 cm-1 = 400 pts at 0.0025 cm-1 
c    iOffSet = (iWideMeshLoop-1)*2000 !!!! 1-400 on kCARTA =1-2000 high res

c **************************** testing the code ************************
c 1) testing NLTE, only for strongest 4.3 um band (sig sig)
c   (a) in kcarta.param, use kProfLayer = 43
c   (b) use edwards_jan2003.nml (which is in NLTE dir)
c   (c) test middle of band, since he used CO2 continuum lineshape which means
c       that in the wings, he has absorption(continuum) + abs(sigsig*cousin)
c       compared to me, who only has abs(sigsig*cousin); BUT his BT in between
c       lines is BIGGER than mine! (in between lines)
c   (d) can test the default kCARTA spacing 
c          dLineStrenMin = -1.0d0     !!!! use all backgnd lines 
c          dDeltaFreqNLTE = -1.0d0    !!!use default dkFreqTag 
c          kBoxCarUse = 5             !!!use default 5 
c        OR the GENLN4 spacing by setting switches at kcartabasic.f
c          dLineStrenMin = 1.0d-25    !!!! use backgnd lines bigger than this 
c          dDeltaFreqNLTE = 1.0d-3    !!!use GENLN4 
c          kBoxCarUse = 1             !!!use GENLN4 
c
c************************************************************************
c06/02/03        Include first order linemixing code into the NLTE .. for the 
c                strong bands (2350,2351), use linemix plus birnbaum; else use 
c                cousin lineshape (for the "other strong bands"); for medium
c                and weak lines, use plain voigt
c                  Subroutine read_lineparameters initially sets this info
c                  Subroutine CousinVsMix        reassigns individual lines
c                                                to linemix, or back to cousin
c                                                (since eg 2350 P branch sux!!)
c                iLineMixBand = -1               plain voigt
c                iLineMixBand = +1               plain voigt times cousin
c                iLineMixBand = +2               plain voigt times linemix
c                The linemix code is slow, and so the code mainly does Cousin 
c                only, by resetting iLineMixBand = 1 in sub read_lineparameters
c                Code can also use fudge files
c                   /asl/data/kcarta/KCARTADATA/General/ChiFile/
c                         nonlte_co2_fudge_2380.txt; 
c                  see NONLTE2/sergio/SECOND_TEST/save_ratio_k.m for details
c
c************************************************************************
...
c input params
c rCO2mult  = tells how much input .rtp file wants the CO2 to be multiplied by
c             remeber this is as compared to kCO2ppmv
      REAL rCO2mult
...
c caaNLTETemp       tells the name of the files containing the nonLTE temps 
      ...
      CHARACTER*120 caOutUAFile,caOutUABloatFile
...
c          print *,iL,raRTemp(iL)-raRTempx(iL),raRAmt(iL)/raRAmtX(iL)
c        END DO
c      call dostop
      END IF
      ...
      iBand = 2350
      iBand = 1
      IF ((iDoUpperAtmNLTE .GT. 0) .AND. (iLTEIn .GT. 0)) THEN 
...
c        DO iL = 1,iNumMixRatioLevs
c          print *,iL,raUpper_Pres(iL),raUpper_MixRatio(iL)
c        END DO
c        print *,' '
c        DO iL = 1,iUpperStd_Num
c          print *,iL,raUpperPress_std(iL),raUpperMixRatio_std(iL)
c        END DO
c        print *,' '
c        DO iL = 1,iUpper
c          print *,iL,raUpperPress(iL),raUpperTemp(iL)
c        END DO
c        CALL DOSTOP
        ...
        IF (iDoUpperAtmNLTE .GT. 0) THEN
          ...
          IF (raUpperPress_Std(iUpperStd_Num).LT.raUpperPress1013(iUpper))THEN
            !! need to turn off the Planck function else we may get bad results
            iZeroPlanck = 1
 10         CONTINUE
            IF (raUpperPress_Std(iZeroPlanck).GT.raUpperPress1013(iUpper) .AND.
     $          iZeroPlanck .LT. iUpperStd_Num) THEN
     ...
     $       (abs(kLongOrShort) .LE. 1)) THEN
            ...
            write(kStdWarn,*) ' '
            IF (iZeroPlanck .LT. (iUpperStd_Num + 1)) THEN
     ...
     $                      iNumNLTEGases,iaNLTEChunks,iaaNLTEChunks)
          ...
          ELSEIF (iDumpAllUARads .LE. 0) THEN
      ...
      IF (rSolzen .GE. 00.0 .AND. rSolzen .LT. 40.0) THEN
        rSolzenX = 00.0
        rSolzenY = 40.0
      ELSEIF (rSolzen .GE. 40.0 .AND. rSolzen .LT. 60.0) THEN
        rSolzenX = 40.0
        rSolzenY = 60.0
      ELSEIF (rSolzen .GE. 60.0 .AND. rSolzen .LT. 80.0) THEN
        rSolzenX = 60.0
        rSolzenY = 80.0
      ELSEIF (rSolzen .GE. 80.0 .AND. rSolzen .LT. 85.0) THEN
        rSolzenX = 80.0
        rSolzenY = 85.0
      ELSEIF (rSolzen .GE. 85.0 .AND. rSolzen .LE. 90.0) THEN
        rSolzenX = 85.0
        rSolzenY = 90.0
      END IF
     ...
     $        NewDataChunk(iLTEIn,iaNLTEChunks,iaaNLTEChunks,rFileStartFr)
        ...
        IF ((iWhichChunk .GT. 0) .AND. (iaGases(iGas) .EQ. 2)) THEN
...
c              print *,'mwah',iL,daaWeakOptDepth(1,iL),daaUpperNLTEGasAbCoeff(1,iL),daaUpperPlanckCoeff(1,iL)
              DO iFr = 1,kMaxPts
...
c             do iL = 1,kProfLayer
c               print *,'place A',iaGases(iGas),iL,daaGasAbCoeff(1,iL),daaPlanckCoeff(1,iL),
c     $                    daaUpperNLTEGasAbCoeff(1,iL),daaUpperPlanckCoeff(1,iL)
c             end do
...
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c SLOW LBL calcs for NLTE ODs and Planck Coeffs
...
c local vars, mainly functions called
...
c see if current gas ID needs nonLTE spectroscopy
     ...
     $        NewDataChunk(iLTEIn,iaNLTEChunks,iaaNLTEChunks,rFileStartFr)
        ...
        IF ((iWhichChunk .GT. 0) .AND. (iaGases(iGas) .EQ. 2) 
     $          .AND. (abs(raNLTEstrength(iLTEIn)-1.0) .LE. 0.01))  THEN
        ...
        IF ((iWhichChunk .GT. 0) .AND. (iaGases(iGas) .EQ. 2) 
     $          .AND. (raNLTEstrength(iLTEIn) .LT. 0))  THEN
     ...
     $          iUpper,daaGasAbCoeff,iSplineType)
        ...
        ELSEIF (iWhichChunk .GT. 0) THEN
          !!!first compute LTE background optical depths
          IF (iUseWeakBackGnd .GT. 0) THEN
...
c use pProfNLTE or pProf??????
     ...
     $          .AND. (raNLTEstrength(iLTEIn) .LT. 0))  THEN 
       ...
       ELSEIF ((iWhichChunk .LT. 0) .OR. (raNLTEstrength(iLTEIn) .LT. 0)) THEN
         iLTEIn = -1
...
c************************************************************************
c this subroutine checks to see if we need to zero the daaPlanckCoeff for
c the particular chunk
...
c input params
      ...
      CHARACTER*120 caOutBloatFile        !file to dump high res info to
      CHARACTER*120 caPlanckBloatFile     !file to dump high res info to
...
c local variables
      ...
      DO iGas = 1,iNumGases
        ...
        IF ((iLTEIn .GT. 0) .AND. (kSolarAngle .GE. 0 .AND. kSolarAngle .LE. 90)) THEN
          iWhichChunk = 
     $        NewDataChunk(iLTEIn,iaNLTEChunks,iaaNLTEChunks,rFileStartFr)
          IF ((iWhichChunk .GT. 0) .AND. (iNLTE_SlowORFast .EQ. +1)) THEN
            iChunk_DoNLTE = +1           !!!need to do NLTE slowly
            GOTO 10
          ELSEIF ((iWhichChunk .GT. 0) .AND. (iNLTE_SlowORFast .EQ. -1)) THEN
            iChunk_DoNLTE = +2           !!!need to do NLTE with SARTA model
            GOTO 10
          ELSEIF ((iWhichChunk .GT. 0) .AND. (iNLTE_SlowORFast .EQ. -2)) THEN
 ...
 10   CONTINUE
      ...
      IF (((iChunk_DoNLTE .EQ. 1) .OR. (iChunk_DoNLTE .EQ. 3)) .AND. 
     $    (iDoUpperAtmNLTE .EQ. 1)) THEN
        DO iL = 1,kProfLayer
          DO iFr = 1,kMaxPts
...
c set up the high res wavenumber array
      IF ((iChunk_DoNLTE .EQ. 1) .AND. (iSetBloat .EQ. 1)) THEN
      ...
      !!!                              iChunk_DoNLTE    == +1
      IF ((iChunk_DoNLTE .EQ. 1) .AND. (kBloatOutOpen .LT. 0)) THEN
     ...
     $                     iNumNLTEGases,iaNLTEChunks,iaaNLTEChunks)
      ...
      IF ((iChunk_DoNLTE .EQ. 1) .AND. (kBloatPlanckOpen .LT. 0) 
     $    .AND. (kPlanckOut .EQ. 0)) THEN
        IF (iSetBloat .EQ. 1) THEN
          iType = -1
          CALL OpenOutputBloatFile(
...
c************************************************************************
c this simply adds on two double matrices, from layers iSTart -> kProflayer
c store results in daaSave
c if iAdd = +1, then you add the abs coeffs together in relevant layers
c if iAdd = -1, then you replace daa with daaAdd in relevant layers
c if iAdd =  0, then you replace daa with 0.0 in relevant layers

c note that here we are adding CUMULATIVE things, so we can check to ensure
c kAbs > 0!!
...
c************************************************************************
c this computes the cumulative multiplication modification to Planck, which is
c just 1.000000000000000
...
c recall raaSumAbCoeff(iFr,iM) is the CUMULATIVE abscoeff=sum(LTE) + sum(NLTE)
c where sum(LTE) = all gases that are in LTE, plus CO2 in LTE plus CO2 in NLTE
c       sum(NLTE) = CO2 in NLTE
c thus we need to break down Eqn 20 of the KOPRA paper appropriately
c                            Eqn 16 of the JGR D. Edwards paper appropriately
c by finding numerator of (Eqn 20 kopra or Eqn 16, JGR); denom = raaSumAbCoeff
...
c************************************************************************
c this subroutine changes double --> real for the UPPER stratosphere
c this computes the cumulative multiplication modification to Planck
...
c              dTemp = daaUpperSumNLTEGasAbCoeff(iFr,iL)
c              raaUpperNLTEGasAbCoeff(iFr,iL) = real(dTemp)
              END DO
            END DO
          END IF
...
c************************************************************************
c This subroutine computes the LTE abs coeffs for the background lines,
c   for stuff in the KCARTA atmosphere
c The weakest lines are simple : just read in precomputed US Standard optical
c   depths, and adjust as necessary to current pressure layering and
c   gas amounts (see read_std_optdepths)
c The strong lines, for which run7co2.m does linemixing, are a little more 
c   complicated, as the code uses the current profile to compute voigt optical 
c   depths * birnbaum of these strong lines. These lines are in LTE so use the
c   current temperature profile
...
c input parameters 
c iTag      tells the current frequency step
c iLTEin    tells which set of data to use
c iStart    tells which layer to start NLTE calcs
c iaNLTEBands   tells for each gas, how many are the NON LTE bands bad boys
c caaaNLTEBands tells the name of the files containing the line parameters
c caaNLTETemp   tells the name of the files containing the nonLTE temps
c caaStrongLines     line param files associated with strong lines, in LTE
c raTTemp etc     tell you the LTE gas profile
c raFreq          tells you the wavenumber array
c pProf           tells you the avg pressure in the AIRS layers
c dDeltaFreq are for default (0.0025 spacing, 5 point boxcar) or not
c dLineStrenMin is which min line strength to use
...
c output parameter
c daaNLTEGasAbCoeffCoeff tells how to modify the abs coeff
c daaPlanckCoeff         tells how to modify the Planck function
c daaSumNLTEGasAbCoeff   tells running sum of NLTE GasAbCoeff
      DOUBLE PRECISION daaNLTEGasAbCoeff(kMaxPts,kProfLayer)
      DOUBLE PRECISION daaPlanckCoeff(kMaxPts,kProfLayer)
      DOUBLE PRECISION daaSumNLTEGasAbcoeff(kMaxPts,kProfLayer)
...
c //////////////----> first compute lineshape of stronger lines, at LTE
...
c //////////////----> then read in optical depths of the weaker lines
      !!compute lineshapes, using lineparameters, at layers blah..kProflayer
      IF (iUseWeakBackGnd .EQ. 1) THEN
...
c**** therefore, at the end of this routine
c**** daaNLTEGasAbCoeff = daaSumNLTEGasAbCoeff
c**** and in essence, daaNLTEGasAbCoeff = daaPlanckCoeff also
...
c************************************************************************
c This subroutine computes the LTE abs coeffs for the background lines,
c   for stuff "above" the KCARTA atmosphere ie in the stratospphere
c The weakest lines are simple : just read in precomputed US Standard optical
c   depths, use the uppermost layer (lowest pressure) and scale optical depths
c   accordingly 
c The strong lines, for which run7co2.m does linemixing, are a little more 
c   complicated, as the code uses the current profile to compute voigt optical 
c   depths * birnbaum of these strong lines. These lines are in LTE so use the
...
c input parameters 
c iNLTEStart tells where the non LTE atmosphere starts
c iTag      tells the current frequency step
c iLTEin    tells which set of data to use
c iaNLTEBands   tells for each gas, how many are the NON LTE bands bad boys
c caaaNLTEBands tells the name of the files containing the line parameters
c caaNLTETemp   tells the name of the files containing the nonLTE temps
c raFreq        tells you the wavenumber array
c pProf         tells you the avg pressure in the layers (in mbar)
...
c             remeber this is as compared to kCO2ppmv
      ...
      INTEGER iDoVoigtChi   !!set this THE SAME in SetRunningMesh,voigt_chi
c these are just for dumping stuff to the UA file
      REAL rFrLow,rFrHigh
      INTEGER iDumpAllUASpectra,iDumpAllUARads,iFileIDLo,iFileIDHi
...
c output parameters
c iUpper    tells which layer to end NLTE calcs
c daaPlanckCoeff        tells how to modify the Planck function
c while the rest are the P,PP,T(lte),T(NLTE),Q for the upper atmosphere
      INTEGER iUpper,iNumberUA_NLTEOut
      DOUBLE PRECISION daaUpperNLTEGasAbCoeff(kMaxPts,kProfLayer)
      DOUBLE PRECISION daaUpperSumNLTEGasAbCoeff(kMaxPts,kProfLayer)
      DOUBLE PRECISION daaUpperPlanckCoeff(kMaxPts,kProfLayer)
...
c local variables
     ...
     $                      iNumNLTEGases,iaNLTEChunks,iaaNLTEChunks)
        ...
        IF ((iSetBloat .GT. 0) .AND. (kBloatNLTEOutUAOpen .EQ. -1)) THEN
...
c ---------------> then read in optical depths of the weaker lines
c ---------------> which were computed at US standard profile, and readjust to
c ---------------> the current layering structure
c ---------------> note we use the UA kCARTA database 
...
c************************************************************************
c This subroutine computes the NLTE abs coeffs based on the lineparameters
c and the LTE parameters, for stuff in the KCARTA atmosphere
...
c iLTEin     tells which set of data to use
c iStart     tells which layer to start NLTE calcs
c iStart2350 tells which layer to start NLTE calcs for strongest 4um CO2 band
c iaNLTEBands   tells for each gas, how many are the NON LTE bands bad boys
c caaaNLTEBands tells the name of the files containing the line parameters
c caaNLTETemp   tells the name of the files containing the nonLTE temps
c raTTemp etc   tell you the LTE gas profile
...
c daaNLTEGasAbCoeffCoeff  tells how to modify the abs coeff
c daaPlanckCoeff         tells how to modify the Planck function
c daaSumNLTEGasAbCoeff keeps track of the total NLTE gas abs coeff (JU == 9)
      DOUBLE PRECISION daaSumNLTEGasAbCoeff(kMaxPts,kProfLayer)
      DOUBLE PRECISION daaNLTEGasAbCoeff(kMaxPts,kProfLayer)
      DOUBLE PRECISION daaPlanckCoeff(kMaxPts,kProfLayer)
      INTEGER iDoUpperAtmNLTE,iAllLayersLTE
...
c this is for bloating
     ...
     $     iaJ_UorL,raLTETemp,raNLTETemp,raVibQFT,iAllLayersLTE,dVibCenter) 
        ...
        IF ((iGasID .EQ. 2) .AND. (iISO .EQ. 1) .AND. 
     $        (nint(daJU(1)) .EQ. 9)) THEN
        ...
        DO iL = iStartUse,kProfLayer
          ...
          IF ((iBand .EQ. 1 .AND. iL .EQ. iStartUse) .AND. 
     $       (((abs(kLongOrShort) .EQ. 2) .AND. (kOuterLoop .EQ. 1)) .OR. 
     $        (abs(kLongOrShort) .LE. 1))) THEN
            iPrintTalk = +1
          ELSE
            iPrintTalk = -1
...
c************************************************************************
c this subroutine does the stuff high up in the STRATOSphere!
c assumes that the only contribution is from the VERY strong NLTE bands
c and nothing else from the weak bands or other gases!
c reads in some silly stuff for CO2
...
c this is for dumping stuff
      INTEGER iIOUN,iPrintTalk
      CHARACTER*120 caOutName
      REAL raX(kMaxPts)

c this is mainly junk
      REAL raUpperPress_Std(kProfLayer),raUpperMixRatio_Std(kProfLayer)
      REAL raUpperDZ_Std(kProfLayer),raUpperCO2Amt_Std(kProfLayer)
...
c the file really should have been opened in SUBROUTINE nonlte_spectra
      IF (kNLTEOutUAOpen .EQ. -1) THEN
        !!! open the output file etc
        iType = +1
        CALL OpenUAFile(iType,iUpper,caOutUAFile,rFrLow,rFrHigh,
     ...
     $                      iNumNLTEGases,iaNLTEChunks,iaaNLTEChunks)
      ...
      IF (iGasID .NE. 2) THEN
        ...
        CALL DoStop
      ELSEIF ((iGasID .EQ. 2) .AND. (iDoUpperAtmNLTE .GT. 0)) THEN
     ...
     $                raUpperCO2Amt_Std,raUpperTemp_Std,iUpperStd_Num)
          ...
          IF ((iUpper .GE. 1) .AND. (iDoUpperAtmNLTE .GT. 0)) THEN
...
c************************************************************************
c this subroutine does some inits for the running meshes
...
c local vars
      ...
      IF (dDeltaFreqNLTE .LT. 0.0d0) THEN
...
c************************************************************************
c this sets up the running mesh!!!!
...
c local variables
      ...
      IF (iDoFine .LT. 0) THEN
        write(kStdErr,*) 'Wow .. you have iDoFine = -1!!!'
        CALL DoStop
        END IF

      IF (iDoMedium .LT. 0) THEN
        write(kStdWarn,*) 'Wow .. you have iDoMed = -1!!!'
        END IF

      IF (iDoCoarse .LT. 0) THEN
        write(kStdWarn,*) 'Wow .. you have iDoCoarse = -1!!!'
        END IF

c this is always done, as we ALWAYS do the fine stuff
      ...
      IF (iDoMedium .GT. 0) THEN   !!!run7lbl style
     ...
     $         (daLineShift(iLines) .le. d4)) THEN
      ...
      IF ((nint(sngl(dJU)) .EQ. 9) .AND. (iISO .EQ. 1) .AND. 
     $    (iDoVoigtChi .GT. 0)) THEN
...
c************************************************************************
c this subroutine computes the lineshapes AND
c it also computes the contribution to the numerator, for the Planck coeff
c this is done for ONE band, for ONE layer
...
c input params 
      ...
      INTEGER iDoVoigtChi   !!set this THE SAME in SetRunningMesh,voigt_chi
c output params ... the arrays are looped over ALL lines in band
...
c if we want to dump out line parameters !!!! ugh!!! makes warning.msg LONG!!!!
      ...
      ELSEIF (iAllLayersLTE .EQ. +1) THEN
        !!before March 3, 2005
...
c divide lines into FINE,MEDIUM,COARSE meshes
c lines to use. take [f3 f4] as the limits of the current wide mesh 
c (a) fine lines are always within  [f3-xnear  f4+xnear]     xnear ~ 1 cm-1 
c     ie these lines are always within +/- 1 cm-1 of current wide mesh 
c (b) medium lines are always within  
c             [f3-xmed f3-xnear] U [f4+xnear f4+xmed]      xmed ~ 2 cm-1 
c     ie these lines are always 1-2 cm-1 away from current wide mesh 
c (c) far lines are always within  
c             [f3-xfar f3-xmed] U [f4+xmed f4+xfar]        xfar ~ 25 cm-1 
c     ie these lines are always 2-25 cm-1 away from current wide mesh 

c used to be 2317.2
      iOneLine = -1
      IF (iOneLine .GT. 0) THEN
...
c now make sure everything > 0
     ...
     $           (abs(kLongOrShort) .LE. 1))) THEN
      ...
      IF (iPrintTalk .GT. 0) THEN
        ...
        write(kStdWarn,*)'------------------------------------------------------------------------------'
        DO iLines = 1,iNum
          IF (iaLineMix(iLines) .EQ. 1) THEN        !cousin
            cCousOrBirn = 'C'
          ELSEIF (iaLineMix(iLines) .EQ. 2) THEN    !linemix * birn
            cCousOrBirn = 'B'
          ELSEIF (iaLineMix(iLines) .LT. 0) THEN    !nothing ... plain voigt
            cCousOrBirn = 'V'
          ELSE 
 ...
 1010 FORMAT(I3,' ',D12.5,' ',I4,' ',A1,' ',1(D12.5,' '),5(D12.5,' '))

      RETURN
      END

c************************************************************************
c this subroutine computes the background lineshapes
c loops over ALL lines >= strmin, using LTE 
c does it like run7.m where it divides the 25 cm-1 chunk into 1 cm-1 chunks
...
c************************************************************************
c allows a NLTE atmosphere, does SLOW ACCURATE GENLN2 calc
...
c************************************************************************
c allows a NLTE atmosphere, does FAST SARTA NLTE calc
...
c note iNLTEStart = kProfLayer + 1, unless NLTE computations done!
c so usually only the usual LTE computations are done!!
      IF (iNLTEStart .LE. kProfLayer) THEN        
        write (kStdWarn,*) 'this routine expects SARTA NLTE',iNLTEStart
        CALL DoStop
      ELSE
c        write (kStdWarn,*) 'Normal rad transfer .... '
c        write (kStdWarn,*) 'adding on SARTA NLTE if DAYTIME'
        iLay = 1
...
c           write(kStdWarn,*) iFr,iaRadLayer(iFr),raSunAngles(iaRadLayer(iFr))
c           end do
...
c************************************************************************
c allows a NLTE atmosphere, does USUAL FAST computation using COMPPRESSED NLTE tables
...
c************************************************************************
c this subroutine mimics the SARTA NLTE model
c see /asl/packages/sartaV106/Src/calnte.f
...
c      INTEGER MXCHNN,NNCOEF
c      !MXCHNN = 201   
c      !NNCOEF = 6     !/asl/packages/sartaV106/Src/incFTC_apr05_nte.f
c      PARAMETER(MXCHNN = 201,NNCOEF = 6)
c      REAL COEFN(NNCOEF,MXCHNN)

c     !!/asl/packages/sartaV108/Src/incFTC_airs_apr08_m130_m150_template_cal.f
...
c      need to process this : from be to le
c      !matlab cd : /home/sergio/KCARTADATA/NLTE/SARTA_COEFS/
c      !fname='/asl/data/sarta_database/Data_jan04untun/Coef/setnte_oct05.dat';
...
c
c also look at /asl/data/kcarta_sergio/KCDATA/NLTE/SARTA_COEFS
c /umbc/xfs3/strow/asl/data/kcarta_sergio/KCDATA/NLTE/SARTA_COEFS
c >> x='/asl/data/kcarta_sergio/KCDATA/NLTE/SARTA_COEFS/nonLTE7_m150.be.dat';
c >> [idchan, freq, coef] = rd_nte_be(x);
...
c************************************************************************
c this subroutine very quickly does the radiative transfer
...
c************************************************************************
c this is based on subroutine "othergases"
...
c i_NLTEFile_TYPE     > 100 for LA ods, > 200 for LA planckcoeff
c                     > 300 for UA ods, > 300 for UA planckcoeff
...
c iUAirLA = -1 for usual lower atm (so can do jacs), +1 for extra upper atm (so no jacs)
      ...
      ELSEIF (i_NLTEFile_TYPE .GE. 300) THEN
...
c interpolate compressed data in temperature, to get abs coeff matrix
      IF ((kJacobian .GE. 0) .AND. (iUAoriLA .EQ. -1)) THEN
     ...
     $  daaDQ,daaDT,iDoDQ,iGasID,pProf,iProfileLayers,iSPlinetype)
      ELSEIF ((kJacobian .GE. 0) .AND. (iUAoriLA .EQ. +1)) THEN        
     ...
     $         pProf,iProfileLayers,iSplineType,iLowerOrUpper)
      ELSEIF ((kJacobian .LT. 0) .AND. (iUAoriLA .EQ. -1)) THEN        
        !! no jacs, usual LA
        iLowerOrUpper = -1
        iLowerOrUpper = +3
c        CALL GetAbsCoeffNOJAC(daaAbsCoeff,daToffset,daaaKx,daaUx,
c     $         raPTemp,raRTemp,iaTsort,iNk,iKm,iKn,iUm,iUn,iGasID,
c     $         pProf,iProfileLayers,iSplineType,iLowerOrUpper)
...
c now compute 
c    optical depth = actual gas amount * abs coeff 
c    planck coeff  = ref    gas amount * abs coeff 

      IF ((i_NLTEFile_TYPE .GE. 100) .AND. (i_NLTEFile_TYPE .LT. 200)) THEN
        CALL AmtScale(daaAbsCoeff,raPAmt)
      ELSEIF ((i_NLTEFile_TYPE .GE. 300) .AND. (i_NLTEFile_TYPE .LT. 400)) THEN
        CALL AmtScale(daaAbsCoeff,raPAmt)
      ELSEIF ((i_NLTEFile_TYPE .GE. 200) .AND. (i_NLTEFile_TYPE .LT. 300)) THEN
        CALL AmtScale(daaAbsCoeff,raRAmt)
      ELSEIF ((i_NLTEFile_TYPE .GE. 400) .AND. (i_NLTEFile_TYPE .LT. 500)) THEN
        CALL AmtScale(daaAbsCoeff,raRAmt)
      END IF

c      do iJ = 1,kProfLayer
c        print *,i_NLTEFile_TYPE,iJ,raPAmt(iJ),raRAmt(iJ),raPAmt(iJ)/raRAmt(iJ)
c      end do

c NOTE I have not added the co2 chi functions here as they *should* be in
c the kCompressed Database produced by running kCARTA with these chi fcns on
