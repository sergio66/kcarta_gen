...
c              CALL rad_trans_SAT_LOOK_DOWN_EMISS(raFreq,
     ...
     $         raUpperPress,raUpperTemp,iDoUpperAtmNLTE,
     $         caaScatter,raaScatterPressure,raScatterDME,raScatterIWP)
        ELSEIF ((iVary .EQ. +2) .OR. (iVary .EQ. 3)) THEN
          CALL rad_trans_SAT_LOOK_DOWN_LINEAR_IN_TAU_VARY(iVary,raFreq,
     $         raInten,raVTemp,
     $         raaAbs,rTSpace,rSurfaceTemp,rSurfPress,raUseEmissivity,
     $         rSatAngle,rFracTop,rFracBot,
     $         iNp,iaOp,raaOp,iNpmix,iFileID,
...
c    for the solid angle subtended by the sun as seen from the earth
      ...
      SUBROUTINE rad_trans_SAT_LOOK_DOWN_LINEAR_IN_TAU_VARY(
...
c      CALL nir_vis_oceanrefl(raFreq,muX,muSun,muSat,raSunRefl,raSunXRefl)
      ...
      END
c************************************************************************
c this subroutine computes ocean refelction based on parameterizations
c Reference : 
...
c local var
      REAL raaAbsXL2S(kMaxPts,kMaxLayer)
      REAL rPhaseFcnRayleigh,rX,rY,ttorad_local
      INTEGER iFr,iLay,iL
      REAL rFac,raaSSA(kMaxPts,kProfLayer),raAttenuate(kMaxPts)
...
c      print *,raFreq(1),raSun(1),ttorad_local(raFreq(1),5800.0)*kOmegaSun*muSun
