...
c in kcartamisc.f
c
c      iaaOverrideDefault(2,1) = kTemperVary !!! kTemperVary .... can be reset in nm_radnce, and then subr SetkTemperVary iaaOverrideDefault(2,4)
...
c                                     !!!     = -1, fast accurate diffusive background at acos(3/5) in upper layers, accurate in lower layers << DEFAULT >>
...
c				      !!!                             = -2 for same as -1, except linear-in-tau T variation (or is it +2??)
...
c normal (cont layer temp) LTE radtransfer calcs : surface and refl thermal and atmsopheric emission
     $          raInten,raVTemp,
...
c normal (cont layer temp) LTE radtransfer plus the fast SARTA NLTE calc
...
c normal (cont layer temp) LTE radtransfer plus the slow LBL NLTE calc	    
...
c normal (cont layer temp) LTE radtransfer plus the fast (and so far incorrect) compressed NLTE calc	    	    
...
c iVary = 1 : (exp in tau layer temp) LTE radtransfer calcs : surface and refl thermal and atmsopheric emission	
...
c cannot have "extra" solar,thermal terms since the instrument is looking up
        IF (iVary .EQ. -1) THEN
c normal (cont layer temp) LTE radtransfer calcs : atmsopheric emission	
...
c iVary = 2 : (linear in tau layer temp) LTE radtransfer calcs : atmsopheric emission		
...
c      END IF
      ...
      IF (iaaOverrideDefault(2,6) .EQ. -2) THEN
...
c to keep angles constant
      ...
      IF (kLBLRTM_toa .GT. 0) THEN
        iLay = 1	
 8888   CONTINUE
        IF ((iLay .LT. iNumLayer) .AND. (raPressLevels(iaaRadLayer(iAtm,iLay)) .LT. kLBLRTM_toa)) THEN
	  iLay = iLay + 1
	  GOTO 8888
	END IF
	iLay = iLay - 1
