...
!   kcarta.param  : parameter declarations (for the array sizes)
!   scatter.param : parameters declarations for interfacing RTSPEC code
...
! ************************************************************************
      ...
      WRITE (*,'(a,a)') 'HOME =',TRIM(homedir)
      CALL getenv("PWD", homedir)
      WRITE (*,'(a,a)') 'homedir=',TRIM(homedir)
      CALL getenv("USER", homedir)
      WRITE (*,'(a,a)') 'user=',TRIM(homedir)
    
      CALL getenv("SLURM_NODELIST", homedir)
      WRITE (*,'(a,a)') 'node=',TRIM(homedir)
      CALL getenv("HOSTNAME", homedir)
      WRITE (*,'(a,a)') 'host=',TRIM(homedir)
...
! read in the driver namelist file and profile
      ...
      IF ((kLongOrShort .EQ. 0) .AND. ((kFlux .GT. 0) .OR. (kJacobian .GE. 0))) THEN
...
! outermost loop over the 10000 pt freq chunks
      ...
      !$OMP PARALLEL DEFAULT(PRIVATE)    !! loop indices are always private
      !$OMP DO
...
! first set the cumulative d/dT matrix to zero, if we need Jacobians
        IF ((kJacobian .GT. 0.) .AND. ((kActualJacs .EQ. -1) .OR. (kActualJacs .EQ. 30) .OR. &
