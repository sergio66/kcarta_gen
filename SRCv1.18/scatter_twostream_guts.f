...
c same as /home/sergio/MATLAB/RADTrans/DOWN/cloudyD25_best_prof_solar.m
...
c************************************************************************
c this checks to see if the emissions are positive or not
c flag the negatives as "bad" by keeping a running count of wot's bad
...
c      raE(iF) = max(raE(iF),0.0)
c      raF(iF) = max(raF(iF),0.0)
      IF ((raE(iF) .GT. 0).AND.(raE(iF) .LT. rEmin_positive_for_chunk))THEN
        rEmin_positive_for_chunk = raE(iF)
        END IF
      IF ((raF(iF) .GT. 0).AND.(raF(iF) .LT. rFmin_positive_for_chunk))THEN
...
c same as /home/sergio/MATLAB/RADTrans/GENERAL_CLOUD/asymcoeffs_solar.m
c here, view angle === stream angle
...
c          det = (raW0(iF)*b)**2 - alpha*alpha + (betaa*mu_pm)**2
          det = 
...
c            print *,iF,z1,z2,raG(iF),raH(iF),p_plus,p_minus,1-psi,1+psi
            END IF
...
c      DO iF = 1,kMaxPts
c        print *,iF,raE(iF),raG(iF)
c        END DO
c      CALL DoStop

c if there are "bad" emissions, set them to min(good emissions)
      iReset_raEF = +1     !!go ahead and reset things
      iReset_raEF = -1     !!leave things as they are
      IF ((avg .ge. avgmin) .AND. (iReset_raEF .gt. 0)) THEN
        IF (iEminus .GT. 0) THEN
...
c same as /home/sergio/MATLAB/RADTrans/UP/t_r_e_arb_up_Solar_prof.m
...
c same as /home/sergio/MATLAB/RADTrans/DOWN/t_r_e_arb_down_Solar_prof.m
...
c same as /home/sergio/MATLAB/RADTrans/GENERAL_CLOUD/t_r_e_streams_Solar_prof.m
...
c same as /home/sergio/MATLAB/RADTrans/GENERAL_CLOUD/addstar_arb_solar.m
...
c        print *,raEmissUp1(iF),raEmissUp2(iF),raTrUp1(iF),
c     $         raTrUp2(iF),raE1p(iF),raE2m(iF),raR1(iF),raR2star(iF),det
...
c same as /home/sergio/MATLAB/RADTrans/GENERAL_CLOUD/addstar_solar.m
...
c same as /home/sergio/MATLAB/RADTrans/GENERAL_CLOUD/update_streams_solar.m
...
c this just sets varNew == varOld
c same as /home/sergio/MATLAB/RADTrans/GENERAL_CLOUD/update_streams_arb_solar.m
...
c local variables
      ...
      IF ((rFrac .GE. 0.0) .AND. (rFrac .LE. 1.0)) THEN
        DO iFr = 1,kMaxPts
...
cccc          !!!used only by kTwoStream, apparently!
cccc          !!!log(T(i)/T(i+1))
cccc          raBeta(iFr)     = 1/raTau(iFr) * rBeta  
...
c************************************************************************
c this function computes the phase function, assuming cos(phi1-phi2) factor = 1
      ...
      IF (iPhase .GT. 0) THEN
c ********************* warning .. use rspl if the variables are real
...
c ********************* warning .. use rspl if the variables are reals
        hg2 = yexactNew
        END IF
...
c************************************************************************
c this subroutine finds out the partial fraction for layer : default = +1.0
c for use in  LayerScatteringProp
...
c output vars
      ...
      IF (iUpDown .GT. 0) THEN    !!!!down look instr : radiation going up
        IF (iL .EQ. iaRadLayer(1)) THEN
          rFrac = rFracBot
        ELSEIF (iL .EQ. iaRadLayer(iNumLayer)) THEN
          rFrac = rFracTop
          END IF
      ELSEIF (iUpDown .LT. 0) THEN    !!!!uplook instr : radiation going down
        IF (iL .EQ. iaRadLayer(1)) THEN
          rFrac = rFracTop
        ELSEIF (iL .EQ. iaRadLayer(iNumLayer)) THEN
          rFrac = rFracBot
...
c************************************************************************
c this subroutine does stuff for one layer
...
c input vars which determine how to use this subroutine
