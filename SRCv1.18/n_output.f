...
c************************************************************************
c note if SCATTER is on (use RTSPEC or DISORT), then the user can only output
c radiances at instrument height (ie -1 option, or more than one pressure level
c to output radiances at, is not allowed)
c hopefully this will be changed in the not so far future
c************************************************************************
c are ALWAYS defined wrt to a FULL layer (the pressure level boundaries of 
c which are defined in the KLAYERS files)
...
c this is for the FUTURE         Sergio Dec 8, 2000
c Note : if kScatterCode = 0     (clear sky)  then the above is always true
c      : if kScatterCode = -1,+1 (cloudy sky) then the above can be false
c         - if the "-1" option is used for pressure levels, then everything
c           is ok ie radiances at ALL pressure levels used
c         - if the "-1" option is not used for pressure levels, then pressures
c           are rounded so that they coincide as closely as possible to the 
c           pressure boundaries, so that
c           DISORT and RTSPEC can easily output radiances at layer boundaries
...
c these are temporarily used
      ...
      IF ((kRTP .EQ. -10) .OR. (kRTP .EQ. -5) .OR. (kRTP .EQ. -6)) THEN
        ...
        DO iI=1,kMaxPrint
          DO iJ=1,kProfLayer
            IF (raaOp(iI,iJ) .LT. rMaxMinLBLRTMOp) THEN
              iMaxMinLBLRTMOp = iJ
...
c          IF ((kWhichScatterCode .NE. 0) .AND. (iNp .NE. 1)) THEN
c            write(kStdErr,*) 'For DISORT,RTSPEC, can only output radiance'
c            write(kStdErr,*) 'at one level. Please reset iaNp(j) and retry'
c            CALL DoStop
c          END IF
...
c since DISORT and RTSPEC would like to output stuff at layer boundaries, we 
c have to round the pressures so that they coincide with layer boundaries
...
c if DISORT or RTSPEC, as a first cut, find out which pressure level rP 
c lies closest to. we really HAVE to be careful about up and downlook stuff
        IF ((kWhichScatterCode .NE. 0) .AND. 
     $      ((kWhichScatterCode .NE. 1) .OR. (kWhichScatterCode .NE. 4) .OR.
     $       (kWhichScatterCode .NE. 5))) THEN
          rPressDiffMin = +1.0e10
          DO i1 = 1,kProfLayer + 1
            raPressDiff(i1) = abs(rP - raPressLevels(i1))
          END DO
          DO i1 = 1,kProfLayer + 1
            IF (raPressDiff(i1) .LT. rPressDiffMin) THEN
          ...
          iSilly = 1
 111      CONTINUE
          IF ((rP .le. raPressLevels(iSilly)) .AND. 
     $        (rP .ge. raPressLevels(iSilly+1))) THEN
            GOTO 121
          ELSE
            iSilly = iSilly + 1
            GOTO 111
 ...
 121      CONTINUE
          ...
          ELSEIF (iDirection .gt. 0) THEN 
            rPScatter1 = raPressLevels(iSilly+1) !downlook, reset to layer top
            write(kStdWarn,*) 'top level of output layer is ',rPScatter1, 'mb' 
        END IF

          IF ((i2 .NE. iSilly).OR.(abs(rPscatter-rPScatter1) .ge. 1.0e-3)) THEN
            rPScatter = rPScatter1
          END IF

c having saved this in variable rPscatter, make sure you are still w/in atm!!!
          IF (iDirection .lt. 0) THEN  !!! you probably want P ~ 1200 mb
            !!!! raaPrBdry(iAtm,1) = TOA, raaPrBdry(iAtm,2) = GND 
            !!!! check that things are ok for downward dir to instr at GND
            IF (rPScatter .GT. raaPrBdry(iAtm,2)) THEN
              rPScatter = raaPrBdry(iAtm,2)
            END IF
            IF (rPScatter .LT. raaPrBdry(iAtm,1)) THEN
          ...
          IF (iDirection .gt. 0) THEN
            ...
            IF (rPScatter .GT. raaPrBdry(iAtm,1)) THEN
              rPScatter = raaPrBdry(iAtm,1)
            END IF
            IF (rPScatter .LT. raaPrBdry(iAtm,2)) THEN
              rPScatter = raaPrBdry(iAtm,2)
            END IF
          END IF
          rP = rPScatter
