...
c   gauss.param   : parameter declarations for gaussian integration 
...
c   s*.f          : string processing routines  <----> removed!!!!!
...
c this is the AIRS referennce gas amount
      REAL raaQAirs(kProfLayer,kGasStore),raQ(kProfLayer)
...
c thse are actually used
      INTEGER iDummy,iDummy2,iFound,iWhichChunk,NewDataChunk,OutsideSpectra
      INTEGER DoOutputLayer,IERR,iDoDQ,iJax,iOutNum,iCO2,iMicroSoft

c these are temporary dumy variables
c      REAL raX(kMaxPts),raY2(kMaxPts),raY3(kMaxPts) !used for splines
c      REAL rDummy,rDummy2,rDummy3,rDerivTemp,rDerivAmt
...
c read in the reference profiles if the GasID <= kGasComp ... if it is a cross
c section file, or kWaterSelf,kWaterFor, no need to do this
      DO iGas=1,iNumGases
        IF (iaGases(iGas) .LE. kGasXsecHi) THEN
          !read kCARTA kProfLayer reference profile
          CALL FindReferenceName(caFName,iaGases(iGas),1)
          CALL ReadRefProf(caFName,10,iRefLayer,raRAmt,raRTemp,
     $         raRPress,raRPartPress,raQ,iL_low,iL_high,iError)
          CALL StoreReference(raRAmt,raRTemp,raRPress,raRPartPress,raQ,
     $     raaRAmt,raaRTemp,raaRPress,raaRPartPress,raaQAirs,iGas,iaGases)
          END IF
        END DO
      WRITE(kStdWarn,*) 'Read in reference profiles .......'
...
c set up the output binary file and the output header text file
     ...
     $       iNumGases,iaGases,raaAmt,raaTemp,
...
c get the reference profile for the current gas if GAS ID <= kGasXsecHi
          IF (iaGases(iGas) .LE. kGasXsecHi) THEN
            CALL SetReference(raRAmt,raRTemp,raRPress,raRPartPress,raQ,
...
c see if current gas ID needs new spectroscopy, or if is from kComp Database
          ...
          IF (iNewIn .LT. 0) THEN
            ...
            CALL GasContribution(iGas,iaGases(iGas),iRefLayer,
     $          raRAmt,raRTemp,raRPress,raRPartPress,iL_low,iL_high,
     $          raTAmt,raTTemp,raTPress,raTPartPress,iaCont,
     $          kXsecFile,daaGasAbCoeff,raQ,
     $          raVertTemp,iVertTempSet,iFileStartFr,iTag,raFreq,iError,
     $          daaDQ,daaDT,iDoDQ)
                END IF
          IF (iNewIn .GT. 0) THEN
     ...
     $                               iFileStartFr)
            IF (iWhichChunk .GT. 0) THEN
              !read in new spectra
              CALL ReadNewData(iGas,iaGases(iGas),iRefLayer,
...
c send in the BC variables corresponding to iAtm eg rTSPace=raTSpace(iAtm)
              ...
              CALL SetAtmosphereEmissivity(iAtm,raFreq,
...
c for down look instr, calculate the layer dependent solar angles
                IF ((kSolar.GT.0).AND.(raSatHeight(iAtm) .GT. 0.0))THEN
                  CALL FindLayerAngles(1.0e5,raLayerHeight,
     $                  raaPrBdry(iAtm,1),raaPrBdry(iatm,2),
     $                  kSolarAngle,raSunAngles)
                ELSE IF ((kSolar.GT.0) .AND. 
     $                   (raSatHeight(iAtm).LT. 0.0)) THEN
                  DO iDummy=1,kProfLayer
...
c remember satellite angle==solar angle
                IF ((raTspace(iAtm) .GT. 100.0) .AND. 
     $              (raSatHeight(iAtm) .GT. 0.0))THEN
                  CALL FindLayerAngles(1.0e5,raLayerHeight,
     $                  raaPrBdry(iAtm,1),raaPrBdry(iatm,2),
     $                  raSatAngle(iAtm),raSunAngles)
                ELSE IF((raTspace(iAtm) .GT. 100.0) .AND.
     $                  (raSatHeight(iAtm).LT. 0.0)) THEN
                  DO iDummy=1,kProfLayer
              ...
              IF (kScatter .LT. 0) THEN
c %%%%%%%%%%%%% CLEAR SKY %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                write(kStdWarn,*) 'Doing Clear Sky Computations ...'
     ...
     $              raLayAngles,raSunAngles,iTag)
                ...
                IF (kFlux .GT. 0) THEN
                  write(kStdWarn,*) 'Doing Clear Sky Flux Computations ...'
                  CALL find_fluxes(raFreq,rFreqStart,rFreqEnd,
     $              raaSumAbCoeff,raMixVertTemp,caFluxFile,
     $              iOutNum,iAtm,iaNumLayer(iAtm),iaaRadLayer,
     $              raTSpace(iAtm),raTSurf(iAtm),raUseEmissivity,
     $              raSatAngle(iAtm),raFracTop(iAtm),raFracBot(iAtm),
     $              iNpmix,iFileID,iNp,iaOp,raaOp,raaMix,raInten,
     ...
     $              raLayAngles,raSunAngles,kaFrStep(iTag))
              ...
              ELSE IF (kScatter .GT. 0) THEN
c %%%%%%%%%%%%% NOT CLEAR SKY %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                write(kStdWarn,*) 'Doing Scattering Computations ...'
                CALL doscatter(raFreq,
     $              raaSumAbCoeff,raMixVertTemp,caOutName,
     $              iOutNum,iAtm,iaNumLayer(iAtm),iaaRadLayer,
     $              raTSpace(iAtm),raTSurf(iAtm),raUseEmissivity,
     $              raSatAngle(iAtm),raFracTop(iAtm),raFracBot(iAtm),
     ...
     $   iaCloudNumAtm,iaaCloudWhichAtm)
                ...
                IF (kFlux .GT. 0) THEN
                  write(kStdWarn,*) 'Doing Scattering Flux Computations ...'
                  CALL scatterfluxes(raFreq,rFreqStart,rFreqEnd,
     $              raaSumAbCoeff,raMixVertTemp,caFluxFile,
     $              iOutNum,iAtm,iaNumLayer(iAtm),iaaRadLayer,
     $              raTSpace(iAtm),raTSurf(iAtm),raUseEmissivity,
     $              raSatAngle(iAtm),raFracTop(iAtm),raFracBot(iAtm),
