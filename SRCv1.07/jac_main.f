...
c this subroutine prepares the output Jacobians according to kJacobOutput
     ...
     $                         radBTdr,raaAmt,raInten,iGasID,iM)
...
c user wants d(rad)/dq * q
            DO iFr=1,kMaxPts
              raResults(iFr)=raResults(iFr)*raaAmt(iM1,iGasID)
              END DO
          ELSE IF (kJacobOutPut .EQ. 1) THEN
c user wants d(BT)/dq * q
            DO iFr=1,kMaxPts
              raResults(iFr)=raResults(iFr)*raaAmt(iM1,iGasID)*radBTdr(iFr)
              END DO
            END IF
