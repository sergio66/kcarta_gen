...
c iGasID is the gas ID, while iFileStartFr identifies the wavenumber block
     ...
     $  raRAmt,raRTemp,raRPress,raRPartPress,iL,iU,
     $  raAmt,raTemp,raPress,raPartPress,iaCont,
     $  caXsecF,daaTemp,raQAirs,
     $  raVTemp,iVTSet,iFileStartFr,iTag,raFreq,iErr,
     $  daaDQ,daaDT,iDoDQ)
...
c iDoDQ     = -2 if no amt,temp jacs, -1 if no amt jacs, > 0 if amt,temp jacs
      REAL raAmt(kProfLayer),raTemp(kProfLayer),raQAirs(kProfLayer)
      REAL raPress(kProfLayer),raPartPress(kProfLayer)
      INTEGER iaCont(kMaxGas),iVTSet,iTag
...
c local variables
     ...
     $    raQAirs,raFreq,iErr,raAmt,raTemp,raPress,raPartPress,iaCont,
     $    daaDQ,daaDT,iDoDQ)
        END IF
     ...
     $         raQAirs,raAmt,raTemp,raPress,raPartPress,iaCont,
     $         daaDQ,daaDT,iDoDQ)
          END IF
        END IF
...
c this subroutine basically computes water continuum
     ...
     $ iTag,raQAirs,raFreq,iErr,raTAmt,raTTemp,raTPress,raTPart,iaCont,
     $ daaDQ,daaDT,iDoDQ)
...
c iDoDQ     = -2 if no amt,temp jacs, -1 if no amt jacs, > 0 if amt,temp jacs
      REAL raTAmt(kProfLayer),raTTemp(kProfLayer),raQAirs(kProfLayer)
...
c check if user wants to include continuum .. if so, compute and include
        ...
        IF ((iAns .GT. 0) .AND. (kCKD .GE. 0)) THEN
          CALL AddContinuum(iGasID,iRefLayer,raFreq,raTAmt,raTTemp,
...
c this subroutine computes the contribution of Gases 1-kGasComp (if present)
      SUBROUTINE compressed(iCount,iGasID,iRefLayer,raRAmt,raRTemp,
     $ raRPress,raRPartPress,iL,iU,daaTemp,raVTemp,iVTSet,iFileStartFr,
     $ iTag,raQAirs,raFreq,iErr,raTAmt,raTTemp,raTPress,raTPart,iaCont,
     $ daaDQ,daaDT,iDoDQ)
...
c differently than if the gas is one of the others
      ...
      IF (iErr .LE. 0) THEN
        IF (iGasID .EQ. 1) THEN
          CALL water(iGasID,iFileStartFr,iTag,iRefLayer,iL,iU,daaTemp,
     $      raTAmt,raRAmt,raTPart,raRPartPress,raTTemp,raRTemp,iErr1,
     $      daaDQ,daaDT,iDoDQ,raQAirs)
        ELSE 
          CALL othergases(iGasID,iFileStartFr,iTag,iRefLayer,iL,iU,
     $           daaTemp,raTAmt,raRAmt,raTTemp,raRTemp,iErr1,
     $           daaDQ,daaDT,iDoDQ,raQAirs)
          END IF
        IF (iErr1 .LE. 0) THEN
...
c this adds on the continuum
      SUBROUTINE AddContinuum(iGasID,iRefLayer,raFreq,raAmt,raTemp,
...
c figure out the filename
      CALL CKDFileName(caFName,iGasID)
...
c      IF (iLoc .LE. 0) THEN     !!! before June 2011
      IF (iLoc .LT. 0) THEN     !!! after June 2011
        write(kStdErr,*) iLoc
        write(kStdErr,*) iLoc,iCKD,iM,iN   
...
c interpolate
      CALL ComputeCKD(d1,d2,df,daTemprt,daaCKD,raFreq,raTemp,kFrStep,
...
c keeping track of whether the added values are all greater than 0.0
      ...
      DO iLay =1,kProfLayer
        DO iFr=1,kMaxPts
          daaTemp(iFr,iLay)=daaCon(iFr,iLay)*raMult(iLay)*296.0/raTemp(iLay)*
     $       raFreq(iFr)*tanh(kPlanck2*raFreq(iFr)/2/raTemp(iLay))
          END DO
        END DO

      IF (rECCount .gt. 0.5) THEN
        rEC=rEC/rECCount
        write(kStdWarn,*)'Error in CKD0 data!!! Some values negative!!!'
...
c************************************************************************
c this subroutine coimputes the CKD coeffs in the data file in temp,freq
c this uses cubic interpolations
      SUBROUTINE ComputeCKD(d1,d2,df,daTemprt,daaCKD,raFreq,raTemp,kFrStep,
     $                      daaCon,iDoDQ,daaDQ,daaDT,iGasID)
...
c for each layer temp, find where the nearest low CKD tempr grid point is
     ...
     $          daFrDelta(iFr)*(daaCKD(iL+1,iF+1)-daaCKD(iL+1,iF))/df
              ...
              !find out quadratic that goes thru the 3 "x" points to give "y"
              x1 = daTemprt(iL-1)
              x2 = daTemprt(iL)
              x3 = daTemprt(iL+1)
              z1=y1-y3
...
c this uses spline interpolations ........ blooody slow
      SUBROUTINE ComputeCKD1(d1,d2,df,daTemprt,daaCKD,raFreq,raTemp,kFrStep,
...
c wrt partial pressure
     ...
     $  daaAbsCoeff,raPAmt,raRAmt,raPPart,raRPart,raPTemp,raRTemp,iErr,
     $  daaDQ,daaDT,iDoDQ,raQAirs)
...
c raQAirs   = ratio of kLAYERS/KAIRS reference gas amounts
      ...
      REAL raPAmt(kProfLayer),raRAmt(kProfLayer),raQAirs(kProfLayer)
...
c local variables associated with uncompressing the water database files
     ...
     $                 daaaKX5(kMaxK,kMaxTemp,kMaxLayer)
      ...
      iIOUN=kCompUnit
      CALL CompFileName(iGasID,iFileStartFr,iTag,caFName)
...
c to get abs coeff matrix        
     ...
     $       iNk,iKm,iKn,iUm,iUn,daaDQ,daaDT,iDoDQ,raQAirs,iGasID)
      ELSE
          CALL GetAbsCoeffWaterOLD(daaAbsCoeff,daToffset,
     $      daaaKX1,daaaKX2,daaaKX3,daaaKX4,daaaKX5,daaUx,raPTemp,
     $      raRTemp,raPPart,raRPart,iaTsort,iNk,iKm,iKn,iUm,iUn,
     $      raQAirs,iGasID)
          END IF
...
c because iKtype=2 do any necessary jacobians calcs HERE!
        IF (kJacobian .GT. 0) THEN
          IF (iDoDQ .GT. 0)  THEN
            CALL FinalWaterAmtDeriv(iKtype,daaAbsCoeff,daaDQ,
     $                            raPAmt,raRAmt)
            END IF
          CALL FinalTempDeriv(iKtype,daaAbsCoeff,daaDT,raPAmt,raRAmt)
          END IF
...
c because of iKtype=1,2 possibility, do any necessary jacobians calcs HERE!
        IF (kJacobian .GE. 0) THEN
          CALL FinalTempDeriv(iKtype,daaAbsCoeff,daaDT,raPAmt,raRAmt)
          IF (iDoDQ .GT. 0) THEN
            CALL FinalAmtDeriv(daaDQ,raRAmt,iKtype)
            END IF
          END IF
...
c scale the coefficients for the ratios of amounts
      CALL AmtScale(daaAbsCoeff,raPAmt,raRAmt)
...
c have to send in ref temp, amount and profile temp,amount
     ...
     $          daaAbsCoeff,raPAmt,raRAmt,raPTemp,raRTemp,iErr,
     $          daaDQ,daaDT,iDoDQ,raQAirs)
...
c this subroutine finishes the calculation of d/dq(abscoeff)
      SUBROUTINE FinalAmtDeriv(daaDQ,raRAmt,iType)

      include 'kcarta.param'

c daaDT   = d/dT
c raRAmt  = reference gas amounts
c iType   = compression type
      INTEGER iType
      REAL raRAmt(kProfLayer)
      DOUBLE PRECISION daaDQ(kMaxPtsJac,kProfLayerJac)
      ...
      IF (iType .EQ. 2) THEN
        DO iLay = 1,kProfLayerJac
          DO iFr=1,kMaxPtsJac
            daaDQ(iFr,iLay)=(daaDQ(iFr,iLay)**4)/raRAmt(iLay)
            END DO
          END DO
      ELSE
        DO iLay = 1,kProfLayerJac
          DO iFr=1,kMaxPtsJac
            daaDQ(iFr,iLay)=(daaDQ(iFr,iLay))/raRAmt(iLay)
            END DO
          END DO
        END IF
...
c this subroutine finishes the computation of d/dT (absCoeff) 
      SUBROUTINE FinalTempDeriv(iKtype,daaAbsCoeff,daaDA,raPAmt,raRAmt)
...
c ra(P/R)Amt  = reference/actual gas amounts
      INTEGER iKtype
      REAL raPAmt(kProfLayer),raRAmt(kProfLayer)
      DOUBLE PRECISION daaAbsCoeff(kMaxPts,kProfLayer)
...
c local variables
      ...
      IF (iKtype .EQ. 1) THEN
        DO iLay=1,kProfLayer
          rScale=raPAmt(iLay)/raRAmt(iLay)
          DO iFr=1,kMaxPts
...
c local variables
      ...
      DO iLay=1,kProfLayer
        ...
        DO iFr=1,kMaxPts
          daaDA(iFr,iLay)=daaDA(iFr,iLay)*rScale*
     $                      4.0*(daaAbsCoeff(iFr,iLay)**3)
     $        + (daaAbsCoeff(iFr,iLay)**4)/raRAmt(iLay)
          END DO
        END DO
...
c amounts in the reference and gas profiles
      SUBROUTINE AmtScale(daaAbsCoeff,raPAmt,raRAmt)
...
c************************************************************************
c this subroutine basically computes water continuum
      SUBROUTINE newgasesOLD(iCount,iGasID,iRefLayer,raRAmt,raRTemp,
     $ raRPress,raRPartPress,iL,iU,daaTemp,raVTemp,iVTSet,iFileStartFr,
     $ iTag,raQAirs,raFreq,iErr,raTAmt,raTTemp,raTPress,raTPart,iaCont,
     $ daaDQ,daaDT,iDoDQ)
...
c check if user wants to include continuum .. if so, compute and include
        ...
        kFrStep=kaFrStep(iTag)
        IF ((iAns .GT. 0) .AND. (kCKD .GE. 0)) THEN
          CALL AddContinuumOLD(iGasID,iRefLayer,raFreq,raTAmt,raTTemp,
     $                        kFrStep,raTPress,raTPart,iL,iU,daaTemp,
     $                        daaDQ,daaDT,iDoDQ)
          write(kStdWarn,*) 'added self water continuum'
          END IF 
...
c************************************************************************
      SUBROUTINE AddContinuumOLD(iGasID,iRefLayer,raFreq,raAmt,raTemp,
     $ kFrStep,raPress,raPartPress,iL,iU,daaTemp,daaDQ,daaDT,iDoDQ)
...
c kFrStep  = kFreqStep
      DOUBLE PRECISION daaTemp(kMaxPts,kProfLayer)
      INTEGER iGasID,iL,iU,iRefLayer,iDoDQ
      REAL raFreq(kMaxPts),kFrStep
      REAL raAmt(kProfLayer),raTemp(kProfLayer)
      REAL raPartPress(kProfLayer),raPress(kProfLayer)
      DOUBLE PRECISION daaDQ(kMaxPtsJac,kProfLayerJac)
...
c local variables
      ...
      REAL rFStep,rMin
      REAL raaCon(kMaxPts,kProfLayer),rEC,rECCount
      REAL rSelfMult,rForMult

c these are for the Jacobian calculations
...
c initialize raaCon to all zeros
      DO iLay =1,kProfLayer
        DO iFr=1,kMaxPts
          raaCon(iFr,iLay)=0.0
          daaTemp(iFr,iLay)=0.0
          END DO
        END DO

      IF (kJacobian .GT. 0) THEN
c need to initialize the SPLINE(T),SPLINE(q) matrices
        DO iLay =1,kProfLayerJac
          DO iFr=1,kMaxPtsJac
...
c calculate the continuum contribution
      ...
      rForMult=1.0
      IF (iGasID .EQ. kNewGasLo) THEN
        rFormult=0.0          !compute self only
        END IF
      IF (iGasID .EQ. kNewGasHi) THEN
        rSelfmult=0.0          !compute for only
        END IF

c if kJacobian == -1 this call only calculates raaCon, leaving raaSplineDQ,
c raaSplineDT untouched
      IF (kCKD .EQ. 0) THEN
        CALL calcon00(1,iFr,raFreq,rFStep,iLay,raTemp,raPress,
     $    raPartPress, raAmt, raaCon, raaSplineDQ, raaSplineDT,iDoDQ,
     $    rSelfMult,rForMult)           
      ELSEIF (kCKD .EQ. 21) THEN
        CALL calcon21(1,iFr,raFreq,rFStep,iLay,raTemp,raPress,
     $    raPartPress, raAmt, raaCon, raaSplineDQ, raaSplineDT,iDoDQ,
     $    rSelfMult,rForMult)           
      ELSEIF (kCKD .EQ. 23) THEN
        CALL calcon23(1,iFr,raFreq,rFStep,iLay,raTemp,raPress,
     $    raPartPress, raAmt, raaCon, raaSplineDQ, raaSplineDT,iDoDQ,
     $    rSelfMult,rForMult)           
      ELSEIF (kCKD .EQ. 24) THEN
        CALL calcon24(1,iFr,raFreq,rFStep,iLay,raTemp,raPress,
     $    raPartPress, raAmt, raaCon, raaSplineDQ, raaSplineDT,iDoDQ,
     $    rSelfMult,rForMult)           
        END IF
c if kJacobian == 0,1 AND kSpline == 1 then this call will automatically
c  calculate the continuum contributions to daaDQ,daaDT, by storing the
c  relevant contributions in raaSplineDQ, raaSplineDT
...
c keeping track of whether the added values are all greater than 0.0
      ...
      DO iLay =1,kProfLayer
        DO iFr=1,kMaxPts
          IF (raaCon(iFr,iLay) .LT. 0.0) THEN
            rEC=rEC+raaCon(iFr,iLay)
            rECCount=rECCount+1
            IF (raaCon(iFr,iLay) .LT. rMin) THEN
              rMin = raaCon(iFr,iLay)
              END IF
            raaCon(iFr,iLay)=0.0
            END IF
          daaTemp(iFr,iLay)=daaTemp(iFr,iLay)+raaCon(iFr,iLay)
          END DO
        END DO
      ...
      IF (kJacobian .GT. 0) THEN
c do the inclusion of the spline results here
        DO iLay =1,kProfLayerJac
          DO iFr=1,kMaxPtsJac
            daaDT(iFr,iLay)=daaDT(iFr,iLay)+raaSplineDT(iFr,iLay)
            END DO
          END DO
        IF (iDoDq .GT. 0) THEN
          DO iLay =1,kProfLayerJac
            DO iFr=1,kMaxPtsJac
             daaDQ(iFr,iLay)=daaDQ(iFr,iLay)+raaSplineDQ(iFr,iLay)
              END DO
            END DO
          END IF
        END IF
...
c compute the contribution of Gases 29-63 (if present) 
     ...
     $      raQAirs,raTAmt,raTTemp,raTPress,raTPart,iaCont, 
     $      daaDQ,daaDT,iDoDQ) 
...
c kFrStep  = kFreqStep 
      ...
      REAL raTAmt(kProfLayer),raTTemp(kProfLayer) 
      REAL raFreq(kMaxPts),raVTemp(kProfLayer),raQAirs(kProfLayer) 
      DOUBLE PRECISION daaDQ(kMaxPtsJac,kProfLayerJac) 
      DOUBLE PRECISION daaDT(kMaxPtsJac,kProfLayerJac) 
...
c them in raaSplineDQ ,raaSplineDT respectively 
     ...
     $      iLay, raQAirs, raAdjustProf,raXamnt,iNmol,iaMolid, 
     $      raaXsec,iWhichUsed, 
