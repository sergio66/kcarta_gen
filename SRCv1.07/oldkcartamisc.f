...
c do not make sense .. 
      ...
      IF ((kCKD .NE. 0) .AND. (kCKD .NE. -1) .AND. (kCKD .NE. 21)
     $     .AND. (kCKD .NE. 23) .AND. (kCKD .NE. 24)) THEN
        write(kStdErr,*) 'In *PARAMS,  need kCKD = -1,0,21,23 or 24'
        write(kStdErr,*) 'kCKD is water continuum calculation version'
        write(kStdErr,*) 'Please reset and retry'
        CALL DoSTOP 
        END IF
c      IF ((kCKD .EQ. 24)) THEN
c        write(kStdErr,*) 'In *PARAMS,  kCKD = 24 cannot be used as the'
c        write(kStdErr,*) 'database does not use a local lineshape'
c        write(kStdErr,*) 'This will be fixed in future versions'
