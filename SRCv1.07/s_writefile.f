...
c kLongOrShort enables shorted output files to be written out
...
c iTotal = how many of the kcomp files are gonna be unchunked
      ...
      INTEGER iNatmJac,iaLayerJac(kMaxAtm),iIOUN6
      REAL raParams(kMaxUserSet)
...
c now also output the pressure level info NEW!!!!!!!!!!
        WRITE(iIOUN,*) 'M1000mb,M100mb,MSub = ',M1000mb,M100mb,MSubLayer
        WRITE(iIOUN,*) 'M50mb,M10mb,MThick = ',M50mb,M10mb,MThickLayer
        !!!!!! this worked v1.03-1.05 !!! WRITE(iIOUN,*) (pProf(iI),iI=1,kProfLayer+1)
        WRITE(iIOUN,*) (pProf(iI),iI=1,kProfLayer)
        WRITE(iIOUN,*) '***********************************************'
          ...
          DO iJ=1,iOutTypes
            IF ((iaPrinter(iJ) .EQ. 3).AND.(iaAtmPr(iJ).EQ.iI)) THEN
              ...
              IF ((kScatter .GT. 0) .AND. (iaNp(iJ) .GT. 1)) THEN
                write(kStdErr,*)'Atm #',iI,' too many layers to be printed!'
              ...
              IF (iaNp(iJ) .GT. 0) THEN
                iEnd=iaNp(iJ)
                WRITE(iIOUN,*) '# of MP layers to be printed=',iEnd
...
c now also output the pressure level info NEW!!!!!!!!!!
      ...
      !!this worked v1.03-1.05!! WRITE(iIOUN1) (pProf(iI),iI=1,kProfLayer+1)
      WRITE(iIOUN1) (pProf(iI),iI=1,kProfLayer)        
...
c open unformatted file as a fresh file to be written to
        OPEN(UNIT=iIOUN6,FILE=caFluxFile,STATUS='NEW',
     $      FORM='UNFORMATTED',IOSTAT=iFileErr)
c if file error, inform user and stop program
        IF (iFileErr .NE. 0) THEN
...
c set the number of layers in this atmosphere
        ...
        WRITE(iIOUN6) iNatmJac
        WRITE(iIOUN6) (iaLayerJac(iI),iI=1,iNatmJac)
...
c for each atmosphere, how many layers
        write(iIOUN6) iTotal,iNatmJac,iImportant
        write(iIOUN6) (iaLayerJac(iI),iI=1,iNatmJac)               
