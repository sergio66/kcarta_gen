...
!         = array initialized to sum(k) from TOA to instr if instr inside atm
    ...
    REAL :: waveno,raRad(kMaxPts),rak(kMaxPts),mudown
      ...
      DO i=1,kMaxClouds
        IF (iaCloudWithThisAtm(i) == 1) THEN
          ...
          DO iStep = 1, iaCloudNumLayers(i)
            ...
            raCldLayer(iLayers) = +1.0 * (iT_atm - iaaCloudWhichLayers(i,iStep) + 1)
...
!        rOmegaSun = 6.785087652174316e-5      version on Jan 2006
    ...
    raSun = raSun * rOmegaSun * rExtraFac
...
! c this is new .. where subroutine differs from Find_Radiance_TOA_to_Instr
    ...
    IF (iExtra > 0) THEN
      DO iI = iT,iNumLayer+1,-1
        iJ = iaRadLayerTemp(iI)
        raExtra = raExtra + raaAbs(:,iJ)
      END DO
      DO iI = iNumLayer,iNumLayer
        iJ = iaRadLayerTemp(iI)
        raExtra = raExtra + raaAbs(:,iJ)*(1-rFracTop)
      END DO
    ...
    IF (rScat > 0.0) THEN
      ...
      DO N = ICLDTOP, ICLDBOT-1
        ...
        DO iFr = 1,kMaxPts
          ...
          raaScatTemp(iFr,iI) = IWP(L)*EXTINCT/1000*SSALB(L)
          SSALB(L) = SSALB(L)*TAUC_L/TAUCG_L
          raaScatTemp(iFr,iI) = SSALB(L)*TAUTOT_N
          ...
          !     $          raaExtTemp(iFr,iI),raaScatTemp(iFr,iI),raaAsymTemp(iFr,iI)
    ...
    IF (rScat > 0.0) THEN
      ...
      DO N = ICLDTOP, ICLDBOT-1
        ...
        DO iFr = 1,kMaxPts
          ...
          ! related by raaXYZJacobRME(iFr,iI) = raaXYZJacobDME(iFr,iI)
          ...
          raaExtJacobIWP(iFr,iI) = TAUTOT_N/TAUCG_L*x1 + TAUCG_L*(b-1)*x2
           
          x2 = OMEGA*EXTINCT/1000*TAUGAS/(TAUCG_L**2)
          raaSSAlbJacobIWP(iFr,iI) = x2
          ...
          x3 = -1/2*dASYM_dr
          raaExtJacobDME(iFr,iI) = TAUTOT_N/TAUCG_L*x1 + TAUCG_L*(b-1)*x2 + &
          TAUCG_L*SSALB(L)*x3

          x4 = EXTINCT*IWP(L)/1000/TAUCG_L
          x5 = tautotal_0*SSALB(L)*dEXTINCT_dr*(1-x4)
          x2 = IWP(L)/1000*x5/(TAUCG_L**2) + x4*dSSALB_dr
          raaSSAlbJacobDME(iFr,iI) = x2

          raaAsymJacobDME(iFr,iI) = dASYM_dr

          !! --------> d/d(g) <---------  !!
          raaPhaseJacobASYM(iFr,iI) = hg2_real_deriv_wrt_g(-mu_sun,mu_sat,ASYM)
...
! ---------------------------- local variables ----------------------------
    ...
    INTEGER :: iFr,iG,iaRadLayer(kProfLayer),iC
    ...
    raTempLay(1:iNumLayer)  = raVtemp(iaRadLayer(1:iNumLayer))
...
!!!!first find out where the cloud top is, in kCARTA layering
    ...
    DO iG = 1,iNclouds
      ...
      DO iFr = 1,kMaxPts
        ...
        raE(iFr) = EXTINCT/1000
        raW(iFr) = SSALB(L)
        raG(iFr) = ASYM_RTSPEC(L)
      END DO          !loop over freqs
...
! first sum over optical depths to find (weighted) total optical depth
    DO iG = 1,iNclouds
      DO iI = 1,iNumLayer
        ...
        raaExtTemp(:,iL) = max(raaExtTemp(:,iL) + raaEAll(iG,:,iL),0.0)
      END DO
    END DO
...
! now find the weighted single scattering parameter
    DO iG = 1,iNclouds
      DO iI = 1,iNumLayer
        iL = iaaRadLayer(iAtm,iI)
        raX = raaWAll(iG,:,iL)*raaEAll(iG,:,iL)
        raY = raaExtTemp(:,iL)
        raaScatTemp(:,iL) = max(raaScatTemp(:,iL)+(raX)/(raY+1.0e-16),0.0)
      END DO
    END DO
...
! ---->> flip things for the code to work well : flip the layers!!!! <<----
    DO iG = 1,iNumLayer
      ...
      raaEall(1,:,iI) = raaExtTemp(:,iL)
      raaWall(1,:,iI) = raaScatTemp(:,iL)
...
! temporarily save things in raaWAll,raaGAll,raaEall for later flip
    DO iG = 1,iNumLayer
      ...
      DO iFr = 1,kMaxPts
        waveno = raFreq(iFr)
        taugas = raaGasAbs(iFr,iI)
        rAbs   = taugas
        !  Compute the optical depth of cloud layer, including gas
        TAUC_L   = raaEall(1,iFr,iI)
        TAUCG_L  = TAUGAS + TAUC_L
        TAUTOT_N = TAUCG_L
        ! the SSALB coeff
...
! ow get the optical properties for the cloud layers
    ...
    DO iG = 1,iNumLayer
      ...
      DO iFr = 1,kMaxPts
        ...
        raaSSAlbTemp(iFr,iL) = SSALB(iI)

        ! ---------------> now add on the backscattered part <--------------------
        b = (1.0 - ASYM_RTSPEC(iI))/2.0
        TAUTOT_N = TAUTOT_N * (1 - SSALB(iI)*(1.0-b))
        raaExtTemp(iFr,iL)  = TAUTOT_N
        ! ---------------> now add on the backscattered part <--------------------

        IF (IWP(iI) >= 1.0e-5) THEN
          raaAsymTemp(iFr,iL) = ASYM_RTSPEC(iI)
        ELSE
        ...
        x1 = EXTINCT/1000
        x2 = OMEGA*EXTINCT/1000*TAUGAS/(TAUCG_L**2)
        raaExtJacobIWP(iFr,iL) = TAUTOT_N/TAUCG_L*x1 + TAUCG_L*(b-1)*x2
         
        x2 = OMEGA*EXTINCT/1000*TAUGAS/(TAUCG_L**2)
        raaSSAlbJacobIWP(iFr,iL) = x2

        raaAsymJacobIWP(iFr,iL) = 0.0
        ...
        x3 = -1/2*dASYM_dr
        raaExtJacobDME(iFr,iL) = TAUTOT_N/TAUCG_L*x1 + TAUCG_L*(b-1)*x2 + &
        TAUCG_L*SSALB(iI)*x3

        x4 = EXTINCT*IWP(iI)/1000/TAUCG_L
        x5 = tautotal_0*SSALB(iI)*dEXTINCT_dr*(1-x4)
        x2 = IWP(iI)/1000*x5/(TAUCG_L**2) + x4*dSSALB_dr
        raaSSAlbJacobDME(iFr,iL) = x2

        raaAsymJacobDME(iFr,iI) = dASYM_dr

        !! --------> d/d(g) <---------  !!
        raaPhaseJacobASYM(iFr,iL) = hg2_real_deriv_wrt_g(-mu_sun,mu_sat,ASYM)
    ...
    IF (IWP(1) > 0.0) THEN
      ...
      DO N = 1,iNumLayer
        ...
        DO iFr = 1,kMaxPts
          ...
          x2 = IWP(L)/1000*x5/(TAUCG_L**2) + x4*dSSALB_dr
          x3 = -1/2*dASYM_dr
          raaExtJacobDME(iFr,iI) = TAUTOT_N/TAUCG_L*x1 + TAUCG_L*(b-1)*x2 + TAUCG_L*SSALB(L)*x3
