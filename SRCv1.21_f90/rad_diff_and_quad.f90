...
! factor from the azimuthal integration
    raThermal = raThermal*2.0*kPi
    raExtraThermal = raExtraThermal*2.0*kPi
...
! local variables
    ...
    REAL :: rMPTemp,raFreqAngle(kMaxPts),raFreqAngle_m1(kMaxPts),raPlanck(kMaxPts)

! to do the angular integration
    REAL :: raAngleTr_m1(kMaxPts),raAngleTr(kMaxPts)
    ...
    raAvgAnglePerLayer = 0.0
    raMeanOD = 0.0

    DO iLay = 1,kMaxLayer
      raMeanOD(iLay) = raMeanOD(iLay) + sum(raaOrigAbsCoeff(:,iLay))
    END DO
...
! calculate raL2Gm1 which is the L2G optical depth from TOA layer to ground
    DO iLay = iS0-1,1,-1
      ...
      IF (iLay /= 1) THEN
        raL2Gm1 = raL2Gm1 + raaOrigAbsCoeff(:,iL)
      ELSE
        raL2Gm1 = raL2Gm1 + raaOrigAbsCoeff(:,iL)*rFracBot
      END IF
    END DO
...
! do not have to worry about fractional top layers here, 'cos abs coeffs are
! for FULL layers!
    raL2G = raL2Gm1 + raaOrigAbsCoeff(:,iL)
...
! do top part of atmosphere, where we can use acos(3/5)
    IF ((iCase == 1)  .OR. (iCase == 2)) THEN
      ...
      DO iLay=iS0,iBdryp1_O,-1
        ...
        raAvgAnglePerLayer(iL) =  raAvgAnglePerLayer(iL) + rCosDiff*kMaxPts
    ...
    IF ((iCase == 1) .OR. (iCase == 3)) THEN
      ...
      IF (iE0 == 1) THEN
        ...
        raAngleTr = raFindDiffusiveAngleExp(raL2G)
        raFreqAngle = raAngleTr
        rCosDiff = raFreqAngle(1)
...
! do top part of atmosphere, where we can use acos(3/5)
    IF ((iCase == 1)  .OR. (iCase == 2)) THEN
      ...
      DO iLay=iS0,iBdryp1_O,-1
        ...
        END IF
        ! find the diffusive angles for the layer beneath
        raAngleTr_m1 = exp(-raL2Gm1/raCosDiff)
        raAngleTr    = exp(-raL2G/raCosDiff)
        ! get ready for the layer beneath
        raL2G   = raL2Gm1
        raL2Gm1 = raL2Gm1 - raaOrigAbsCoeff(:,iLm1)*rW
    ...
    IF ((iCase == 1) .OR. (iCase == 3)) THEN
      ...
      DO iLay=iBdry0,iSecondEnd,-1
        ...
        raFreqAngle_m1       = raAngleTr_m1
        raAngleTr_m1         = exp(-raL2Gm1/raAngleTr_m1)
        raAngleTr            = exp(-raL2G/raFreqAngle)
        ! get ready for the layer beneath
        raL2G       = raL2Gm1
        raL2Gm1     = raL2Gm1-raaOrigAbsCoeff(:,iLm1)*rW
        raFreqAngle = raFreqAngle_m1
        ! raCosDiff   = raFreqAngle       ! returns cosine BUT THIS ANGLE TOO SMALL in WINDOW
        ! raCosDiff   = raFreqAngle-0.05  ! returns cosine ADJUST THIS ANGLE, over compensates
        ! raCosDiff   = raFreqAngle-0.1   ! returns cosine ADJUST THIS ANGLE, over compensates
        ! raCosDiff   = raFreqAngle+0.1   ! returns cosine ADJUST THIS ANGLE, under compensates
        raCosDiff   = raFreqAngle+0.05  ! returns cosine ADJUST THIS ANGLE, best ****
        raCosDiff   = min(max(raCosDiff,0.0),1.0)
        CALL RT_ProfileDNWELL_LINEAR_IN_TAU_FORFLUX_ang(raFreq,raaOrigAbsCoeff,iL,raTPressLevels,raVT1, &
            raCosDiff,1.0, &
            iVary,raTemp)
      END DO
...
! this is the thermal diffusive approx ==> multiply by 0.5
    ...
    IF ((iExtraThermal > 0) .AND. (kJacobian > 0)) THEN
      raExtraThermal = raExtraThermal*0.5
    END IF
...
!              this would affect the backgnd thermal calculation
    ...
    REAL :: raPlanck(kMaxPts),raIntenAtmos(kMaxPts)

    REAL :: rMPTemp,raAngleTrans(kMaxPts),raAngleEmission(kMaxPts)
    INTEGER :: iL,iLay
    ...
    IF (iExtraThermal < 0) THEN
      ...
      DO iAngle = 1,kGauss
        ...
        raThermal = raThermal+raTemp*SNGL(daGaussWt(iAngle))*rCosAngle
      END DO
    END IF
    ...
    IF (iExtraThermal > 0) THEN
      ...
      DO iAngle = 1,kGauss
        ...
        raExtraThermal = raExtraThermal+raTemp*SNGL(daGaussWt(iAngle))*rCosAngle
...
!              this would affect the backgnd thermal calculation
    ...
    IF (iExtraThermal < 0) THEN
      ...
      DO iAngle = 1,iGaussPts
        ...
        ! COMMENTED OUT           raThermal = raThermal+raTemp*SNGL(daGaussWt(iAngle))*rCosAngle
         raThermal = raThermal+raTemp*SNGL(daGaussWt(iAngle))
        END DO
    END IF
    ...
    IF (iExtraThermal > 0) THEN
      ...
      DO iAngle = 1,iGaussPts
        ...
        DO iLay = iT,iNumLayer+1,-1
          ...
          ! add the contribution from this angle to raThermal -- do the weighting AFTER
          ! the next set of loops
          raExtraThermal = raExtraThermal+raTemp*SNGL(daGaussWt(iAngle))
        END DO
...
! local variables
    ...
    REAL :: raPlanck(kMaxPts),rMPTemp
    REAL :: raFreqAngle(kMaxPts),raFreqAngle_m1(kMaxPts)
    REAL :: raAvgAnglePerLayer(kProfLayer),raMeanOD(kProfLayer)
          
! to do the angular integration
    REAL :: raAngleTr_m1(kMaxPts),raAngleTr(kMaxPts),raL2G(kMaxPts),raL2Gm1(kMaxPts)
...
! initalize raL2G,raL2Gm1
    ...
    DO iLay = 1,kMaxLayer
      raMeanOD(iLay) = raMeanOD(iLay) + sum(raaAbs(:,iLay))
    END DO
...
! start at TOA
    DO iLay = iS-1,2,-1
      iL = iaRadLayer(iLay)
      raL2Gm1 = raL2Gm1 + raaAbs(:,iL)
    END DO
    DO iLay = 1,1
      iL = iaRadLayer(iLay)
      raL2Gm1 = raL2Gm1 + raaAbs(:,iL)*rFracBot
    END DO
     
! calculate raL2G which is the L2G transmission from layer iS to ground
! and initialise the angles
    iL = iaRadLayer(iS)
    raL2G = raL2Gm1 + raaAbs(:,iL)
    raFreqAngle = raFindDiffusiveAngleExp(raL2G)
...
! 1/e^2 ~ 0.3678*0.3678 =    0.13533528323661
    ...
    FUNCTION raFindDiffusiveAngleExp(rak)
    ...
    REAL :: raX(kMaxPts),raL(kLarge),raM(kMedium),raS(kSmall),raVS(kVerySmall)
    INTEGER :: iI,iFr
    
    REAL :: rak(kMaxPts)
    REAL :: raFindDiffusiveAngleExp(kMaxPts)
...
! this is for region 5 < k < 20
    ...
    raX = 0.5   !! diffusive approx     
    DO iFr = 1,kMaxPts
      IF (raK(iFr) <= 1.0e-4) THEN
        raX(iFr) = 0.5
      ELSE IF ((raK(iFr) > 1.0e-4) .AND. (raK(iFr) <= 5.0e-2)) THEN
        raX(iFr) = 0.0
        DO iI = 0,kVerySmall-1
          raX(iFr) = raVS(iI+1)*(raK(iFr)**iI) + raX(iFr)
        END DO
      ELSE IF ((raK(iFr) > 5.0e-2) .AND. (raK(iFr) <= 0.1)) THEN
        raX(iFr) = 0.0
        DO iI = 0,kSmall-1
          raX(iFr) = raS(iI+1)*(raK(iFr)**iI) + raX(iFr)
        END DO
      ELSE IF ((raK(iFr) > 0.1) .AND. (raK(iFr) <= 5.0)) THEN
        raX(iFr) = 0.0
        DO iI = 0,kMedium-1
          raX(iFr) = raM(iI+1)*(raK(iFr)**iI) + raX(iFr)
        END DO
      ELSE IF ((raK(iFr) > 5.0) .AND. (raK(iFr) <= 20.0)) THEN
        raX(iFr) = 0.0
        DO iI = 0,kLarge-1
          raX(iFr) = raL(iI+1)*(raK(iFr)**iI) + raX(iFr)
        END DO
      ELSE IF (raK(iFr) > 20.0) THEN
        raX(iFr) = 0.8700244
      END IF
    END DO

    raFindDiffusiveAngleExp = raX           !!!! directly return the COSINE

! debug!!!!!! this is to set the textbook diffusive approx
!      FindDiffusiveAngleExp = kThermalAngle*kPi/180.0
     
    RETURN
    end FUNCTION raFindDiffusiveAngleExp
...
! by cos(90) == 0.0!!! so do everything upto just below 90 degrees
    IF (iExtraThermal < 0) THEN
      ...
      DO iAngle = 0,iPi-1
        ...
        ! been included in the RadTr routine
         raThermal = raThermal + raTemp*sin(rAngle)
        END DO
      ! now multiply by rDelta
      raThermal = raThermal*rDelta
    END IF
    ...
    IF (iExtraThermal > 0) THEN
      ...
      DO iAngle = 0,iPi-1
        ...
        raExtraThermal = raExtraThermal+raTemp*sin(rAngle)*cos(rAngle)
        ...
        ! add the contribution from this angle to raThermal -- the sin(theta) is from
        ! the solid angle contribution
        raThermal = raThermal+raTemp*sin(rAngle)*cos(rAngle)
      END DO

      ! now multiply by rDelta
      raExtraThermal = raExtraThermal*rDelta
      raThermal = raThermal*rDelta
