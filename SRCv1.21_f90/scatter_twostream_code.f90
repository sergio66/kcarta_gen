...
! -->>> compute downward solar radiation incident at cloud top
    ...
    ! his is what compares best with DISORT, before Jan 2006 when I was
    ! essing up factors of pi
    !!!musun = musun * kOmegaSun/kPi

    ! his is what I think it should be, after Jan 2006
    ! ad to fix up SUBR SolarBeam so that there is a factor of pi there
    ! hich should work out fine for DISORT
    ! musun = musun * kOmegaSun

    ! f you take it out, you need to modify SolarScatter accordingly
        ...
        DO iLay = iLocalCldTop,iLocalCldBot
            ...
            ! since we need to go do the scattering calculation
                ...
                IF (iDp > 0) THEN
                    ...
                    ! nly output at final TWOSTREAM pass from CLD to GND
                        write(kStdWarn,*) 'output',iDp,' rads at',iLay,' th rad layer'
                        DO iOutput=1,iDp
                            CALL RadianceInterPolate(-1,raOutFrac(iOutput),raFreq, &
                            raVTemp,muSat,iLay,iaRadLayer,raaExt,raInten,raInten2, &
...
! this is for the solar radiation coming downwards
    IF (iDoSolar >= 0) THEN
        IF (iRepeat == 0) THEN
        ! eep on computing the solar radiation as it comes down atm
            DO iLay = iLocalCldBot+1,iNumLayer-1
                iL        = iaRadLayer(iLay)
...
! this is if the blahX are double
    ...
    DO iFr = 1,kMaxPts
        ...
        if ((raDiffuseInten(iFr) > 800.00) .OR. &
        (raDiffuseInten(iFr) < 0.00)) then
        ! at 500K, max radiance ~ 0.8 W m-2 sr-2 cm-1 (at about 1000 cm-1)
...
!     END DO
    ...
    IF (kSolar >= 0) THEN
        DO iF = 1,kMaxPts
            ...
            raEmissUp(iF) + raSunUp(iF)*radSolarCld(iF)
            raInten(iF) = (raInten(iF) + rad)*exp(-raTau(iF)/abs(mu_view))
        ! un          rad = raSunUp(iF)*radSolarCld(iF)
        ! un          raInten(iF) = rad*exp(-raTau(iF)/abs(mu_view))
        END DO
    ELSE
        DO iF = 1,kMaxPts
            rad = raRadBb(iF)*raTrUp(iF)+raRadBt(iF)*raReUp(iF)+raEmissUp(iF)
...
! cc           raSun(iFr)=raSun(iFr)*exp(-raKAbs(iFr))/kPi
    DO iLay=1,1
        ...
        ! Pi before May 2003
        ! Pi           raSun(iFr)=raSun(iFr)*exp(-raKAbs(iFr))*muSat
        ! Pi after May 2003
