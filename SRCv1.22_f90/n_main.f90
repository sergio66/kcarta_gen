...
! default NLTE
    ...
    iaNLTEChunks     = -9999   !number of NLTE chunks
...
! test overrides
    ...
    IF (iaaOverride(2,1) /= iaaOverrideDefault(2,1)) THEN
      write(kStdWarn,'(A,2(I3))') & 
        'At beginning of n_main we set kTemperVary in, iaaOverrideDefault(2,1) = ',kTemperVary,iaaOverrideDefault(2,1)
      write(kStdWarn,'(A,I3)') & 
        '       but                 in nml file : user set    iaaOverride(2,1) = ',iaaOverride(2,1)
      write(kStdWarn,'(A,I3,A)') & 
        '                           now setting kTemperVary = iaaOverride(2,1) = ',iaaOverride(2,1),' ... '
      kTemperVary = iaaOverride(2,1)
    END IF

    iaaOverrideDefault = iaaOverride
    write(kStdWarn,'(A,3(I3))') & 
      ' middle n_main, kTemperVary,iaaOverrideDefault,iaaOverride = ',kTemperVary,iaaOverrideDefault(2,1),iaaOverride(2,1)
    ...
    DO iI  =  1,kMaxAtm
      IF (raSatHeight(iI) < 0) THEN
        write(kStdWarn,FMT) 'atm no. ',iI,' raAtmLoop raSatHeight = ',raSatHeight(iI), 'reset to 705 km'
        raSatHeight(iI) = 705000.0
      END IF

      IF ((raKSolarAngle(iI) < -90.0) .AND. (raKSolarAngle(iI) >=  -120)) THEN
        write(kStdWarn,'(A,I2,A,F10.4,A)') &
        ' probably iNumNLTEGases = 1 : profile ',iI,' resetting solar angle from ',raKSolarAngle(iI),' to 89.9'
...
! now go ahead and read in the new data
    ...
    write(kStdWarn,*) ' '
    write(kStdWarn,'(A,2(I3))') 'before OverrideNML_nmParam_nmSpectra, kTemperVary,iaaOverrideDefault(2,1) = ',& 
      kTemperVary,iaaOverrideDefault(2,1)
...
!    END IF
    ...
    iaaOverrideDefault = iaaOverride
    write(kStdWarn,'(A,3(I3))') & 
      ' middle OverrideNML_nmParam_nmSpectra, kTemperVary,iaaOverrideDefault,iaaOverride = ', &
      kTemperVary,iaaOverrideDefault(2,1),iaaOverride(2,1)
...
!    CALL printstar
    ...
    CALL Check_iaaOverrideDefault
    kTemperVary = iaaOverrideDefault(2,1)
    write(kStdWarn,'(A,3(I3))') 'after OverrideNML_nmParam_nmSpectra+1, kTemperVary,iaaOverrideDefault,iaaOverride = ', &
      kTemperVary,iaaOverrideDefault(2,1),iaaOverride(2,1)

    CALL CheckParams
    write(kStdWarn,'(A,3(I3))') 'after OverrideNML_nmParam_nmSpectra+2, kTemperVary,iaaOverrideDefault,iaaOverride = ', &
      kTemperVary,iaaOverrideDefault(2,1),iaaOverride(2,1)
    CALL printstar
...
! ******** PARAMS section
    ...
    write(kStdWarn,'(A,3(I3))') &
      ' after end of check_params :  iTemperVary,kTemperVary,iaaOverrideDefault = ',iTemperVary,kTemperVary,iaaOverrideDefault(2,1)
...
!#ENDIF
    ...
    IF ((kRTP == 0) .OR. (kRTP == 1))  THEN
      ...
      ELSEIF (kWhichScatterCode .EQ. 5) THEN
        write(kStdWarn,'(A,I3,A,I3)') &
          'PCLSAM scattering model : ',kWhichScatterCode,&
          ' (+1/+2/+3/+4/+5 = Similarity,Chou,Maestri,Similarity+Tang,Chou+Tang) ',kScatter
        write(kStdErr,'(A,I3,A,I3)')  &
          'PCLSAM scattering model : ',kWhichScatterCode,&
          ' (+1/+2/+3/+4/+5 = Similarity,Chou,Maestri,Similarity+Tang,Chou+Tang) ',kScatter
      END IF
...
! ******** duplicate the atmospheres if needed section
    ...
    IF (iResetCldFracs < 0) THEN
      ...
      ELSEIF ((iNclouds > 0) .AND. (kWhichScatterCode == 5) .AND. (iCldProfile > 0) .AND. (iMRO == -1))  THEN
        ...
        write(kStdErr,'(A,2(5(F12.5),I4))') & 
          '  [ctop1 cbot1 cngwat1 cpsize1 cfrac1 ctype1    ctop2 cbot2 cngwat2 cpsize2 cfrac2 ctype2] = ', &
      ...
      ELSEIF ((iNclouds > 0) .AND. (kWhichScatterCode == 5) .AND. (iCldProfile < 0)) THEN
        ...
        ELSEIF ((cngwat2 <= 0) .AND. (cfrac2 <= 0) .AND. (iaCloudScatType(2) <= 0))  THEN
          ...
          write(kStdErr,'(A,5F12.5,I8,/A,5F12.5,I8,/A,F12.5)') &
            ' [ctop1 cbot1 cngwat1 cpsize1 cfrac1 ctype1] = ',ctop1,cbot1,cngwat1,cpsize1,cfrac1,ctype1, &
          ...
          write(kStdWarn,'(A,5F12.5,I8,/A,5F12.5,I8,/A,F12.5)') &
             ' [ctop1 cbot1 cngwat1 cpsize1 cfrac1 ctype1] = ',ctop1,cbot1,cngwat1,cpsize1,cfrac1,ctype1, &
             ' [ctop2 cbot2 cngwat2 cpsize2 cfrac2 ctype2] = ',ctop2,cbot2,cngwat2,cpsize2,cfrac2,ctype2,' cfrac12 = ',cfrac12
        ...
        ELSE
          write(kStdWarn,'(A,3(F12.5))') 'Something wrong with (PCLSAM) clouds, cfrac1,cfrac2,cfrac12 = ', &
            cfrac1,cfrac2,cfrac12
          write(kStdWarn,*) 'iNatm has remained ',iNatm
          write(kStdWarn,'(A,4(F12.4),I4)') 'ctop1,cbot1,cngwat1,cfrac1,iaCloudScatType(1) = ', & 
            ctop1,cbot1,cngwat1,cfrac1,iaCloudScatType(1)
          write(kStdWarn,'(A,4(F12.4),I4)') 'ctop2,cbot2,cngwat2,cfrac2,iaCloudScatType(2) = ', & 
            ctop2,cbot2,cngwat2,cfrac2,iaCloudScatType(2)

          write(kStdErr,'(A,3(F12.5))') 'Something wrong with (PCLSAM) clouds, cfrac1,cfrac2,cfrac12 = ', & 
            cfrac1,cfrac2,cfrac12
          write(kStdErr,*) 'iNatm has remained ',iNatm
          write(kStdErr,'(A,4(F12.4),I4)') 'ctop1,cbot1,cngwat1,cfrac1,iaCloudScatType(1) = ', &
            ctop1,cbot1,cngwat1,cfrac1,iaCloudScatType(1)
          write(kStdErr,'(A,4(F12.4),I4)') 'ctop2,cbot2,cngwat2,cfrac2,iaCloudScatType(2) = ', & 
            ctop2,cbot2,cngwat2,cfrac2,iaCloudScatType(2)
          CALL DoStop
        END IF
...
!!!kRTP = +1  : read RTP style   kLAYERS profile; set atm from RTP file
    ...
    ELSEIF (kRTP >= 0) THEN
      ...
      !write(kStdWarn,*) 'within this file, we will read profile number ',iRTP
...
!!!! only dump gases(1:7) for "var" gases, gas 22 (N2) as the broadener
    ...
    IF ((iLBLDIS > 0) .AND. (abs(kLongOrShort) <= 1)) THEN
      IF (iaaOverrideDefault(3,8) .EQ. -1) THEN
        ...
        write(kStdWarn,'(A,A)') &
           'ohoh iLBLDIS > 0 but we are using nonstandard rtp file (iaaOverrideDefault(3,8) .EQ. -1) ', & 
           'and raaAmt may later be modified in SetQ_NotInRTP'
        write(kStdErr,'(A,A)')  & 
           'ohoh iLBLDIS > 0 but we are using nonstandard rtp file (iaaOverrideDefault(3,8) .EQ. -1) ', & 
           'and raaAmt may later be modified in SetQ_NotInRTP'
        CALL DoStop 
