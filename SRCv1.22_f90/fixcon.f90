...
!*****************************************************************************80
! http://people.sc.fsu.edu/~jburkardt/f_src/fixcon/fixcon.html
! usage        fixcon file.f file.f90
! compilation  ifort fixcon.f90 -o fixcon
! modification (1) orig code truncated everything down to 72 characters, I commented that out
!              (2) if first character is 'C' or 'c' then replace with '!'
!
...
!
!  If it's talking about INTEGER etc then move it over...but change to F90 format.
!
!    if ( &
!      new_line(7:10) == 'REAL' .or. &
!      new_line(7:10) == 'real' .or. &      
!      new_line(7:13) == 'INTEGER' .or. &
!      new_line(7:13) == 'integer' .or. &      
!      new_line(7:15) == 'CHARACTER' .or. &
!      new_line(7:15) == 'character' .or. &
!      new_line(7:22) == 'DOUBLE PRECISION' .or. &
!      new_line(7:22) == 'double precision' .or. &
!      new_line(7:16) == 'SUBROUTINE' .or. &
!      new_line(7:16) == 'subroutine') then
!
!      if ( have_line ) then
...
!      new_line = new_line(7:len(new_line))
