...
! read the no of gases stored in the file
    ...
    ELSE
      write(kStdErr,*) 'input user iNgas = ',iNgas
...
! > 0   means you choose the gases (using iaLXsecNL)
    ...
    iErr = -1
    IF ((iNxsec > 0) .AND. (kPlanet == 03)) THEN
      write(kStdWarn,*) 'including user specified xsc gases for Earth Atmosphere'
    ...
    ELSEIF ((iNxsec == -1) .AND. (kPlanet == 03)) THEN
    ...
    ELSE
      ...
      write(kStdErr,*) '  valid iNXsec : -5/-6 (lblrtm) -1 (all) +N select few for Earth Atmosphere'
      CALL DoStop
    END IF
...
! local variables
    ...
    CHARACTER(160) :: caNONLTETempKC
    INTEGER :: iRegr,iMethod,iDefault
    REAL :: raTemp(kProfLayer)
    CHARACTER(160) :: caaaNLTEBandsOrig(kGasStore,kNumkCompT)
...
! simple routine to get US STD 2350 NLTE temps
    ...
    CALL concatCA160(caAuxNLTERefsPath,caFName)
...
! local vars
    ...
    CHARACTER(160) :: caaaNLTEBandsOrig(kGasStore,kNumkCompT),caX,caY
    CHARACTER(4) :: ca4
 ...
 40      FORMAT(I4)
         ...
         CALL ConcatCA160(caStrongLineParams,caY)
...
! local
    ...
    INTEGER :: iKLBLRTMgases,ikMarsGases,iMax_molgas
    ...
    IF (iWhichKC == -1) THEN
      IF (kPlanet == 03) THEN
        ...
        iMax = kGasComp
        write(kStdWarn,*) 'Earth Atmosphere : including all lbl gases from 1 to ', kGasComp
        write(kStdWarn,*) '                   plus 101,102,103 for Self,Forn cont, heavy water'
      ELSEIF (kPlanet == 04) THEN
        iMax = 6
        write(kStdWarn,*) 'Martian Atmosphere : including all lbl gases from 1 to ',ikMarsGases
        write(kStdWarn,*) '                     plus 101,102,103 for Self,Forn cont, heavy water'
      END IF
    ELSEIF ((iWhichKC == -5) .OR. (iWhichKC == -6)) THEN
      IF (kPlanet == 03) THEN
        ...
        iMax = iKLBLRTMgases
        write(kStdWarn,*) 'Erath Atmosphere : including LBLRTM gases from 1 to ',iMax
        write(kStdWarn,*) '                   plus 101,102,103 for Self,Forn cont, heavy water'
      ELSEIF (kPlanet == 04) THEN
        iMax = 6
        write(kStdWarn,*) 'Martian Atmosphere : including all lbl gases from 1 to ',ikMarsGases
...
! local
    ...
    IF ((iWhichXSC == -1) .AND. (kPlanet == 03)) THEN
      write(kStdWarn,*) 'including all Earth Atmosphere xsc gases from ',kGasXsecLo, ' to ', kGasXsecHi
    ...
    ELSEIF ((iWhichXSC > 0) .AND. (kPlanet == 03)) THEN
      DO iC = 1,iNXsec
        iaTemp(iC) = iaLXsecNL(iC)
      END DO

    ELSEIF ((iWhichXSC > 0) .AND. (kPlanet == 04)) THEN
      write(kStdWarn,*) 'xscgas4 : Mars Atmosphere does not need xsec gases'
      write(kStdErr,*) 'xscgas4 : Mars Atmosphere does not need xsec gases'
