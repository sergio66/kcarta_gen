...
%%%%%%%%% if ystart(iCnt) > 22000    %%% orig plan was to only do UV
    if ystart(iCnt) < 605          %%% but have decided to update NIR onwards, esp since there were many gaps
      iNeed = iNeed + 1;

      fprintf(1,'looks like we need this solar file %5i \n',ystart(iCnt))
      freqs = ystart(iCnt) : df : yend(iCnt)-df;
      whos freqs
      solarrads = ttorad(freqs,5800)*solidangle; figure(3); plot(freqs,solarrads,'o-'); 
