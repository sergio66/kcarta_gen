function for2mat(gid, vchunk, cdir, fdir, ppmvratio, dtype)
...
%   ppmvratio - amount to scale the coeffs by
...
%%% scale the abs coeffs
fprintf(1,'scaling the coeffs by %8.6f \n',ppmvratio);
kcomp = kcomp * ((ppmvratio)^0.25);
%%% scale the abs coeffs
...
outname = [cdir '/cg' num2str(gid) 'v' num2str(vchunk) '.mat'];
ee = exist(outname,'file');
if ee == 0
