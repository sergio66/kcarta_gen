function [data, wnums, caVersion] = readkcstd_smart(kfile, dfile)

% function [data, wnums] = readkcstd_smart(kfile, dfile)
% same as readkcstd, but tries to error out gracefully in kfile is not complete
%    in that case, data = wnums = []
%
%
...
for chunk = 1:nchunk
  ...
  for odb = 1:nODBs
    ...
    for ODBrow = 1:nODBrows(odb)
      ...
      if count ~= 10000
        errstr = ['fread failed, odb=',num2str(odb),' chunk=',num2str(chunk)];
	fprintf(1,' >>>>>>>>>>>>>>>> %s \n',errstr);
	wnums = [];
	data = [];
	return
