...
pah = rrtm_resultsx.f1
wah = rrtm_resultsx.f2
iaBands = input('Enter [Start Stop] bands : ');
woo = find(w >= iaBands(1)-0.0025/2 & w <= iaBands(2)+0.0025/2);
%woo = woo';
...
pah = pah(1:end-1);
aha = find(pah >= iaBands(1) & pah < iaBands(2));
if length(aha) > 1
  rrtm_up_flux = sum(squeeze(rrtm_results.heating_rate_info(1,aha,:,3)));
  rrtm_dn_flux = sum(squeeze(rrtm_results.heating_rate_info(1,aha,:,4)));
else
  rrtm_up_flux = (squeeze(rrtm_results.heating_rate_info(1,aha,:,3)));
  rrtm_dn_flux = (squeeze(rrtm_results.heating_rate_info(1,aha,:,4)));
end
...
sum_fUP  = sum(fUP,2)*0.0025/1000*pi;
...
sum_fDN  = sum(fDN,2)*0.0025/1000*pi;
...
rrtmy = rrtm_results.heating_rate_info;
rrtmy = squeeze(rrtmy(1,aha,:,6));
if length(aha) > 1
  rrtmy = sum(rrtmy,1);
end
...
plot(heat_kc,hgt,'bx-',...
    ...
    heat_ml,hgt,'ro-',...
    rrtmy,p2h(rrtm_results.plevs)/1000,'k',...
    'linewidth',2);
hl = legend('kCARTA flux->hr','raw kCARTA HR','matlab flux->hr','RRTM','location','southwest'); grid
title('heating rate'); xlabel('K/day'); ylabel('p(mb)')

quick_rrtm_surf_flux   = ttorad(iaBands(1):0.0025:iaBands(2),p.stemp)*pi/1000*0.0025; 
quick_kcarta_surf_flux = ttorad(iaBands(1):0.0025:iaBands(2),p.stemp)*pi/1000*0.0025;
