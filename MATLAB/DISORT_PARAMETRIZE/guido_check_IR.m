...
for ii = 1 :7
  fin = ['Prof1/GUIDO/IR/DISORT/ICE/rad.dat' num2str(ii) '_0605'];
  [disort_ice(:,ii),w] = readkcstd(fin);

  fin = ['Prof1/GUIDO/IR/DISORT/WATER/rad.dat' num2str(ii) '_0605'];
  [disort_water(:,ii),w] = readkcstd(fin);

  fin = ['Prof1/GUIDO/IR/CHOU/ICE/rad.dat' num2str(ii)];
  [junk,w] = readkcstd(fin);
  [mm,nn] = size(junk);
  chou_ice(:,ii) = junk(:,nn);

  fin = ['Prof1/GUIDO/IR/CHOU/WATER/rad.dat' num2str(ii)];
  [junk,w] = readkcstd(fin);
  [mm,nn] = size(junk);
  chou_water(:,ii) = junk(:,nn);

  fin = ['Prof1/GUIDO/IR/TANG/ICE/rad.dat' num2str(ii)];
  [junk,w] = readkcstd(fin);
  [mm,nn] = size(junk);
  tang_ice(:,ii) = junk(:,nn);

  fin = ['Prof1/GUIDO/IR/TANG/WATER/rad.dat' num2str(ii)];
  [junk,w] = readkcstd(fin);
  [mm,nn] = size(junk);
