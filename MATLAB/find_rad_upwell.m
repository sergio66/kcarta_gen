function r = find_rad_upwell(w,r0,od,mu,iLayNum,playstemp,plevstemp,iNlevs,iVers)

%% does rt through one layer, upwelling
%% input
%%   w = wavenumber
...
fprintf(1,'%3i %3i iLay* iLp1* %8.6f %8.6f  \n',iLayNum,iBeta,playstemp(iBeta),plevstemp(iBeta))
...
elseif iVers == 42
  ...
  if length(boo) > 0  
    ...
    rFcn(boo) = rAbs(boo).*raIntenP1(boo) + rZeta(boo).*(1-rAbs(boo)/2) - rTrans(boo)' .* rZeta(boo);
  end
  r = r0'.*rTrans + rFcn;
else
  error('unknown iVers')
