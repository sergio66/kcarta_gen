...
c caFluxFile gives the file name of the unformatted output
      SUBROUTINE find_fluxes(raWaves,raaAbs,raVTemp,
...
c        rMPTemp=raVT1(iL)
c        DO iFr=1,kMaxPts
c          rPlanck=exp(r2*raWaves(iFr)/rMPTemp)-1.0
c          rPlanck=r1*((raWaves(iFr)**3))/rPlanck
c          END DO
c        END DO
...
c then do the top of this layer
     ...
     $                          raTemp(iFr)*SNGL(daGaussWt(iAngle))
            END DO 
            print *,'upfl=',iLay+1,raaUpFlux(1,iLay+1),raaAbs(1,iL),rMPTemp
          END DO 
