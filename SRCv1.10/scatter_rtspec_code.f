...
c 1) all subroutines now call include '../INCLUDE/scatter110.param'
...
C     RADOBS : Output of subroutine, Upwelling radiation at observation height
      ...
      REAL    TEMP(KPROFLAYER), TAUGAS(KPROFLAYER-1)
...
C           Planck function radiances in units of W m^-2 sr^-1 cm
      PLANCK0 = r1 *WAVENO**3
     .            / (EXP(r2*WAVENO/TTOP) - 1)
      PLANCK1 = r1 *WAVENO**3
     .            / (EXP(r2*WAVENO/TBOT) - 1)
...
C               two stream problem.
      ...
      PLANCK1 = 0.5*r1 *WAVENO**3
     .            / (EXP(r2*WAVENO/TEMPS(1)) - 1)
      TAU = 0.0
      I = 2
      DO L = 1, NLAYER
        DELTAU = OPTDEPTHS(L)
...
C               Calculate thermal source terms
            ...
            PLANCK2 = 0.5*r1 *WAVENO**3
     .            / (EXP(r2*WAVENO/TEMPS(L+1)) - 1)
...
C     condition when integrating gas incident on bottom of cloud. 
      ...
      REAL    TEMP(KPROFLAYER), TAU(KPROFLAYER), COALB(KPROFLAYER)
...
C          Loop through layers, starting at bottom and going up
     ...
     .            / (EXP(r2*WAVENO/SFCTEMP) - 1)
      ...
      PLANCK1 = r1 *WAVENO**3
     .            / (EXP(r2*WAVENO/TEMP(NLEV)) - 1)
      DO I = NLEV-1, ICLDTOP+1, -1
        PLANCK0 = r1 *WAVENO**3
     .            / (EXP(r2*WAVENO/TEMP(I)) - 1)
        TAUO = TAU(I)/MU
...
C     are input.
      ...
      REAL    WAVENO, TEMP(KPROFLAYER), TAU(KPROFLAYER), MU, RAD
      INTEGER I
