...
c (max of kNumkComp blocks, from 605 to 2805)
      INTEGER iFileIDLo,iFileIDHi,iFileStartFrLo,iFileStartFrHi,iInt,iFileID
...
c %%%%%%%%%%%%% CLOUDY SKY  %%%%%% SIMPLE  %%%%%%%%%%%%%%%%%%%%%%%%%%%%
                ELSE IF (kWhichScatterCode .EQ. 1) THEN
                  write(kStdWarn,*) ' ---> 2STREAM SCATTERTING Computations...'
                  CALL doscatter_twostream(raFreq,raaSumAbCoeff,raMixVertTemp,
     ...
     $   iaCloudNumAtm,iaaCloudWhichAtm,iTag)
                  ...
                  IF (kFlux .GT. 0) THEN
                    write(kStdWarn,*) ' ---> 2STREAM Flux Computations ...'
