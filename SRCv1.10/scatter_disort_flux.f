...
c caFluxFile gives the file name of the unformatted output
      ...
      SUBROUTINE scatterfluxes_disort(raWaves,raaAbs,raVTemp,
...
c actually, this is technically done above)
     ...
     $         iaaRadLayer(iatm,1),iaaRadLayer(iatm,inumlayer)
      ...
      CALL flux_disort(raWaves,raVTemp,
...
c ------------ first see if the sky is clear; if so, call clear sky flux --
     ...
     $        iCloudySky, IACLDTOP, IACLDBOT) 
      ...
      IF (iCloudySky .LT. 0) THEN
        ...
        CALL find_fluxes(raWaves,raaAbs,raVTemp,caFluxFile,
     $              iOutNum,iAtm,iNumLayer,iaaRadLayer,
     $              rTSpace,rTSurf,raUseEmissivity,
     $              rSatAngle,rFracTop,rFracBot,
     $              iNpmix,iFileID,iNp,iaOp,raaOp,raaMix,
     $              raSurface,raSun,raThermal,raSunRefl,
...
c        END DO 
     ...
     $                         rFracTop,raWaves,raaAbs,raDown)  

      DO iFr = 1,kMaxPts,1000
        print *, raWaves(iFr),radtot(raWaves(iFr),raUp(iFr)),
     $                        radtot(raWaves(iFr),raDown(iFr))
        END DO
...
c -------------------------------------------------------------------------
      ...
      ELSEIF (iDownWard .EQ. -1) THEN
        DO iLay = 2,iNumLayer+1
          ...
          rMPTemp=raVT1(iL)  
          print *,'going up   ',iLay,iL,rMPTemp
          DO iFr=1,kMaxPts  
            rPlanck=exp(r2*raWaves(iFr)/rMPTemp)-1.0  
            rPlanck=r1*((raWaves(iFr)**3))/rPlanck  
            rAngleTrans=exp(-raaAbs(iFr,iL))  !use rCosAngle = 1.0 = nadir
          ...
          iL=iaRadLayer(iLay-1)  
          rMPTemp=raVT1(iL)  
          print *,'coming down',iLay,iL,rMPTemp
          DO iFr=1,kMaxPts  
            rPlanck=exp(r2*raWaves(iFr)/rMPTemp)-1.0  
            rPlanck=r1*((raWaves(iFr)**3))/rPlanck  
            rAngleTrans=exp(-raaAbs(iFr,iL))  !use rCosAngle = 1.0 = nadir
          ...
          DO iFr=1,kMaxPts,iStep
            print *,iStep,iLay,iFr,radtot(raWaves(iFr),raKCUp(iFr)),
     $                       radtot(raWaves(iFr),raKCDown(iFr))
            END DO  
...
c now net flux density at each level = upgoing flux - downgoing flux
      DO iLay = 1,iNumLayer+1
        DO iFr=1,kMaxPts  
          ...
          END DO  
        CALL InterpolateFlux(raaFlux,iLay,raDown,raWaves,iStep)       
        END DO
