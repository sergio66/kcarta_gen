...
c   scatter*.f    : scattering routines     -----------> use only ONE of them
c                                                        look for XXXX
...
ccccccc              raTTemp(iInt)=rDerivTemp+raTTemp(iInt)
ccccccc              raMixVertTemp(iInt)=rDerivTemp+raTTemp(iInt)
...
c see if current gas ID needs nonLTE spectroscopy
     ...
     $            iL_low,iL_high,raTAmt,raTTemp,raTPress,raTPartPress,
     $            iaCont,iTag,raFreq,daaDQ,daaDT,iDoDQ,
     $            daaLTEGasAbCoeff,iLTEIn,iWhichChunk,caaaNewChunks,
     $            kaFrStep(iTag),iFileStartFr*1.00000)
...
c %%%%%%%%%%%%% CLEAR SKY %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     ...
     $              raThickness,raPressLevels,iProfileLayers,pProf)
              ...
              ELSE IF (abs(kWhichScatterCode) .NE. 0) THEN
c %%%%%%%%%%%%% CLOUDY SKY  %%%%%% SSCCATT %%%%%%%%%%%%%%%%%%%%%%%%%%%%
                write(kStdWarn,*) ' ---> Scattering Computations ...'

                iDoFlux = -1
ccccc           CALL doscatter_XXXX(raFreq,
                CALL doscatter_twostream(raFreq,
     $              raaSumAbCoeff,raMixVertTemp,caOutName,
     $              iOutNum,iAtm,iaNumLayer(iAtm),iaaRadLayer,
     $              raTSpace(iAtm),raTSurf(iAtm),raUseEmissivity,
     $              raSatAngle(iAtm),raFracTop(iAtm),raFracBot(iAtm),
     ...
     $   iaCloudNumAtm,iaaCloudWhichAtm,iTag,raNumberDensity,iDoFlux)
                ...
                IF (kFlux .GT. 0) THEN
                  write(kStdWarn,*) ' ---> Flux Computations ...'
cccc                    CALL scatterfluxes_XXXX(raFreq,
                  CALL scatterfluxes_twostream(raFreq,
     $              raaSumAbCoeff,raMixVertTemp,caOutName,
     $              iOutNum,iAtm,iaNumLayer(iAtm),iaaRadLayer,
     $              raTSpace(iAtm),raTSurf(iAtm),raUseEmissivity,
     $              raSatAngle(iAtm),raFracTop(iAtm),raFracBot(iAtm),
     ...
     $   pProf,raThickness,iProfileLayers)
                  CALL PrintPound
                  END IF

                IF (kJacobian .GE. 0) THEN
                  write(kStdWarn,*) ' ---> Doing Simple Scattering Jacobians'
                  write(kStdWarn,*) ' --> ERROR : this feature is  off < --'
                  CALL DoStop
                  CALL find_jacobians_scat(raFreq,iFileID,caJacobFile,
     $                raTSpace(iAtm),raTSurf(iAtm),raUseEmissivity,
     $                raSatAngle(iAtm),raMixVertTemp,iNumGases,iaGases,
     $                iAtm,iNatm,iaNumLayer(iAtm),iaaRadLayer,
     $                raaaAllDQ,raaAllDT,raaSumAbCoeff,raaAmt,raInten,
