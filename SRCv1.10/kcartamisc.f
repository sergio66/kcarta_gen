...
c this is the number of args
      ...
      IF (iMicroSoft .GT. 0) THEN 
        ...
        iMicrosoft = iDummy    !tells number of output files (w/o flux)
       ...
       ELSE 
        ...
        iMicrosoft = iDummy-1    !tells number of output files (w/o flux)
        END IF 
...
c these are the reference profiles stored in matrices 
     ...
     $                          iaGases(iGas) 
          WRITE(kStdErr,*) 'Layer ',iI,' has negative amount' 
          CALL DoStop 
          END IF 
        ...
        IF (raRTemp(iI) .LT. 0.0) THEN 
          WRITE(kStdErr,*) 'Error in Ref Profile for Gas', 
     $                          iaGases(iGas) 
          WRITE(kStdErr,*) 'Layer ',iI,' has negative tempr' 
          CALL DoStop 
          END IF 
        ...
        IF (raRPress(iI) .LT. 0.0) THEN 
          WRITE(kStdErr,*) 'Error in Ref Profile for Gas', 
     $                          iaGases(iGas) 
          WRITE(kStdErr,*) 'Layer ',iI,' has negative pressure' 
          CALL DoStop 
          END IF 
...
c      print *,'for gas ID = ',iGasID,' have found posn = ',iAns
...
c              -1 : no continuum
c              00 : version 00 
c              01 : version 01    ... this is the MK_CKD version of Dec 2002
c              02 : version 02    ... this is the MK_CKD version of Dec 2002,
c                                     but CS modified by Scott Hannon Aug 2003
c              21 : version 2.1
c              23 : version 2.3
c              24 : version 2.4
c ----------------------------------------------------------------------------
c              12 : self = version 2.4, foreign = blend of 0.0 and 2.4
c                                       from 0-1575, 1625-3000, use v2.4
c                                       from 1575-1625 linearly blend v2.4,0.0
c                                       using a triangle
c              13 : self = version 2.3, foreign = dave tobin's v2.3
c              90 : self = version 2.4, foreign = blend of 2.4,Tobin's thesis
c                                       from 0-1300, 1900-3000, use v2.4
c                                       from 1300-1900 use Tobins thesis
c              50 : self, foreign       blend of 2.4, RAL data
c                                       from 0-1300, 1900-3000, use v2.4
c                                       from 1300-1900 use RAL data
c                                       mst50.m used linear tempr interpolation
c              51 : self, foreign       blend of 2.4, RAL data
c                                       from 0-1300, 1900-3000, use v2.4
c                                       from 1300-1900 use RAL data
...
c              56 : self, foreign       same as 51 above, but uses
...
c                                d) CKD 2.4 used upto 1400 cm-1
c              60 : self, foreign     is a hybrid of 51,55 done by scott
...
cc CKD    releases are 0,21,23,24
cc MT_CKD releases are 1
cc our modifications are 2 (derived from 1)
cc                       51,55,60 (derived from analysing RAL data)
cc and various 12,13,50,52,56 which might be gotten rid of eventually
c      IF ((kCKD .NE. -1) .AND. (kCKD .NE. 0) 
c     $    .AND. (kCKD .NE. 1) .AND. (kCKD .NE. 2) 
c     $    .AND. (kCKD .NE. 21) .AND. (kCKD .NE. 23) .AND. (kCKD .NE. 24) 
c     !!!!!!!!! these are the research versions
c     $    .AND. (kCKD .NE. 12) .AND. (kCKD .NE. 13)
c     $    .AND. (kCKD .NE. 50) .AND. (kCKD .NE. 51) .AND. (kCKD .NE. 52)
c     $    .AND. (kCKD .NE. 55) .AND. (kCKD .NE. 56) .AND. (kCKD .NE. 60)) THEN
c        write(kStdErr,*) 'In *PARAMS, need kCKD = [-1] for no continuum OR'
c        write(kStdErr,*) '                AER    versions 0,1,21,23 or 24'
c        write(kStdErr,*) '      (latest AER versions = 1, released Dec 2002)'
c        write(kStdErr,*) '      Also have research v2,12,13,50,51,52,55,56,60'
c        write(kStdErr,*) '      of which v2,51,55,60 are best'
c        write(kStdErr,*) '      (from a combination of RAL and AIRS data)'
c        write(kStdErr,*) 'kCKD is water continuum calculation version'
...
c and various 12,13,50,52,56 which might be gotten rid of eventually 
     ...
     $    .AND. (kCKD .NE. 55) .AND. (kCKD .NE. 56) .AND. (kCKD .NE. 60)) THEN 
        ...
        write(kStdErr,*) '              MT_CKD    versions 1,2,3,4,(5)' 
      ...
      IF (iabs(kLongOrShort) .GT. 1) THEN
        write(kStdErr,*) 'In *PARAMS, program needs kLongOrShort = -1,+1,0'
        write(kStdErr,*) 'kLongOrShort = complete header info (+1) or not (-1)'
...
c        END IF
      ...
      IF (abs(kRTP) .GT. 1) THEN
        write(kStdErr,*) 'Need to set RTP = -1,0, or +1'
...
c this subroutine computes avg pressure of the layers
      ...
      END
c************************************************************************
c this simply adds on two double matrices, storing results in daa
      SUBROUTINE addlte(daa,daaADD,rS)
    
      include '../INCLUDE/kcarta.param'

      DOUBLE PRECISION daa(kMaxPts,kProfLayer),daaADD(kMaxPts,kProfLayer)
      REAL rS

      INTEGER iFr,iL

      write(kStdWarn,*) 'Adding in the NONLTE contribution ....'

      DO iL = 1,kProfLayer
        DO iFr = 1,kMaxPts
          daa(iFr,iL) = daa(iFr,iL) + daaAdd(iFr,iL)*rS
          IF (daa(iFr,iL) .LT. 0.0) daa(iFr,iL) = 0.0d0
          END DO
        END DO


c      DO iL = 1,kProfLayer
c        print *,iL,daa(1,iL),daaAdd(1,iL),daa(1,iL)/daaAdd(1,iL)
c        END DO
