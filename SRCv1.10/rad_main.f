...
c caOutName gives the file name of the unformatted output
      SUBROUTINE find_radiances(raFreq,raaAbs,raVTemp,
...
c retrievals."
     ...
     $         iaaRadLayer(iatm,1),iaaRadLayer(iatm,inumlayer)
      ...
      iVary = -1             !!!temperature in layer constant
     ...
     $        raThickness,raPressLevels,iProfileLayers,pProf)
        ELSE
          CALL rad_trans_SAT_LOOK_DOWN_VARY(iVary,raFreq,
     $        raInten,raVTemp,
     $        raaAbs,rTSpace,rSurfaceTemp,raUseEmissivity,
     $        rSatAngle,rFracTop,rFracBot,
     $        iNp,iaOp,raaOp,iNpmix,iFileID,
...
c first do the bottommost layer (could be fractional)
      DO iLay=1,1
         iL=iaRadLayer(iLay)
         rCos=cos(raLayAngles(MP2Lay(iL))*kPi/180.0)
c         write (KstdWarn,*) iLay,raLayAngles(MP2Lay(iL)), 
c     $                      raLayAngles(MP2Lay(iL))*kPi/180.0,1/rCos 
         rMPTemp=raVT1(iL)
...
c indpt of surface emissivity, surface temperature
      ...
      SUBROUTINE rad_trans_SAT_LOOK_UP(raFreq,
...
c raaOp      = list of fractions used for output for current atmosphere
      REAL raFreq(kMaxPts),raVTemp(kMixFilRows),rSatAngle,rFracTop
...
c geometry factor
      ...
      IF (iDoSolar .EQ. 0) THEN
        !! need to compute ttorad(ff,5600)
        rSunTemp = 5600.0
...
c      IF (iDoSolar .GT. 0) THEN
c        IF (rSunTemp .GT. 0) THEN
c          DO iFr=1,kMaxPts
cc NOTE! no geometry factor (rOmegaSun=1.0), need cos(rSunAngle) eventually
cc compute the Plank radiation from the sun
c            rPlanck=exp(r2*raFreq(iFr)/rSunTemp)-1.0
c            raSun(iFr)=r1*((raFreq(iFr))**3)/rPlanck
c            END DO
c        ELSE
c          CALL ReadSolarData(raFreq,raSun,iTag)
c          END IF
c      ELSE
c        DO iFr=1,kMaxPts
c          raSun(iFr)=0.0
...
c        END IF
      ...
      IF (iDoSolar .EQ. 0) THEN 
        DO iFr=1,kMaxPts 
c NOTE! no geometry factor (rOmegaSun=1.0), need cos(rSunAngle) eventually 
...
c initialize the cumulative thermal radiation
        rPlanck=exp(r2*raFreq(iFr)/kTSpace)-1.0
        raThermal(iFr)=r1*((raFreq(iFr))**3)/rPlanck
        END DO
...
c      IF ((iDoSolar .GT. 0) .AND. (kJacobian .GT. 0)) THEN
cc do solar contribution at top of atmosphere
c        IF (rSunTemp .GT. 0) THEN
c          DO iFr=1,kMaxPts
cc NOTE! no geometry factor (rOmegaSun=1.0), need cos(rSunAngle) eventually
cc compute the Plank radiation from the sun
...
c        END IF
      ...
      IF ((iDoSolar .EQ. 0) .AND. (kJacobian .GT. 0)) THEN 
        DO iFr=1,kMaxPts 
c NOTE! no geometry factor (rOmegaSun=1.0), need cos(rSunAngle) eventually 
c compute the Plank radiation from the sun 
          rPlanck=exp(r2*raFreq(iFr)/rSunTemp)-1.0 
          raSun(iFr) = r1*((raFreq(iFr))**3)/rPlanck 
          END DO 
      ELSEIF ((iDoSolar .EQ. 1) .AND. (kJacobian .GT. 0)) THEN 
       CALL ReadSolarData(raFreq,raSun,iTag) 
      ELSE 
        DO iFr=1,kMaxPts 
          raSun(iFr)=0.0 
...
c            fractional weight here
      ...
      IF (iDir .LT. 0) THEN            !radiance going down to instr on gnd
        CALL UpLookInstrInterp(iDir,rFrac,raFreq,raVTemp,rCos,
     $    iLay,iaRadLayer,raaAbs,raInten,raInten2,raSun,iSun,
     $    iNumLayer,rFracTop,rFracBot,iProfileLayers,raPressLevels)
      ELSE                             !radiance going up to instr in space
        CALL DownLookInstrInterp(iDir,rFrac,raFreq,raVTemp,rCos,
     $    iLay,iaRadLayer,raaAbs,raInten,raInten2,raSun,iSun,
     $    iNumLayer,rFracTop,rFracBot,iProfileLayers,raPressLevels)
        END IF
...
c note : remember raaOp is the list of fractions with respect to FULL layers
c also note all temperature interpolations are done wrt ORIG temps raVTemp
      SUBROUTINE UpLookInstrInterp(iDir,rFrac,raFreq,raVTemp,rCos,
     $    iLay,iaRadLayer,raaAbs,raInten,raInten2,raSun,iSun,
     $    iNumLayer,rFracTop,rFracBot,iProfileLayers,raPressLevels)
...
c====================== presslev(i1)
      ...
      ELSE                           !interpolate
        ...
        DO iFr=1,kMaxPts
          rPlanck=exp(r2*raFreq(iFr)/rT)-1.0
          rPlanck=r1*((raFreq(iFr)**3))/rPlanck
...
c note : remember raaOp is the list of fractions with respect to FULL layers
c also note all temperature interpolations are done wrt ORIG temps raVTemp
      SUBROUTINE DownLookInstrInterp(iDir,rFrac,raFreq,raVTemp,rCos,
     $    iLay,iaRadLayer,raaAbs,raInten,raInten2,raSun,iSun,
     $    iNumLayer,rFracTop,rFracBot,iProfileLayers,raPressLevels)
...
c    for the solid angle subtended by the sun as seen from the earth
      ...
      SUBROUTINE rad_trans_SAT_LOOK_DOWN_VARY(iVary,raFreq,raInten,raVTemp,
     $    raaAbs,rTSpace,rTSurf,raUseEmissivity,rSatAngle,
     $    rFracTop,rFracBot,iNp,iaOp,raaOp,iNpmix,iFileID,
     $    caOutName,iOutNum,iAtm,iNumLayer,iaaRadLayer,raaMix,
     $    raSurface,raSun,raThermal,raSunRefl,raLayAngles,raSunAngles,iTag,
...
c                   user specified value if positive
      ...
      CHARACTER*80 caOutName
c these are to do with the arbitrary pressure layering
      REAL raThickNess(kProfLayer),pProf(kProfLayer),raPressLevels(kProfLayer+1)
      INTEGER iProfileLayers
...
c now do the radiative transfer thru this bottom layer
        IF (iVary .EQ. 1) THEN
          CALL RT_ProfileUP(raFreq,raaAbs,iL,TEMP,rCos,rFracBot,iVary,raInten)
        ELSE
         CALL RT_ProfileUP(raFreq,raaAbs,iL,ravt2,rCos,rFracBot,iVary,raInten)
          END IF
        END DO
c^^^^^^^^^^^^^^^^^^^^^^^^^VVVVVVVVVVVVVVVVVVVV^^^^^^^^^^^^^^^^^^^^^^^^
c then do the rest of the layers till the last but one(all will be full)
...
c now do the radiative transfer thru this complete layer
c        IF ((iL .GE. 45) .AND. (iL .LE. 48)) THEN
c          iVary = +1
c        ELSE 
c          iVary = -1
c          END IF
        IF (iVary .EQ. 1) THEN
          CALL RT_ProfileUP(raFreq,raaAbs,iL,TEMP,rCos,-1.0,iVary,raInten)
        ELSE
         CALL RT_ProfileUP(raFreq,raaAbs,iL,ravt2,rCos,-1.0,iVary,raInten)
          END IF
        END DO
