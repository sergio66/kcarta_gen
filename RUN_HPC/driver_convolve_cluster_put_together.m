...
foutsave = ['JUNK/xconvolved_kcarta_' strinstr '_put_together.mat'];
saver = ['save ' foutsave ' goodlist use_this_rtp fKc dKcall kcartaexec'];
myPrompt = ['save to ' foutsave ' ? (default -1/+1) : '];
iSave = input(myPrompt, 's');
if length(iSave) == 0
  iSave = -1;
end
if iSave > 0
...
addpath /home/sergio/git/matlabcode/DUSTFLAG
if iWhichInstr == 1
  g = dogoodchan;
else
  g = 1 : chinstr;
...
if ~isfield(p,'robs1')
  tkCarta = real(rad2bt(fKc,dKcall));
  i1231 = find(fKc >= 1231,1);
  [Y,I] = sort(p.stemp);
  figure(1); plot(p.stemp(I),tkCarta(i1231,I),'rx-',p.stemp,p.stemp,'k'); xlabel('SKT'); ylabel('KCARTA BT231');
  figure(2); plot(fKc,nanmean(tkCarta,2),fKc,nanstd(tkCarta,[],2)+nanmean(p.stemp));
    legend('mean','std + mean(SKT)','location','best');
    
elseif isfield(p,'robs1')
