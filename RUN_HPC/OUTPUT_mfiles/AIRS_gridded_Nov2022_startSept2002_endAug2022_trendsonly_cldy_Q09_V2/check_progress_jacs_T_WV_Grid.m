...
lser = ['!ls -lt AllDemJacsClr/ind*jac.mat | wc -l']; eval(lser)

for ii = 1 : 72
  for jj = 1 : 64
    istr = num2str(ii,'%02d');
    jstr = num2str(jj,'%02d');
    kstr = num2str((jj-1)*72+ii);
    thedir = dir(['AllDemJacsAnomaly/T_WV_Grid/' jstr '/' istr '/ind*jac.mat']);
    thedir = dir(['AllDemJacsClr/individual_prof_convolved_kcarta_airs_' kstr '_jac.mat']);
...
fprintf(1,'done %8i so far out of %8i or %8.5f percent \n',sum(sum(thecount)),25*64*72,sum(sum(thecount))/(64*72)*100)
