...
SUBROUTINE AddRayleigh(  &
    ...
    iProfileLayers,ICLDTOPKCARTA,ICLDBOTKCARTA, iaCldTop,iaCldbot,iNclouds,  &
    raMeanT,iSquish,iaMeanLay, raaExtTemp,raaScatTemp,raaAsymTemp)
...
NO TYPE, INTENT(OUT)                     :: iaCldTop
NO TYPE, INTENT(IN OUT)                  :: iaCldbot
INTEGER, INTENT(IN OUT)                  :: iNclouds
REAL, INTENT(IN OUT)                     :: raMeanT(kProfLayer)
INTEGER, INTENT(OUT)                     :: iSquish
INTEGER, INTENT(IN OUT)                  :: iaMeanLay(kProfLayer)
...
INTEGER :: iaaRadLayer(kMaxAtm,kProfLayer),iNumLayer
...
IF (iNclouds == 0) THEN
  ...
  CALL DoStop
ELSE IF (iNclouds == 1) THEN
  iNumCldLay = ICLDTOPKCARTA - ICLDBOTKCARTA + 1
ELSE IF (iNclouds == 2) THEN
  iNumCldLay = iaCldBotX(1)-iaCldTopX(1) + 1 + iaCldBotX(2)-iaCldTopX(2) + 1
ELSE IF (iNclouds > 2) THEN
...
!     $                 exp(-raaExtTemp(1,iL))
    ...
    ELSE IF (iaRadLayer(1) > iaRadLayer(2)) THEN
