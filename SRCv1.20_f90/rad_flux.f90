...
SUBROUTINE find_fluxes(raFreq,raaAbs,raVTemp,  &
    ...
    rFracTop,rFracBot, iNpmix,iFileID,iNp,iaOp,raaOp,raaMix,  &
    raSurface,raSun,raThermal,raSunRefl, raLayAngles,raSunAngles,rDelta,iTag,  &
...
!!!use expint3; accurate as it uses rad as function of cos(theta)
  ...
  ELSE
    ...
    CALL DOStop
  END IF
ELSE IF (iMuDMu_or_Moment == +2) THEN
...
!! constant T, loop over angles
    ...
    CALL flux_moment_slowloopConstT(raFreq,raVTemp,raaAbs,rTSpace,rSurfaceTemp,rSurfPress,  &
        ...
        raThickness,raPressLevels,raTPressLevels,iProfileLayers,pProf,  &
        caaScatter,raaScatterPressure,raScatterDME,raScatterIWP)
  ELSE IF (((iVary == +3) .OR. (iVary == 4) .OR. (iVary == 41) .OR. (iVary == 42) .OR. (iVary == 43))  &
...
IF ((iCloudLayerTop > 0) .AND. (iCloudLayerBot > 0)) THEN
  ...
  ELSE IF (iCloudLayerTop == iaRadLayer(iNumLayer)) THEN
...
REAL, INTENT(IN OUT)                     :: rSurfPress
NO TYPE, INTENT(IN OUT)                  :: raUseEmiss
NO TYPE, INTENT(IN OUT)                  :: raSUnRefl
REAL, INTENT(IN)                         :: rFracTop
REAL, INTENT(IN)                         :: rFracBot
INTEGER, INTENT(OUT)                     :: iNpmix
INTEGER, INTENT(IN OUT)                  :: iFileID
...
SUBROUTINE cumulativelayer_expint3(raFreq,raaAbs,raaRad,raVT1,raBC,  &
    iLay,iL,iNumLayer,iUD,iComputeAll, raaCumSum,raTemp,raY,troplayer)


NO TYPE, INTENT(IN OUT)                  :: raFreq
NO TYPE, INTENT(IN OUT)                  :: raaAbs
NO TYPE, INTENT(IN OUT)                  :: raaRad
NO TYPE, INTENT(IN OUT)                  :: raVT1
NO TYPE, INTENT(OUT)                     :: raBC
NO TYPE, INTENT(OUT)                     :: iLay
NO TYPE, INTENT(OUT)                     :: iL
NO TYPE, INTENT(IN OUT)                  :: iNumLayer
NO TYPE, INTENT(OUT)                     :: iUD
NO TYPE, INTENT(IN OUT)                  :: iComputeAl
NO TYPE, INTENT(IN OUT)                  :: raaCumSum
NO TYPE, INTENT(IN OUT)                  :: raTemp
NO TYPE, INTENT(OUT)                     :: raY
NO TYPE, INTENT(IN OUT)                  :: troplayer
IMPLICIT NONE
...
IF (iUD == +1) THEN
  CALL cumulativelayer_expint3_upwellflux( raFreq,raaAbs,raaRad,raVT1,raBC,  &
      iLay,iL,iNumLayer,iUD,iComputeAll, raaCumSum,raTemp,raY,troplayer)
ELSE IF (iUD == -1) THEN
  CALL cumulativelayer_expint3_downwellflux(  &
      raFreq,raaAbs,raaRad,raVT1,raBC, iLay,iL,iNumLayer,iUD,iComputeAll,  &
...
NO TYPE, INTENT(IN OUT)                  :: raFreq
NO TYPE, INTENT(IN)                      :: raaAbs
NO TYPE, INTENT(IN)                      :: raaRad
NO TYPE, INTENT(IN OUT)                  :: raVT1
NO TYPE, INTENT(OUT)                     :: raBC
NO TYPE, INTENT(OUT)                     :: iLay
NO TYPE, INTENT(OUT)                     :: iL
...
IF ((iComputeAll > 0) .OR. (iComputeAll < 0 .AND. iLay == 1) .OR.  &
...
!                   user specified value if positive
INTEGER :: iProfileLayers
REAL :: raThickness(kProfLayer),raPressLevels(kProfLayer+1),raTPressLevels(kP
...
CHARACTER (LEN=80), INTENT(IN OUT)       :: caFluxFile
INTEGER, INTENT(IN)                      :: iNumLayer
INTEGER, INTENT(IN OUT)                  :: troplayer
INTEGER, INTENT(IN OUT)                  :: iAtm
REAL, INTENT(IN OUT)                     :: raFreq(kMaxPts)
REAL, INTENT(IN OUT)                     :: rDelta
REAL, INTENT(OUT)                        :: raaUpFlux(kMaxPts,kProfLayer+1)
NO TYPE, INTENT(IN OUT)                  :: raaDownFlu
REAL, INTENT(IN OUT)                     :: raDensityX(kProfLayer)
REAL, INTENT(IN OUT)                     :: raDensity0(kProfLayer)
NO TYPE, INTENT(IN OUT)                  :: raThicknes
NO TYPE, INTENT(IN OUT)                  :: raDeltaPre
NO TYPE, INTENT(IN OUT)                  :: raPressLev
...
REAL :: raThickness(kProfLayer),raDeltaPressure(kProfLayer)
...
!!do flux at TOP (1) or BOTTOM (3) of  each layer
  CALL PrintFlux13(iIOUN,caFluxFile,iNumLayer,troplayer,iAtm,  &
      raFreq,rDelta,raaUpFlux,raaDownFlux,raDensityX)
ELSE IF (kFlux == 2) THEN  !!do HEAT RATE at each level, plus 0 at TOA for heat rate
...
ELSE IF (kFlux == 4) THEN  !!do a OLR computation only at TOA
!do the integral over z axis (2pi)
  ...
  END DO
ELSE IF (kFlux == 5) THEN  !!do OLR only at TOA and ILR at GND, and at trp
  CALL PrintFlux5(iIOUN,caFluxFile,iNumLayer,troplayer,iAtm,  &
      raFreq,rDelta,raaUpFlux,raaDownFlux,raDensityX)
ELSE IF (kFlux == 6) THEN  !!do    UPwell at top of each layer, DNwell at bottom of each layer
...
REAL, INTENT(IN OUT)                     :: raFreq(kMaxPts)
REAL, INTENT(IN OUT)                     :: rDelta
REAL, INTENT(IN)                         :: raaUpFlux(kMaxPts,kProfLayer+1)
NO TYPE, INTENT(IN OUT)                  :: raaDownFlu
REAL, INTENT(IN)                         :: raDensityX(kProfLayer)
NO TYPE, INTENT(IN OUT)                  :: raPressLev
INTEGER, INTENT(IN OUT)                  :: iaRadLayer(kProfLayer)
IMPLICIT NONE
...
NO TYPE, INTENT(IN OUT)                  :: raPressLev
NO TYPE, INTENT(IN OUT)                  :: raThicknes
REAL, INTENT(OUT)                        :: raDensityX(kProfLayer)
REAL, INTENT(OUT)                        :: raDensity0(kProfLayer)
NO TYPE, INTENT(IN OUT)                  :: raDeltaPre
REAL, INTENT(IN)                         :: rFracTop
REAL, INTENT(IN)                         :: rFracBot
IMPLICIT NONE
...
REAL :: raDeltaPressure(kProfLayer)
...
RETURN
END SUBROUTINE  Set_Flux_Derivative_Denominator
