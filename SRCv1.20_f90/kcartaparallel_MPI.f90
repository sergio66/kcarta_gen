...
CALL ReadNameListFile(iaGases,iNumGases,rFreqStart,rFreqEnd,  &
    ...
    raFracTop,raFracBot,raaPrBdry, raaMix,iNpmix,caaMixFileLines,iMixFileLines,  &
    iOutTypes,iaPrinter,iaGPMPAtm, iaNp,iaaOp,raaOp,raaUserPress,iNatm2,  &
    caDriverName,caComment,iError, iJacob,iaJacob,  &
...
!c iaTagIndex tells which Tag is associated with which file
...
! first set the cumulative d/dT matrix to zero, if we need Jacobians
  IF (kJacobian > 0. AND.  &
        ((kActualJacs == -1) .OR. (kActualJacs. EQ. 30) .OR.  &
...
! %%%%%%%%%%%%% CLOUDY SKY %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
          ...
          CALL InterfaceScattering(  &
              ...
              iUpper,raaUpperPlanckCoeff,raaUpperSumNLTEGasAbCoeff,  &
              raUpperPress,raUpperTemp,iDoUpperAtmNLTE,  &
              caJacobFile,caJacobFile2, raTPressLevels,iKnowTP,  &
