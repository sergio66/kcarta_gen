...
!************** that interface with S.Machado's "perturb" code **********
!************** Any additional routines are also included here **********
!************************************************************************
!************* THis is based on scatter_twostream_main.f ****************
!************************************************************************
...
SUBROUTINE doscatter_perturb(raFreq,raaAbs,raVTemp,  &
...
! this code uses asymmetry plus single scattering albedo plus SOLAR beam
    CALL interface_perturb(raFreq,raInten,raVTemp,  &
    ...
    RETURN
  END SUBROUTINE doscatter_perturb
  
!************************************************************************
! interface to "perturb" code
...
! NO temperature variations in a layer, which should be more
! more important in the lower wavenumbers (far infrared and sub mm)
! NO solar radiation, which would be important in near IR and vis
...
!         Radiative transfer variables:
  ...
  IF (kSolar < 0) THEN
    WRITE (kStdWarn,*) 'FIRST ORDER PERTURBATION THEORY'
    WRITE (kStdWarn,*) 'No layer temperature profile effects'
    
  ELSE IF (kSolar >= 0) THEN
    WRITE (kStdWarn,*) 'FIRST ORDER PERTURBATION THEORY cannot handle sun'
    CALL DoStop
...
!        DO iF = 1,iNumLayer
!          iL = iaaRadLayer(iAtm,iF)
!          print *,iF,iL,raaExtTemp(1,iL),raaScatTemp(1,iL),raaAsymTemp(1,iL)
!         END DO
    
    CALL find_radiances_perturb(raFreq, raaExtTemp,raaScatTemp,raaAsymTemp,  &
  ...
  RETURN
END SUBROUTINE interface_perturb
