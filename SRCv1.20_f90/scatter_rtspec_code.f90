...
SUBROUTINE COMPUTE_RADIATIVE_TRANSFER (RTMODEL, MUOBS, IOBS, WAVENO,  &
...
NSCATTAB, MUINC, NMUOBS, MUTAB, NDME, DMETAB, NWAVETAB, WAVETAB,  &
    ...
    RADOBS, ibdry, TOA_to_instr,iiDiv)  !3 new parameters by SSM
...
CHARACTER (LEN=1), INTENT(IN OUT)        :: RTMODEL
REAL, INTENT(IN OUT)                     :: MUOBS
INTEGER, INTENT(IN OUT)                  :: IOBS
REAL, INTENT(IN OUT)                     :: WAVENO
INTEGER, INTENT(IN)                      :: NLEV
REAL, INTENT(IN)                         :: TEMP(KPROFLAYER+1)
REAL, INTENT(IN)                         :: TAUGAS(KPROFLAYER)
REAL, INTENT(IN OUT)                     :: SFCTEMP
REAL, INTENT(IN OUT)                     :: SFCEMIS
INTEGER, INTENT(IN)                      :: NCLDLAY
...
IF (NLEV > MAXNZ) STOP 'COMPUTE_RADIATIVE_TRANSFER: MAXNZ exceeded'
...
! **************** orig version **************************
!ccccC          If clear sky only do GASRT1 and GASRT2
!cccc      IF (IWPTOT.EQ.0) THEN
!ccccC            Integrate radiance from surface to observer
!ccccC           First go from surface to cloud bottom
!cccc        CALL GASRT1 (ABS(MUOBS), WAVENO, SFCTEMP, SFCEMIS,
!cccc     .        NLEV, TEMP, TAUTOT, COALB, ICLDTOP, ICLDBOT,
!cccc     .                   NCLDLAY, RAD0UPOBS, RAD0DNOBS)
!cccc        IF(MUOBS.GT.0) THEN
!ccccC           if looking down then go from cloud top to observer
!cccc          RADOBS = RAD0UPOBS(1)
!cccc          CALL  GASRT2 (WAVENO, NLEV, TEMP, TAUGAS,
!cccc     .                ICLDTOP+1, IOBS, MUOBS, RADOBS)
!cccc        ELSEIF(MUOBS.LT.0) THEN
!ccccC           Then go from cloud bottom to observer
!cccc          RADOBS = RAD0DNOBS(NCLDLAY)
!cccc          CALL  GASRT2 (WAVENO, NLEV, TEMP, TAUGAS,
!cccc     .                ICLDBOT-1, IOBS, ABS(MUOBS), RADOBS)
!cccc        ENDIF
        
!     If clear sky only do GASRT1 and GASRT2
!ccccccc this was orig rtspec code       IF (IWPTOT.EQ.0) THEN
        IF ((IWP(1) <= 0)) THEN
...
!if looking down, do the nocloud downlook rad trans
            CALL GASRT1_nocloud (ABS(MUOBS), WAVENO, SFCTEMP, SFCEMIS,  &
                ...
                NCLDLAY, RAD0UPOBS, RAD0DNOBS, ibdry, TOA_to_instr, iiDiv)
...
!                for incident radiation on each layer
              CALL GASRT1 (MUINC(1), WAVENO, SFCTEMP, SFCEMIS,  &
                  NLEV, TEMP, TAUTOT, COALB, ICLDTOP, ICLDBOT, NCLDLAY,  &
...
!                Do Single-scattering, Eddington, or Hybrid for each layer
                  IF (RTMODEL == 'S') THEN
                    ...
                    CALL SSCATRT (MUOBS, TTOP, TBOT, WAVENO, MUINC,  &
                        ...
                        PHI1UP(L),PHI1DN(L),PHI2UP(L),PHI2DN(L), RADOBS)
                  ...
                  ELSE
                    ...
                    CALL SEDDSCATRT (MUOBS, TTOP, TBOT, WAVENO, MUINC,  &
                        RADBNDRYUP, RADBNDRYDN, FLUXUPSEDD, FLUXDNSEDD,  &
                        FLUXUP, FLUXDN, RADBOTTOM, TAUCG(L), SSALB(L), ASYM(L),  &
                        PHI1UP(L),PHI1DN(L),PHI2UP(L),PHI2DN(L), RADOBS)
                  END IF
...
!                Do Single-scattering, Eddington, or Hybrid for each layer
                      ...
                      ELSE
                        ...
                        CALL SEDDSCATRT (ABS(MUOBS), TBOT, TTOP, WAVENO, MUINC,  &
                            RADBNDRYDN, RADBNDRYUP, FLUXDNSEDD, FLUXUPSEDD,  &
                            FLUXDN, FLUXUP, RADTOP,  &
                            TAUCG(L), SSALB(L), ASYM(L),  &
                            PHI1UP(L),PHI1DN(L),PHI2UP(L),PHI2DN(L), RADOBS)
                      END IF
...
!************************************************************************
                  ...
                  SUBROUTINE SSCATRT (MUOBS, TTOP, TBOT, WAVENO, MUINC,  &
                      IBNDRYUP, IBNDRYDN, I0UPOBS, TAUTOT, SSALB, ASYM,  &
...
!    fluxes in units of W m^-2 cm.
                  ...
                  REAL, INTENT(IN)                         :: MUOBS
                  REAL, INTENT(IN OUT)                     :: TTOP
                  REAL, INTENT(IN OUT)                     :: TBOT
                  REAL, INTENT(IN OUT)                     :: WAVENO
                  REAL, INTENT(IN)                         :: MUINC(2)
                  REAL, INTENT(IN)                         :: IBNDRYUP(2)
                  REAL, INTENT(IN)                         :: IBNDRYDN(2)
                  REAL, INTENT(IN)                         :: I0UPOBS
                  REAL, INTENT(IN)                         :: TAUTOT
                  REAL, INTENT(IN OUT)                     :: SSALB
                  REAL, INTENT(IN OUT)                     :: ASYM
                  REAL, INTENT(IN OUT)                     :: PHI1UP
                  REAL, INTENT(IN OUT)                     :: PHI1DN
                  REAL, INTENT(IN)                         :: PHI2UP
                  REAL, INTENT(IN OUT)                     :: PHI2DN
                  REAL, INTENT(OUT)                        :: IUPOBS
                  IMPLICIT NONE
...
!************************************************************************
                    ...
                    SUBROUTINE EDDSCATRT (MUOBS, TTOP, TBOT, WAVENO,  &
                        FLUXUP, FLUXDN, I0UPOBS, TAUTOT, SSALB, ASYM, IUPOBS)
!     Calculates upwelling radiance from a single thermally emitting
...
!    fluxes in units of W m^-2 cm.
                    ...
                    REAL, INTENT(IN OUT)                     :: TBOT
                    REAL, INTENT(IN OUT)                     :: WAVENO
                    REAL, INTENT(IN)                         :: FLUXUP
                    REAL, INTENT(IN)                         :: FLUXDN
                    REAL, INTENT(IN)                         :: I0UPOBS
                    REAL, INTENT(IN)                         :: TAUTOT
...
!************************************************************************
                  ...
                  SUBROUTINE SEDDSCATRT (MUOBS, TTOP, TBOT, WAVENO,  &
                      MUINC, IBNDRYUP, IBNDRYDN, FLUXUPSEDD, FLUXDNSEDD,  &
                      FLUXUP, FLUXDN, I0UPOBS, TAUTOT, SSALB, ASYM,  &
                      PHI1UP, PHI1DN, PHI2UP, PHI2DN,  IUPOBS)
!     Calculates upwelling radiance from a single thermally emitting
...
!    fluxes in units of W m^-2 cm.
                  ...
                  REAL, INTENT(IN)                         :: IBNDRYUP(2)
                  REAL, INTENT(IN)                         :: IBNDRYDN(2)
                  REAL, INTENT(IN)                         :: FLUXUPSEDD
                  REAL, INTENT(IN)                         :: FLUXDNSEDD
                  REAL, INTENT(IN)                         :: FLUXUP
                  REAL, INTENT(IN)                         :: FLUXDN
                  REAL, INTENT(IN)                         :: I0UPOBS
                  REAL, INTENT(IN)                         :: TAUTOT
...
!************************************************************************
                    ...
                    SUBROUTINE EDDRTF (NLAYER, OPTDEPTHS, ALBEDOS,  &
                        ASYMMETRIES, TEMPS, WAVENO, FLUXTOP, FLUXBOT,  FLUXES)
...
!                                   L=1 is top, L=NUML+1 is bottom
                    ...
                    REAL, INTENT(IN)                         :: OPTDEPTHS(NLAYER)
                    REAL, INTENT(IN)                         :: ALBEDOS(NLAYER)
                    NO TYPE, INTENT(IN OUT)                  :: ASYMMETRIE
                    REAL, INTENT(IN)                         :: TEMPS(NLAYER+1)
                    REAL, INTENT(IN OUT)                     :: WAVENO
                    REAL, INTENT(IN)                         :: FLUXTOP
                    REAL, INTENT(IN)                         :: FLUXBOT
...
!               MAXLAY is maximum number of layers
                    ...
                    INTEGER, PARAMETER :: MAXLAY=200
                    INTEGER, PARAMETER :: MAXN=2*MAXLAY+1
                    INTEGER :: N, L, I
...
!       Hacked from Linpack DGTSL.
                          ...
                          INTEGER, INTENT(IN)                      :: N
                          REAL*8, INTENT(OUT)                      :: LOWER(*)
                          REAL*8, INTENT(IN OUT)                   :: DIAG(*)
                          REAL*8, INTENT(IN OUT)                   :: UPPER(*)
...
! modify this so that we also do stuff at arccos(3/5)
                                ...
                                REAL, INTENT(IN OUT)                     :: SFCTEMP
                                REAL, INTENT(IN)                         :: SFCEMIS
                                INTEGER, INTENT(IN)                      :: NLEV
                                REAL, INTENT(IN)                         :: TEMP(KPROFLAYER+1)
                                REAL, INTENT(IN)                         :: TAU(KPROFLAYER)
                                REAL, INTENT(IN)                         :: COALB(KPROFLAYER)
                                INTEGER, INTENT(IN OUT)                  :: ICLDTOP
                                INTEGER, INTENT(IN OUT)                  :: ICLDBOT
                                INTEGER, INTENT(IN)                      :: NCLDLAY
                                REAL, INTENT(OUT)                        :: RAD1UP(NCLDLAY)
                                REAL, INTENT(OUT)                        :: RAD0DN(NCLDLAY)
                                INTEGER, INTENT(IN)                      :: iiDiv
                                IMPLICIT NONE
...
!     condition when integrating gas incident on bottom of cloud.
                                      ...
                                      INTEGER, INTENT(IN)                      :: NCLDLAY
                                      REAL, INTENT(OUT)                        :: RAD1UP(NCLDLAY)
                                      REAL, INTENT(IN OUT)                     :: RAD0DN(NCLDLAY)
                                      INTEGER, INTENT(IN OUT)                  :: ibdry
                                      REAL, INTENT(IN OUT)                     :: TOA_to_ins
                                      INTEGER, INTENT(IN)                      :: iiDiv
