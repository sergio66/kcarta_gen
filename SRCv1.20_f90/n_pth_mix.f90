...
!        write(kStdWarn,*) 'wah',iI,raPressLevels(iI),abs(raThickness(iI)), &
...
!        write(kStdWarn,*) 'mah',iI,raPressLevels(iI)
!        write(kStdWarn,*) 'lah',iI,raPressLevels(iI),rJunkHgt
...
! to count toward the tally ...
    ...
    ! o that this gas is not "reread" in from ReadOtherGases
...
!          iNatm    -1   rWeight -1
    ...
    ! aStore will be irrelevant
    ! nfo will be in raStore2
    ...
    ELSE
        ...
        IF (iSpecial < 0) THEN            !all gases to have same weight
            ...
            END DO
        ELSE
        ! irst default all gas weights to rC
            DO iInt=1,kMaxGas
                raStore(iInt)=rC
            END DO
            CALL readlist(caaMixFileLines,1,iSpecial,iMixFileLines, &
            raStore2,iaStore)
        ! hen add on the special weights
            DO iLay=1,iSpecial
...
! s code is ready to read in next unread line
...
! iaStore = output list of integers that are read in
    ...
    IF (iType < 0) THEN
    ! ead in list of integer (discard) followed by iNum reals, using
    ! urrent line as the line to start reading stuff from
        iLines     = iLines
        iLinesStop = iLines-1
    ELSEIF (iType > 0) THEN
    ! ead in list of (integer,real) pairs,  using next line as the
    ! ine to start reading stuff from
        iLines     = iLines+1
        iLinesStop = iLines-1
    END IF
...
! increment iLinesStop by 1 each time this loop is called
    ...
    IF (iType < 0) THEN
    ! ead in list of integer followed by iNum reals
        read (caaTemp,*,err=5,end=10) iJ,(raStore(iI),iI=1,iNum)
    END IF

    IF (iType > 0) THEN
    ! ead in list of iNum (integer,real) pairs
    ...
    !!! find PATH
        ...
        !!! find HEIGHTS
            ...
            IF (iS == -1) THEN
                ...
                GOTO 5
            END IF
        ! c differs from GetMoreInfo becuz of this!      iI = iI + 7 + 8
            iI = iI + 8
            iJ = 130-iI
            iL = iI
            DO iK = 1,iJ
...
! thus the difference between the lists is what we need AFGL  profiles for
    ...
    DO iI = 1,iNumGases
        ...
        ! CALL get_us_std(iIDgas,raPX,raPPX,raTX,raQX)
            ...
            iFoundX,iGasIndex)
            IF (iFoundX > 0) THEN
            ! rite(kStdWarn,4321) iIDGas,iLay,rAmt,rT,rP,rPP
