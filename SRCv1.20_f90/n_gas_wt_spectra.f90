...
! read how many gases will have new spectroscopy
    ...
    DO iCount = 1,iNumNewGases
        IF ((iaNewGasID(iCount) < 1) .OR. &
        (iaNewGasID(iCount) > kMaxGas)) THEN
            ...
            CALL DoSTOP
        END IF
        IF ((iaNewData(iCount) < 1) .OR. &
        (iaNewData(iCount) > kNumkCompT)) THEN
...
! read how many gases will have new nonLTE spectroscopy
    ...
    DO iLTEIn = 1,iNumNLTEGases
        IF ((iaNLTEGasID(iLTEIn) < 1) .OR. &
        (iaNLTEGasID(iLTEIn) > kMaxGas)) THEN
