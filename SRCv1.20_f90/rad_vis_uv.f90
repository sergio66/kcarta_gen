...
! if the top layer is fractional, interpolate!!!!!!
    ...
    (raSunAngles(iaRadLayer(1)) >= 0)) THEN
        ...
        !!! TOA to previous layer
            DO iLay = 1,iNumLayer
            ! rint *,raFreq(1),iLay,raaSSA(1,iLay)
...
! et point 2 (at 0.3 um)
    raLX(2) = 0.30     !! see pg 99, PhD by Wang as well as paper by Morel
    raA(2) = 0.0382
    raAPH(2) = 0.05
    raBW(2) = 0.0226

! et one end point (at 0.42 um) <<< minimum in water absorbance <<<<<<<
    raLX(3) = 0.42
    raA(3) = 4.454e-3     !! see pg 99, PhD by Wang, as well as paper by Morel
    raAPH(3) = 0.05
    raBW(3) = 0.0056

! et one end point (at 4 um or 2500 cm-1) same as data at 1.60 um
    raLX(8) = 4.0
    raA(8) = raA(7)
    raAPH(8) = raAPH(7)
    raBW(8) = raBW(7)
...
!!! include other normalizations
    ...
    RETURN
    end SUBROUTINE compute_rayleigh_correction_downlook_works_butBUGGY
