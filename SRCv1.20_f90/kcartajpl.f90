...
CALL quick_jpl_set( IPROF, AIRSLAY, LAT, LON, ALT, SATANG, SUNANG,  &
    TEMP, FAMNT, WAMNT, OAMNT, CAMNT, MAMNT, SAMNT, HAMNT, NAMNT,  &
    TSURF, NEMIS, NRHO, PSURF, XEMIS, FEMIS, XRHO, FRHO, RAD, NCHAN, IVCHAN)

CALL airs_kcarta_forward ( IPROF, AIRSLAY, LAT, LON, ALT, SATANG, SUNANG,  &
    TEMP, FAMNT, WAMNT, OAMNT, CAMNT, MAMNT, SAMNT, HAMNT, NAMNT,  &
    TSURF, NEMIS, NRHO, PSURF, XEMIS, FEMIS, XRHO, FRHO, RAD, NCHAN, IVCHAN)
CALL EXIT(0)           !!!!happy exit!

END PROGRAM kcartajpl
...
INTEGER, INTENT(IN OUT)                  :: AIRSLAY
REAL, INTENT(IN OUT)                     :: LAT
REAL, INTENT(IN OUT)                     :: LON
REAL, INTENT(IN OUT)                     :: ALT(MAXLAY)
NO TYPE, INTENT(IN OUT)                  :: SATANG
REAL, INTENT(IN OUT)                     :: SUNANG
...
REAL, INTENT(IN OUT)                     :: TSURF
INTEGER, INTENT(IN OUT)                  :: NEMIS
INTEGER, INTENT(IN OUT)                  :: NRHO
REAL, INTENT(IN OUT)                     :: PSURF
REAL, INTENT(IN OUT)                     :: XEMIS(MXEMIS)
REAL, INTENT(IN OUT)                     :: FEMIS(MXEMIS)
REAL, INTENT(IN OUT)                     :: XRHO(MXEMIS)
REAL, INTENT(IN OUT)                     :: FRHO(MXEMIS)
REAL, INTENT(IN OUT)                     :: RAD(MXCHAN)
INTEGER, INTENT(IN OUT)                  :: NCHAN
INTEGER, INTENT(IN OUT)                  :: IVCHAN(MXCHAN)
IMPLICIT NONE
...
INTEGER, PARAMETER :: MAXLAY=kProfLayer
INTEGER, PARAMETER :: MXEMIS=kEmsRegions
INTEGER, PARAMETER :: MXCHAN=kMaxPts
...
REAL :: ! prof layer altitudes

REAL :: !! CO2 molecules/cm2
REAL :: !! WV
REAL :: !! O3
REAL :: !! CO
REAL :: !! CH4
...
INTEGER :: ! Channel numbers for used channels
! these are JPL interfaces
...
CALL ReadJPLListFile( IPROF, AIRSLAY,  &
    LAT, LON, ALT, SATANG, SUNANG,  &
    TEMP, FAMNT, WAMNT, OAMNT, CAMNT, MAMNT, SAMNT, HAMNT, NAMNT,  &
    TSURF, NEMIS, NRHO, PSURF, XEMIS, FEMIS, XRHO, FRHO, RAD, NCHAN, IVCHAN,  &
    ...
    raaMix,iNpmix,caaMixFileLines,iMixFileLines, iOutTypes,iaPrinter,iaGPMPAtm,  &
    iaNp,iaaOp,raaOp,raaUserPress,iNatm2, caDriverName,caComment,iError,  &
    iJacob,iaJacob, iaSetEms,raaaSetEmissivity,iaSetSolarRefl,raaaSetSolarRefl,  &
    ...
    iaNLTEGasID,iaNLTEChunks,iaaNLTEChunks, caaStrongLines,iaNLTEBands,  &
    ...
    iDoUpperAtmNLTE,caaUpperMixRatio,caPlanckUAfile,caOutUAFile, caOutName)
...
! uncompression
CALL GetFreq(rFreqStart,rFreqEnd, iFileIDLo,iFileIDHi,  &
    raBlock,raFiles,iaTagIndex,iaActualTag, raFileStep,iaList,iTotal)
...
!! set up the ref and current profiles
  CALL Set_Ref_Current_Profs( iJax,rDerivTemp,rDerivAmt,  &
  ...
  CALL xWeights(raTPartPress,raTTemp,pProfNLTE, iProfileLayers,iSplineType,  &
...
CALL PrepareOutput(caDriverName,caOutName,caJacobFile,caJacobFile2,  &
    ...
    iOutTypes,iaPrinter,iaGPMPAtm,iaNp,iaaOp,raaUserPress, iJacob,iaJacob,  &
...
! END PROGRAM
!************************************************************************
IF ((iaCloudNumLayers(1) == iaNumLayer(1)) .AND. (iCldProfile < 0)) THEN
...
! first set the cumulative d/dT matrix to zero, if we need Jacobians
  IF (kJacobian > 0. AND.  &
        ((kActualJacs == -1) .OR. (kActualJacs. EQ. 20) .OR.  &
...
! check to see if any of the printing options set iPrinter=1
  ...
  IF ((iFound < 0) .AND. (iOutNum <= iOutTypes)) THEN
    CALL SetUpCurrentPrint(iOutNum,iPrinter,iAtmPr,iNp,iaOp,1,  &
        iaPrinter,iaGPMPAtm,iaNp,iaaOp, iNumGases,iNpmix,iaNumLayer,-1)
...
! see if current gas ID needs nonLTE spectroscopy
      IF ((iChunk_DoNLTE == 1) .OR. (iChunk_DoNLTE == 3)) THEN
        CALL NLTEDriver(  &
            ...
            raUpperGasAmt,raUpperNLTETemp, iUpper,iDoUpperAtmNLTE,  &
            dLineStrenMin,dDeltaFreqNLTE, caaUpperMixRatio,iNumberUA_NLTEOut,  &
            ...
            daaPlanckCoeffBloat, daaUpperPlanckCoeff,  &
...
!! accumulate d/dT for ALL gases
      ...
      ELSE IF ((kActualJacs == -2) .OR. (kActualJacs == 32) .OR.  &
...
! elements of the iIpmix th row of raaMix
    IF (iDoAdd > 0) THEN
      ...
      END DO
      IF ((iLTEIn < 0) .AND. (iSetBloat > 0) .AND. (iChunk_DoNLTE == 1)) THEN
...
! have to be output
      IF (iDummy2 > 0) THEN
        CALL out_trans_path(raFreq,rFreqStart,rFreqEnd,  &
            raaTempAbCoeff,iPrinter, raTAmt,raTTemp,raTPress,raTPartPress,  &
      ...
      IF ((iChunk_DoNLTE == 1) .AND. (iSetBloat > 0)) THEN
        CALL out_bloat(raFreq,rFreqStart,rFreqEnd,+1,daFreqBloat,  &
            ...
            caPlanckBloatFile,caOutBloatFile, iFileID,iaPaths,iNp,iaOp)
      END IF
    END IF
...
! (need to check if any of the printing options set iPrinter=2).
  ...
  iOutNum=1
  41     CONTINUE
  IF ((iFound < 0) .AND. (iOutNum <= iOutTypes) .AND. (iNpmix > 0)) THEN
  ...
  IF (iFound > 0) THEN
    ...
    CALL DoOutputMixedPaths( iFound,iPrinter,caOutName,  &
  ...
  IF ((iSetBloat > 0) .AND. (iChunk_DoNLTE == 1)) THEN
    ...
    CALL SetPlanckCoeffBloat(iNLTEStart,iAtm,iaaRadLayer,  &
        ...
        raaSumAbCoeff,raaPlanckCoeff, raaRestOfLTEGases,raaCO2_LTE,  &
    ...
    IF (iPrinter == 2) THEN
      ...
      CALL out_bloat(raFreq,rFreqStart,rFreqEnd,+1,daFreqBloat,  &
          ...
          iPrinter,caPlanckBloatFile,caOutBloatFile, iFileID,iaPaths,iNp,iaOp)
    END IF
...
! atmosphere
      ...
      IF ((iFound < 0) .AND. (iOutNum <= iOutTypes) .AND. (iNpmix > 0)) THEN
        ...
        IF ((iPrinter == 3) .AND. ((iAtmPr == iAtm) .OR. (iAtmPr < 0))) THEN
          iFound=1
        ELSE
          iOutNum=iOutNum+1
        END IF
...
!!need to dump out 1's as eventually, we will be doing NLTE
        ...
        ELSE IF ((iChunk_DoNLTE == 1).AND.(iFunnyCousin == +1)) THEN
...
! %%%%%%%%%%%%% CLEAR SKY %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        ELSE IF ((kWhichScatterCode == 0) .AND. (iaLimb(iAtm) > 0)) THEN
! %%%%%%%%%%%%% CLEAR SKY %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
          WRITE(kStdWarn,*) ' ---> Clear Sky LIMB Computations ...'
          CALL InterfaceClearSkyLimb( raFreq,  &
              raaSumAbCoeff,raMixVertTemp,caOutName,  &
              iOutNum,iAtm,iaNumLayer,iaaRadLayer,  &
              raTSpace,raTSurf,rSurfPress,raUseEmissivity,  &
              raSatAngle,raFracTop,raFracBot,  &
...
SUBROUTINE ReadjplListFile( IPROF, AIRSLAY,  &
    LAT, LON, ALT, SATANG, SUNANG,  &
    TEMP, FAMNT, WAMNT, OAMNT, CAMNT, MAMNT, SAMNT, HAMNT, NAMNT,  &
    TSURF, NEMIS, NRHO, PSURF, XEMIS, FEMIS, XRHO, FRHO, RAD, NCHAN, IVCHAN,
...
raaAmt,raaTemp,raaPress,raaPartPress,raLayerHeight,iaCont,  &
    iProfileLayers,raPressLevels,raThickness,raTPressLevels,iKnowTP,
! atmosphere info  &
iNatm,raTSpace,raTSurf,raSatAngle,raSatHeight, iaNumLayer,iaaRadLayer,  &
    raFracTop,raFracBot,raaPrBdry,
! mixpath info  &
raaMix,iNpmix,caaMixFileLines,iMixFileLines,
! output info  &
iOutTypes,iaPrinter,iaGPMPAtm, iaNp,iaaOp,raaOp,raaUserPress,iNatm2,
! general stuff  &
...
raNLTEstrength,iNumNLTEGases,iNLTE_SlowORFast,iaNLTEGasID,  &
    iaNLTEChunks,iaaNLTEChunks, caaStrongLines,iaNLTEBands,  &
    iaNLTEStart,iaNLTEStart2350,caaaNLTEBands,caaNLTETemp,  &
    iAllLayersLTE,iUseWeakBackGnd,  &
    iSetBloat,caPlanckBloatFile,caOutBloatFile,caOutUABloatFile,  &
...
REAL, INTENT(IN)                         :: XEMIS(MXEMIS)
REAL, INTENT(IN)                         :: FEMIS(MXEMIS)
REAL, INTENT(IN)                         :: XRHO(MXEMIS)
REAL, INTENT(IN)                         :: FRHO(MXEMIS)
REAL, INTENT(IN OUT)                     :: RAD(MXCHAN)
INTEGER, INTENT(IN OUT)                  :: NCHAN
INTEGER, INTENT(IN OUT)                  :: IVCHAN(MXCHAN)
IMPLICIT NONE
...
REAL :: ! prof layer altitudes  ---> changed to thickness in quick_jpl_txt.m
...
CHARACTER (LEN=80) :: cakSolarRefl(kMaxAtm)
...
CHARACTER (LEN=120) :: caComment
CHARACTER (LEN=80) :: caLogFile
...
CHARACTER (LEN=80) :: caPlanckUAfile,caOutUAfile,caOutUABloatFile
...
ELSE IF ( (((cfrac1 > 0.001) .AND. (ctype1 <= 10)) .OR.  &
...
ELSE IF ((cfrac > 0.001) .AND. (cngwat > 0.001) .AND. (iNclouds_RTP > 0)) THEN
  WRITE (kStdWarn,*) 'successfully checked radnce .....'
  WRITE(kStdWarn,*) ' '
...
! trying to do fluxes for PCLSAM clouds as well
  ...
  ELSE IF ((caCloudPFname(1:5) == 'dummy') .AND. (k100layerCloud == +1)) THEN
...
!!!cloud stuff is defined in .nml file and not in the .rtp file
    ...
    ELSE IF ((ctype1 >= 200) .AND. (ctype1 <= 299)) THEN
      iaCloudScatType(1) = 202    !! so ctype = 201 (ice) maps to p.gas_202
    ELSE IF ((ctype1 >= 300) .AND. (ctype1 <= 399)) THEN
    ...
    ELSE IF ((ctype2 >= 200) .AND. (ctype2 <= 299)) THEN
      iaCloudScatType(2) = 202    !! so ctype2 = 201 (ice) maps to p.gas_202
    ELSE IF ((ctype2 >= 300) .AND. (ctype2 <= 399)) THEN
...
!!! p.gas_201 corresponds to 201 (ice) and p.gas_202 corresponds to 101 (water)
    ...
    CALL READRTP_CLD100LAYER(iRTP,iProfileLayers,  &
        ...
        iNclouds,iaCldTypes,raaKlayersCldAmt, ctype1,ctype2,cfrac1,cfrac2,  &
        caaScatter,raaScatterPressure,raScatterDME,raScatterIWP,  &
        raCemis,raCprtop,raCprbot,raCngwat,raCpsize,iaCtype,  &
        iScatBinaryFile,iNclouds,iaCloudNumLayers,caaCloudName,  &
        raaPCloudTop,raaPCloudBot,raaaCloudParams,raExp,iaPhase,  &
...
IF (iNumNewGases > 0) THEN
  ...
  CALL printstar
  iaKeyword(12) = 1
ELSE IF (iNumAltComprDirs > 0) THEN
  iNewLBL = 2
...
IF (iNumNLTEGases > 0) THEN
  iNewLBL = 1
  iNonlte = +1
  CALL nonlte( iRTP,iaGases,iNumGases,raaTemp,raaPress,  &
...
ELSE IF ((iNclouds > 0) .AND. (iSetRTPCld > 0)) THEN
...
!! go ahead and set up multiple cloud runs if doing PCLSAM (could also do this with eg DISORT)
  IF ((iNclouds > 0) .AND. (kWhichScatterCode == 5) .AND. (iCldProfile < 0)) THEN
    ...
    IF ((cngwat2 > 0) .AND. (cfrac2 > 0) .AND. (iaCloudScatType(2) > 0))  THEN
      ...
      raAtmLoop(4) = 1.0
      raAtmLoop(5) = 1.0
    ELSE IF ((cngwat2 <= 0) .AND. (cfrac2 <= 0) .AND. (iaCloudScatType(2) <= 0))  THEN
...
CALL output4(iaPrinter,iaGPMPAtm,iaNp, iaaOp,raaOp,raaUserPress,  &
    iaNumLayer,iaaRadLayer,iNatm,iNatm2, iOutTypes,iNumGases,iNpmix,  &
    raFracTop,raFracBot,raaPrBdry,raPressLevels, iaGases,caComment)
...
INTEGER :: iIOUN2, iaGasPathCounter(kProfLayer)
...
caStr160=' Lay    P(gA)      PP(gA)       Temp        GasID=       GasID=  &
    GasID=       GasID=       GasID=       GasID=       GasID= GasID='
...
SUBROUTINE quick_jpl_set ( IPROF, AIRSLAY, LAT, LON, ALT, SATANG, SUNANG,  &
    TEMP, FAMNT, WAMNT, OAMNT, CAMNT, MAMNT, SAMNT, HAMNT, NAMNT,  &
    TSURF, NEMIS, NRHO, PSURF, XEMIS, FEMIS, XRHO, FRHO, RAD, NCHAN, IVCHAN)


INTEGER, INTENT(OUT)                     :: IPROF
INTEGER, INTENT(OUT)                     :: AIRSLAY
REAL, INTENT(OUT)                        :: LAT
REAL, INTENT(OUT)                        :: LON
REAL, INTENT(IN OUT)                     :: ALT(MAXLAY)
NO TYPE, INTENT(IN OUT)                  :: SATANG
...
INTEGER, INTENT(OUT)                     :: IVCHAN(MXCHAN)
IMPLICIT NONE
