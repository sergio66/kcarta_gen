...
raNLTEstrength1,iNumNLTEGases1,iNLTE_SlowORFast1,  &
    ...
    iaNLTEBands1,raNLTEstart1,caaaNLTEBands1, caaNLTETemp1,caaUpperMixRatio1,  &
...
CHARACTER (LEN=80) :: caaTextOverride,caaTextOverride1
...
CHARACTER (LEN=80) :: caPFname,caPFName1,caCloudPFname,caCloudPFName1
...
CHARACTER (LEN=130) :: caaMixFileLines(kProfLayer),caaMixFileLines1(kProfLayer)
...
CHARACTER (LEN=80) :: caEmissivity(kMaxAtm),caEmissivity1(kMaxAtm)
CHARACTER (LEN=80) :: cakSolarRefl(kMaxAtm),cakSolarRefl1(kMaxAtm)
...
CHARACTER (LEN=80) :: caaScatter1(kMaxAtm),caaScatter(kMaxAtm)
...
CHARACTER (LEN=120) :: caComment,caComment1
...
CHARACTER (LEN=120) :: caaCloudFile(kMaxClouds),caaCloudFile1(kMaxClouds)
...
CHARACTER (LEN=120) :: caaaScatTable1(kMaxClouds,kCloudLayers)
CHARACTER (LEN=120) :: caaCloudName(kMaxClouds)
CHARACTER (LEN=120) :: caaCloudName1(kMaxClouds)
...
CHARACTER (LEN=80) :: caaaNewChunks1(kGasStore,kNumkCompT)
...
CHARACTER (LEN=80) :: caaaAltDirs(kGasStore,kNumkCompT)
CHARACTER (LEN=80) :: caaaAltDirs1(kGasStore,kNumkCompT)
...
CHARACTER (LEN=80) :: caaStrongLines1(kGasStore)
...
NAMELIST /nm_prfile/namecomment,caPFName,iRTP,iAFGLProf,  &
    iMPSetForRadRTP,ibinORasc, caaCloudFile,iNClouds_RTP,iaNML_Ctype
...
NAMELIST /nm_radnce/namecomment,  &
    ...
    raTSpace,raTSurf,raSatAngle,raSatHeight, caEmissivity,raSetEmissivity,  &
...
NAMELIST /nm_spectr/namecomment,iNumNewGases,iaNewGasID,iaNewData,  &
    iaaNewChunks,caaaNewChunks, iNumAltDirs,iaAltDirs,caaaAltDirs
...
CALL TranslateNameListFile(caDriverName, rf_low,rf_high,  &
    iNGas,iaGasesNL,iNXsec,iaLXsecNL,  &
    caPFName,caCloudPFName,iRTP,iNclouds_RTP,iAFGLProf, iNpmix,caaMixFileLines,  &
    ...
    raSatAzimuth,raSolAzimuth,raWindSpeed,  &
    caaScatter,raaScatterPressure,raScatterDME,raScatterIWP,  &
    iAtmLoop,raAtmLoop, iMPSetForRadRTP,iBinOrAsc,caaCloudFile,iaNML_Ctype,  &
    iJacob,iaJacob, caLogFile,caComment,iaPrinter,iaGPMPAtm,iaNp,iaaOp,raaOp,  &
...
ELSE IF ((kRTP > 0) .AND. (iaaOverrideDefault(1,8) == 1)) THEN
!no need to check freqs as this is done in kcartamain.f (GetFreq)
ELSE IF ((kRTP > 0) .AND. (iaaOverrideDefault(1,8) == -1)) THEN
...
IF (k100layerCloud == +1) THEN
  ...
  IF ((caJunk80(1:1) == '!') .OR. (caJunk80(1:1) == '%')) GO TO 1011
...
ELSE IF ((cfrac > 0.0) .AND. (iNclouds_RTP > 0) .AND. (kAllowScatter < 0)) THEN
  WRITE (kStdWarn,*) 'successfully checked radnce .....'
  WRITE(kStdWarn,*) ' '
...
!! go ahead and set up multiple cloud runs if doing PCLSAM (could also do this with eg DISORT)
  ...
  ELSE IF ((iNclouds > 0) .AND. (kWhichScatterCode == 5) .AND. (iCldProfile > 0) .AND. (iMRO == +1))  THEN
  ...
  ELSE IF ((iNclouds > 0) .AND. (kWhichScatterCode == 5) .AND. (iCldProfile < 0)) THEN
    iAtmLoop = 10
...
IF ((kJacobian > 0).AND.(iNewLBL == 1) .AND. (iNonLte < 0)) THEN
  ...
  CALL DOStop
ELSE IF ((kJacobian > 0).AND.(iNewLBL == 2) .AND. (iNonLte < 0)) THEN
...
NO TYPE, INTENT(IN OUT)                  :: iaAllowedG
NO TYPE, INTENT(IN OUT)                  :: iaWhichGas
INTEGER, INTENT(IN OUT)                  :: iaKeyWord(kNumWords)
...
CHARACTER (LEN=7) :: caKeyWord(kNumWords)
...
IF ((kRTP < 0) .AND. (kRTP >= -2)) THEN
  ...
  END IF
!ccc NOPE NO MORE iProfileLayers = kProfLayer !!!!!expect kProfLayer layers
ELSE IF (kRTP >= 0) THEN
  WRITE(kStdErr,*) 'this does NOT want RTP setup'
  CALL DoStop
