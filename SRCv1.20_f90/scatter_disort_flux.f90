...
raFreq,raVTemp, raaAbs,rTSpace,rSurfaceTemp,rSurfPress,raUseEmissivity,  &
...
!INTEGER J0, NBAND, IBAND, NWAVENO(MAXSPEC), NOUTNU
!LOGICAL TWOSIDEBAND ********* not using outtb(maxspec),outtriang(maxspec)
!      LOGICAL BINARYABSFILE
!      REAL    ZOBS, MUOBS(MAXSPEC), ZTOP, IWP(MAXNZ), DME(MAXNZ)
!      REAL    SFCTEMP, SFCEMIS
!REAL SFCEMIS1(MAXSPEC), SFCEMIS2(MAXSPEC)
!REAL RADOBS(2,MAXSPEC,MAXABSNU), OUTRAD(MAXABSNU) (real radobs,outrad,onu)
...
!REAL*8  BANDCENTER(MAXSPEC), BANDOFFSET(MAXSPEC)
...
!CHARACTER*24  OUTUNITS(MAXSPEC), OUTAVERAGE(MAXSPEC) (was outunits,outavging)
...
! to test no scattering, just replace following doloops with DO N = 1,-1
  ...
  IF (iRayleigh == -1) THEN     !want cloud, not Rayleigh scattering
    CALL SetUpClouds(nstr,nmuobs(1),iaCloudWithThisAtm,  &
        ...
        iAtm,iaaRadLayer,iNumLayer, IWP, DME, NDME, DMETAB, NWAVETAB, WAVETAB,  &
...
!     note we do need flux here!!!!
  ...
  CALL FinalInitialization(  &
      ...
      ibcnd,lamber,albedo, btemp,ttemp,temis,plank,onlyfl,accur,prnt,header)
