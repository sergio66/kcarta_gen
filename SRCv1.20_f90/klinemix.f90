...
CHARACTER (LEN=120), INTENT(OUT)         :: caFName1
CHARACTER (LEN=120), INTENT(OUT)         :: caFName2
DOUBLE PRECISION, INTENT(OUT)            :: dT1
DOUBLE PRECISION, INTENT(OUT)            :: dT2
DOUBLE PRECISION :: dJU      !local kinetic temp, upper state quantum
...
CHARACTER (LEN=30) :: caFName,caT1,caT2
CHARACTER (LEN=3) :: ca3
DOUBLE PRECISION :: dT
...
ELSE IF ((iISO == 2) .AND. (nint(SNGL(dJU)) == 9)) THEN
  caFName = 'band2351_linemix.dat'
ELSE IF ((iISO == 3) .AND. (nint(SNGL(dJU)) == 9)) THEN
  caFName = 'band2352_linemix.dat'
ELSE IF ((iISO == 1) .AND. (nint(SNGL(dJU)) == 16)) THEN
  caFName = 'band2320_linemix.dat'
ELSE IF ((iISO == 1) .AND. (nint(SNGL(dJU)) == 24)) THEN
...
DOUBLE PRECISION, INTENT(OUT)            :: daYmix(kHITRAN)
INTEGER, INTENT(OUT)                     :: iTooFar
...
! output parameters
DOUBLE PRECISION :: !line mix coeffs
...
CHARACTER (LEN=120) :: caFname1,caFName2
CHARACTER (LEN=30) :: caX
DOUBLE PRECISION :: dDiff0,dDiff1,dT1,dT2,dBlah
CHARACTER (LEN=1) :: caPorR_1(kHITRAN),caPorR_2(kHITRAN)
...
! open first file
iIOun = kTempUnit
OPEN(UNIT=iIOun,FILE=caFName1,FORM='formatted',STATUS='OLD', IOSTAT=iErr)
IF (iErr /= 0) THEN
...
IF ((iGasID == 2) .AND. (iISOTOPE == iISO) .AND. (jHigh == nint(dJU))) THEN
  dDiff0 = 1.0D0
ELSE
...
! open second file
iIOun = kTempUnit
OPEN(UNIT=iIOun,FILE=caFName2,FORM='formatted',STATUS='OLD', IOSTAT=iErr)
IF (iErr /= 0) THEN
  WRITE (kStdErr,*) 'in subroutine linemix, error reading'
...
INTEGER, INTENT(OUT)                     :: iLine
DOUBLE PRECISION, INTENT(IN OUT)         :: dJL
DOUBLE PRECISION, INTENT(OUT)            :: dJU
NO TYPE, INTENT(IN OUT)                  :: dJLowerQua
INTEGER, INTENT(IN)                      :: iISO
DOUBLE PRECISION, INTENT(IN OUT)         :: dLTE
DOUBLE PRECISION, INTENT(IN OUT)         :: dP
...
DOUBLE PRECISION :: daF1(kHITRAN),daF2(kHITRAN)
DOUBLE PRECISION :: daY1(kHITRAN),daY2(kHITRAN)
CHARACTER (LEN=120) :: caFnameALL
CHARACTER (LEN=30) :: caX
DOUBLE PRECISION :: dDiff0,dDiff1,dT1,dT2,dBlah
CHARACTER (LEN=1) :: caPorR_1(kHITRAN),caPorR_2(kHITRAN)
CHARACTER (LEN=80) :: caSkip80
...
iIOun = kTempUnit
OPEN(UNIT=iIOun,FILE=caFNameALL,FORM='formatted',STATUS='OLD', IOSTAT=iErr)
IF (iErr /= 0) THEN
  WRITE (kStdErr,*) 'in subroutine linemix, error reading'
...
DOUBLE PRECISION, INTENT(OUT)            :: z(kMaxPtsBox)
DOUBLE PRECISION, INTENT(IN OUT)         :: v(kMaxPtsBox)
DOUBLE PRECISION, INTENT(IN OUT)         :: v0
DOUBLE PRECISION, INTENT(IN OUT)         :: w_tot
DOUBLE PRECISION, INTENT(OUT)            :: T_in
DOUBLE PRECISION, INTENT(IN)             :: tau2
...
CHARACTER (LEN=80) :: caFName,caFName1
...
DOUBLE PRECISION, INTENT(OUT)            :: chiBirn(kMaxPts)
DOUBLE PRECISION, INTENT(OUT)            :: xBirn(kMaxPts)
INTEGER, INTENT(OUT)                     :: iNptsBirn
DOUBLE PRECISION, INTENT(IN OUT)         :: T_in
DOUBLE PRECISION, INTENT(IN)             :: tau2
IMPLICIT NONE
...
DOUBLE PRECISION, INTENT(IN OUT)         :: v0
INTEGER, INTENT(IN)                      :: n_in
DOUBLE PRECISION, INTENT(IN)             :: chiBirn(kMaxPts)
DOUBLE PRECISION, INTENT(IN OUT)         :: xBirn(kMaxPts)
INTEGER, INTENT(IN)                      :: iNptsBirn
IMPLICIT NONE
