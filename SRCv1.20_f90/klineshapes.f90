...
NO TYPE, INTENT(IN OUT)                  :: dPartition
...
! output params
DOUBLE PRECISION :: dPartitionFcn
...
DOUBLE PRECISION, INTENT(IN)             :: daW_For(kHITRAN)
DOUBLE PRECISION, INTENT(IN)             :: daW_Self(kHITRAN)
DOUBLE PRECISION, INTENT(IN)             :: daW_temp(kHITRAN)
DOUBLE PRECISION, INTENT(IN OUT)         :: dP
DOUBLE PRECISION, INTENT(IN OUT)         :: dPP
DOUBLE PRECISION, INTENT(IN OUT)         :: dLTE
DOUBLE PRECISION, INTENT(OUT)            :: daBroad(kHITRAN)
IMPLICIT NONE
...
SUBROUTINE Strengths_lte_only( iNum,iGasID,dGasAmt,daPartitionFcn,dLTE,  &
    daLineShift,daELower,daStren296, daStrenLTE,daQtipsFactor)


INTEGER, INTENT(IN)                      :: iNum
INTEGER, INTENT(IN OUT)                  :: iGasID
DOUBLE PRECISION, INTENT(IN)             :: dGasAmt
NO TYPE, INTENT(IN OUT)                  :: daPartitio
DOUBLE PRECISION, INTENT(IN)             :: dLTE
NO TYPE, INTENT(IN OUT)                  :: daLineShif
NO TYPE, INTENT(IN)                      :: daELower
DOUBLE PRECISION, INTENT(IN)             :: daStren296(kHITRAN)
DOUBLE PRECISION, INTENT(OUT)            :: daStrenLTE(kHITRAN)
NO TYPE, INTENT(IN OUT)                  :: daQtipsFac
IMPLICIT NONE
...
DOUBLE PRECISION :: daPartitionFcn(kHITRAN)
DOUBLE PRECISION :: daLineShift(kHITRAN),daElower(kHITRAN)
DOUBLE PRECISION :: !!straight from HITRAN at 296 K
! output params
...
SUBROUTINE Strengths_nlte_vibpartfcn(  &
    iL,iNum,iGasID,iISO,dGasAmt,dPartitionFcn, dLTE,dNLTE,dvibqft,  &
...
NO TYPE, INTENT(IN OUT)                  :: dPartition
DOUBLE PRECISION, INTENT(IN)             :: dLTE
DOUBLE PRECISION, INTENT(IN OUT)         :: dNLTE
DOUBLE PRECISION, INTENT(IN OUT)         :: dvibqft
NO TYPE, INTENT(IN OUT)                  :: daLineShif
NO TYPE, INTENT(IN)                      :: daELower
DOUBLE PRECISION, INTENT(IN)             :: daStren296(kHITRAN)
NO TYPE, INTENT(IN OUT)                  :: daStrenNLT
NO TYPE, INTENT(IN OUT)                  :: daQtipsFac
IMPLICIT NONE
...
SUBROUTINE NLTEPopulationRatios( iL,iNum,dP,dLTE,dLTEr1r2,dNLTE,dVibCenter,  &
...
DOUBLE PRECISION, INTENT(IN OUT)         :: dP
DOUBLE PRECISION, INTENT(IN OUT)         :: dLTE
DOUBLE PRECISION, INTENT(IN)             :: dLTEr1r2
DOUBLE PRECISION, INTENT(IN)             :: dNLTE
DOUBLE PRECISION, INTENT(IN)             :: dVibCenter
DOUBLE PRECISION, INTENT(IN)             :: daElower(kHITRAN)
NO TYPE, INTENT(IN OUT)                  :: daLineShif
INTEGER, INTENT(OUT)                     :: iaJ_UorL(kHITRAN)
DOUBLE PRECISION, INTENT(OUT)            :: daRTop(kHITRAN)
DOUBLE PRECISION, INTENT(OUT)            :: daRBot(kHITRAN)
DOUBLE PRECISION, INTENT(OUT)            :: daCFactor(kHITRAN)
DOUBLE PRECISION, INTENT(OUT)            :: daKFactor(kHITRAN)
IMPLICIT NONE
...
!! we matched the iUSGQ
    ...
    iTdat = +1
  ELSE IF (iaJ_UorL(iLines) == -1) THEN
...
! !! genln4 stuff
  ...
  IF (DABS(dTK-dNLTE) > 1.0D-3) THEN
    ...
    daKFactor(iLines) = (daRBot(iLines)-daRTop(iLines)*daDelta(iLines))*dDiv
  ELSE
...
!        END IF
  ...
  IF (iTalk > 0) THEN
    WRITE(*,2000) iLines,daRTop(iLines),daRBot(iLines),daDelta(iLines),  &
        1.0D0-daCFactor(iLines)/daKFactor(iLines), daKFactor(iLines)
...
INTEGER, INTENT(IN OUT)                  :: iLines
NO TYPE, INTENT(IN OUT)                  :: iUpdateSUM
NO TYPE, INTENT(IN OUT)                  :: rNLTEstren
DOUBLE PRECISION, INTENT(IN)             :: daRTop(kHITRAN)
DOUBLE PRECISION, INTENT(IN OUT)         :: daRBot(kHITRAN)
DOUBLE PRECISION, INTENT(IN OUT)         :: daCFactor(kHITRAN)
DOUBLE PRECISION, INTENT(IN)             :: daKFactor(kHITRAN)
...
SUBROUTINE read_std_optdepths_new(iGasID,raFreq,iStart,  &
    ...
    raVertTemp,iVertTempSet,rFileStartFr,iTag,iActualTag, iUnCompressType,  &
...
NO TYPE, INTENT(IN OUT)                  :: iUnCompres
...
SUBROUTINE read_std_optdepths_upper_UA(  &
    iGas,iSplineType,raUpperPressLevels, pProfNLTE_upatm,iNumProfNLTE_upatm,  &
    rFileStartFr,iTag,iActualTag, iGasID,raFreq,  &
    raTAmt,raTTemp,raTPress,raTPartPress, iNumUpperLayers,daaWeakOptDepth)
...
NO TYPE, INTENT(IN OUT)                  :: iNumProfNL
NO TYPE, INTENT(IN OUT)                  :: rFileStart
INTEGER, INTENT(IN OUT)                  :: iTag
INTEGER, INTENT(IN OUT)                  :: iActualTag
...
NO TYPE, INTENT(IN OUT)                  :: raTPartPre
NO TYPE, INTENT(IN OUT)                  :: iNumUpperL
NO TYPE, INTENT(IN OUT)                  :: daaWeakOpt
IMPLICIT NONE
...
REAL :: raTPartPress(kProfLayer), rFileStartFr
...
CALL upper_co2_default_profile(iGas,iGasID,pProf,iNumUpperLayers,  &
    raUpperPressLevels,iSplineType, raRAmt,raRTemp,raRPress,raRPartPress)
...
CALL read_std_optdepths_new(iGasID,raFreq,iStart,  &
    ...
    raRAmt,raRTemp,raRPress,raRPartPress,iL_low,iL_high,  &
    iNumUpperLayers,iSplineType,pProf, raVertTemp,iVertTempSet,rFileStartFr,  &
...
! input
REAL :: !!! this is the layers profile
...
INTEGER, INTENT(IN)                      :: iLTEin
NO TYPE, INTENT(IN OUT)                  :: dLineStren
CHARACTER (LEN=80), INTENT(IN OUT)       :: caStrong
INTEGER, INTENT(OUT)                     :: iNum
NO TYPE, INTENT(OUT)                     :: daIso
DOUBLE PRECISION, INTENT(OUT)            :: daElower(kHITRAN)
NO TYPE, INTENT(IN OUT)                  :: daLineCent
DOUBLE PRECISION, INTENT(OUT)            :: daJL(kHITRAN)
DOUBLE PRECISION, INTENT(OUT)            :: daJU(kHITRAN)
DOUBLE PRECISION, INTENT(OUT)            :: daPshift(kHITRAN)
DOUBLE PRECISION, INTENT(OUT)            :: daStren296(kHITRAN)
NO TYPE, INTENT(IN OUT)                  :: daW_For
DOUBLE PRECISION, INTENT(OUT)            :: daW_self(kHITRAN)
DOUBLE PRECISION, INTENT(OUT)            :: daW_temp(kHITRAN)
IMPLICIT NONE
...
CHARACTER (LEN=80) :: caFname,caTemp
CHARACTER (LEN=80) :: caaCheckNamesIn(kGasStore)
CHARACTER (LEN=80) :: caaCheckNamesMatch(kGasStore)
CHARACTER (LEN=80) :: caaCheckNamesUse(kGasStore)
...
DO iI = 1,iCumLineSum
  ...
  IF (LOG10(daSTren296Sum(iI)) > -20.0) THEN
    iaBin(1) = iaBin(1) + 1
  ELSE IF (LOG10(daSTren296Sum(iI)) > -21.0) THEN
    iaBin(2) = iaBin(2) + 1
  ELSE IF (LOG10(daSTren296Sum(iI)) > -22.0) THEN
    iaBin(3) = iaBin(3) + 1
  ELSE IF (LOG10(daSTren296Sum(iI)) > -23.0) THEN
    iaBin(4) = iaBin(4) + 1
  ELSE IF (LOG10(daSTren296Sum(iI)) > -24.0) THEN
    iaBin(5) = iaBin(5) + 1
  ELSE IF (LOG10(daSTren296Sum(iI)) > -25.0) THEN
    iaBin(6) = iaBin(6) + 1
  ELSE IF (LOG10(daSTren296Sum(iI)) > -26.0) THEN
    iaBin(7) = iaBin(7) + 1
  ELSE IF (LOG10(daSTren296Sum(iI)) > -27.0) THEN
    iaBin(8) = iaBin(8) + 1
  ELSE IF (LOG10(daSTren296Sum(iI)) > -28.0) THEN
    iaBin(9) = iaBin(9) + 1
  ELSE
    iaBin(10) = iaBin(10) + 1
  END IF
...
!!!if no need to prune, set all lines as output
  ...
  ELSE IF (daStren296Sum(iI) < dLineStrenMin) THEN
...
DOUBLE PRECISION, INTENT(IN OUT)         :: daPshift(kHITRAN)
...
CHARACTER (LEN=1), INTENT(IN OUT)        :: caJPQR(kHITRAN)
...
INTEGER :: iLTEIn, iDOVoigtChi
CHARACTER (LEN=80) :: caaaNLTEBands(kGasStore,kNumkCompT)
! output params
INTEGER :: iLineMixBand
DOUBLE PRECISION :: daLineCenter(kHITRAN)
...
ELSE IF ((iLineMixBand == 2) .AND. (iJU == 9)) THEN
  IF (iISO <= 2) THEN
    WRITE(kStdWarn,*) 'iJL, iJU,iSO = ',iJL,iJU,iISO
...
INTEGER, INTENT(IN OUT)                  :: iISO
NO TYPE, INTENT(IN OUT)                  :: dPartition
DOUBLE PRECISION, INTENT(OUT)            :: dMass
IMPLICIT NONE
...
NO TYPE, INTENT(IN)                      :: IGAS
NO TYPE, INTENT(IN OUT)                  :: ISO
NO TYPE, INTENT(IN)                      :: TP
NO TYPE, INTENT(OUT)                     :: SQR
NO TYPE, INTENT(OUT)                     :: SQRSTD
NO TYPE, INTENT(OUT)                     :: SQV
NO TYPE, INTENT(OUT)                     :: SQVSTD
IMPLICIT NONE
...
!        read(*,*) iso
  IF(Iso /= 1 .AND. Iso /= 2 .AND. Iso /= 3 .AND. Iso /= 4 ) THEN
...
!        read(*,*) iso
  IF(Iso /= 1 .AND. Iso /= 2 .AND. Iso /= 3 .AND. Iso /= 4  &
        .AND. Iso /= 5 .AND. Iso /= 6 .AND. Iso /= 7 .AND. Iso /= 8) THEN
    WRITE(*,*) '  Please enter isotope in list'
    STOP 'QNLTE'
!          go to 6
  END IF
...
SUBROUTINE QrH2O( iso,       ! isotope code (HITRAN notation)
!            ! 1 = 161
!            ! 2 = 181
!            ! 3 = 171
!            ! 4 = 162  &
...
INTEGER, INTENT(IN OUT)                  :: iso
, INTENT(IN OUT)                         :: ! isotope
...
DATA xgj/ 1.,1.,6.,6./
!...        --   161
DATA (Qcoef( 1,j),j=1,4)/-.53023D+01, .28404D+00, .12564D-02,-.56017D-06/
!...        --   181
DATA (Qcoef( 2,j),j=1,4)/-.53509D+01, .28640D+00, .12671D-02,-.56496D-06/
!...        --   171
DATA (Qcoef( 3,j),j=1,4)/-.31968D+02, .17117D+01, .75725D-02,-.33761D-05/
!...        --   162
DATA (Qcoef( 4,j),j=1,4)/-.27586D+02, .13989D+01, .62715D-02,-.27996D-05/
...
Qr = Qcoef(iso,1) + Qcoef(iso,2)*T  &
    + Qcoef(iso,3)*T*T + Qcoef(iso,4)*T*T*T
...
SUBROUTINE QrCO2( iso,       ! isotope code (HITRAN notation)
!            ! 1 = 626
!            ! 2 = 636
!            ! 3 = 628
!            ! 4 = 627
...
!...        --   626
DATA (Qcoef( 1,j),j=1,4)/ .16706D+00, .89058D+00, .99971D-05, .23271D-11/
!...        --   636
DATA (Qcoef( 2,j),j=1,4)/ .33413D+00, .17811D+01, .19992D-04, .68786D-11/
!...        --   628
DATA (Qcoef( 3,j),j=1,4)/ .33417D+00, .18877D+01, .21196D-04, .11156D-12/
!...        --   627
DATA (Qcoef( 4,j),j=1,4)/ .20037D+01, .11014D+02, .12437D-03, .98216D-10/
!...        --   638
DATA (Qcoef( 5,j),j=1,4)/ .66833D+00, .37755D+01, .42368D-04, .24009D-10/
!...        --   637
DATA (Qcoef(6,j),j=1,4)/ .40097D+01, .22029D+02, .24703D-03, .58218D-10/
!...        --   828
DATA (Qcoef(7,j),j=1,4)/ .16713D+00, .10020D+01, .11252D-04,-.72034D-12/
!...        --   728
DATA (Qcoef(8,j),j=1,4)/ .20048D+01, .11683D+02, .13095D-03, .58038D-10/
...
SUBROUTINE QrO3( iso,       ! isotope code (HITRAN notation)
!            ! 1 = 666
!            ! 2 = 668
!            ! 3 = 686
!            ! 4 = 667
...
DATA xgj/ 1.,1.,1.,6.,6./
!...        --   666
DATA (Qcoef(1,j),j=1,4)/-.11668D+03, .53395D+01, .24438D-01,-.11317D-04/
!...        --   668
DATA (Qcoef(2,j),j=1,4)/-.24923D+03, .11401D+02, .52186D-01,-.24168D-04/
!...        --   686
DATA (Qcoef(3,j),j=1,4)/-.12178D+03, .55723D+01, .25504D-01,-.11811D-04/
!...        --   667
DATA (Qcoef(4,j),j=1,4)/-.14489D+04, .66289D+02, .30342D+00,-.14052D-03/
!...        --   676
DATA (Qcoef(5,j),j=1,4)/-.71583D+03, .32756D+02, .14992D+00,-.69429D-04/
...
SUBROUTINE QrCO( iso,       ! isotope code (HITRAN notation)
!            ! 1 = 26
!            ! 2 = 36
!            ! 3 = 28
!            ! 4 = 27
...
!...        --    26
DATA (Qcoef(1,j),j=1,4)/ .33715D+00, .35986D+00, .28521D-05,-.80804D-10/
!...        --    36
DATA (Qcoef(2,j),j=1,4)/ .67401D+00, .75291D+00, .59556D-05,-.15617D-09/
!...        --    28
DATA (Qcoef(3,j),j=1,4)/ .33701D+00, .37790D+00, .29903D-05,-.80827D-10/
!...        --    27
DATA (Qcoef(4,j),j=1,4)/ .20224D+01, .22153D+01, .17551D-04,-.50468D-09/
!...        --    38
DATA (Qcoef(5,j),j=1,4)/ .67363D+00, .79248D+00, .62561D-05,-.15419D-09/
!...        --    37
DATA (Qcoef(6,j),j=1,4)/ .40433D+01, .46404D+01, .36125D-04,-.10135D-08/
...
SUBROUTINE QrNO2( iso,       ! isotope code (HITRAN notation)
!            ! 1 = 646  &
T,       ! temperature in K  &
    gj,       ! state independent nuclear degeneracy factor  &
    Qr)      ! rotational partition function at T
...
!...        --   646
DATA (Qcoef(j),j=1,4)/-.46394D+03, .21206D+02, .97410D-01,-.44482D-04/
!     NO2
gj = 3.
q296 =  .131941D+05
...
Qr = Qcoef(1) + Qcoef(2)*T  &
    + Qcoef(3)*T*T + Qcoef(4)*T*T*T
...
SUBROUTINE QvH2O( iso,       ! isotope code (HITRAN notation)
!            ! 1 = 161
!            ! 2 = 181
!            ! 3 = 171
!            ! 4 = 162  &
T,       ! temperature in K Qv)      ! rotational partition function at T
...
!...        --   161
DATA (Qcoef( 1,j),j=1,4)/ .99824D+00, .31601D-04, -.17994D-06, .33068D-09/
!...        --   181
DATA (Qcoef( 2,j),j=1,4)/ .99823D+00, .31881D-04, -.18197D-06, .33539D-09/
!...        --   171
DATA (Qcoef( 3,j),j=1,4)/ .99823D+00, .31751D-04, -.18101D-06, .33316D-09/
!...        --   162
!...        --   162
DATA (Qcoef( 4,j),j=1,4)/ .99803D+00, .38536D-04, -.24214D-06, .49662D-09/
...
SUBROUTINE QvCO2( iso,       ! isotope code (HITRAN notation)
!            ! 1 = 626
!            ! 2 = 636
!            ! 3 = 628
!            ! 4 = 627
...
!...        --   626
DATA (Qcoef( 1,j),j=1,4)/ .10428D+01,-.69155D-03, .27905D-05, .80679D-10/
!...        --   636
DATA (Qcoef( 2,j),j=1,4)/ .10464D+01,-.75718D-03, .31239D-05,-.12129D-09/
!...        --   628
DATA (Qcoef( 3,j),j=1,4)/ .10437D+01,-.70716D-03, .28643D-05, .48899D-10/
!...        --   627
DATA (Qcoef( 4,j),j=1,4)/ .10432D+01,-.69871D-03, .28282D-05, .67812D-10/
!...        --   638
DATA (Qcoef( 5,j),j=1,4)/ .10471D+01,-.77104D-03, .31995D-05,-.14088D-09/
!...        --   637
DATA (Qcoef( 6,j),j=1,4)/ .10467D+01,-.76445D-03, .31635D-05,-.13131D-09/
!...        --   828
DATA (Qcoef( 7,j),j=1,4)/ .10445D+01,-.72141D-03, .29381D-05, .24622D-10/
!...        --   728
DATA (Qcoef( 8,j),j=1,4)/ .10445D+01,-.71842D-03, .29025D-05, .58059D-10/
...
SUBROUTINE QvO3( iso,       ! isotope code (HITRAN notation)
!            ! 1 = 666
!            ! 2 = 668
!            ! 3 = 686
!            ! 4 = 667
...
!...        --   666
DATA (Qcoef( 1,j),j=1,4)/ .10220D+01,-.30911D-03, .87191D-06, .15073D-08/
!...        --   668
DATA (Qcoef( 2,j),j=1,4)/ .10242D+01,-.34657D-03, .10436D-05, .14088D-08/
!...        --   686
DATA (Qcoef( 3,j),j=1,4)/ .10243D+01,-.34434D-03, .10054D-05, .14929D-08/
!...        --   667
DATA (Qcoef( 4,j),j=1,4)/ .10220D+01,-.30911D-03, .87191D-06, .15073D-08/
!...        --   676
DATA (Qcoef( 5,j),j=1,4)/ .10220D+01,-.30911D-03, .87191D-06, .15073D-08/
...
SUBROUTINE QvCO( iso,       ! isotope code (HITRAN notation)
!            ! 1 = 26
!            ! 2 = 36
!            ! 3 = 28
!            ! 4 = 27
...
!...        --    26
DATA (Qcoef( 1,j),j=1,4)/ .99939D+00, .10070D-04, -.50995D-07, .81541D-10/
!...        --    36
DATA (Qcoef( 2,j),j=1,4)/ .99931D+00, .11310D-04, -.57586D-07, .92720D-10/
!...        --    28
DATA (Qcoef( 3,j),j=1,4)/ .99931D+00, .11433D-04, -.58223D-07, .93783D-10/
!...        --    27
DATA (Qcoef( 4,j),j=1,4)/ .99935D+00, .10761D-04, -.54661D-07, .87746D-10/
!...        --    38
DATA (Qcoef( 5,j),j=1,4)/ .99923D+00, .12847D-04, -.65811D-07, .10682D-9/
!...        --    37
DATA (Qcoef( 6,j),j=1,4)/ .99934D+00, .10926D-04, -.55541D-07, .89245D-10/
...
SUBROUTINE QvNO2( iso,       ! isotope code (HITRAN notation)
!            ! 1 = 646  &
T,       ! temperature in K Qv)      ! rotational partition function at T
...
!...        --   646
DATA (Qcoef(j),j=1,4)/ .10145D+01,-.20470D-03, .59546D-06, .88374D-09/
