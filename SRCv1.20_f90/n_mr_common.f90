...
!      caPFname = '/home/sergio/KCARTA/INCLUDE/glatm_16Aug2010.dat'
    ...
    IF (iNumLevsx > kProfLayer+1) THEN
        write(kStdErr,*) 'oops iNumLevsx > kProfLayer+1 ',iNumLevsx,kProfLayer+1
        CALL DoStop
    END IF
...
!! do a union of iaG and iaPlanetMolecules
    ...
    DO iG = 1,iNumGases
        ...
        ! test    CALL getAFGL(-1,iaG(iG),raPX,raPPX,raTX,raQX) ! this is what "AddOnAFGLProfile_arblevels" uses
        ! test        DO iL = 1,kMaxProfLayer
        ! test          raMMX(iL) = raPPX(iL)/raPX(iL)
        ! test        END DO
...
! also need to figure out the gas multiplier offset
    ...
    ! aT(iNumLevs) = raR100Temp(iL) + rToffset
        ...
        DO iG = 1,iNumGases
        ! aaG_VMR(iNumLevs,iG) = raaR100MR(iL,iG) * raOffset(iG)
            IF ((iL-iAbove) > iMerge) THEN
...
!      write(kStdWarn,*) 'h.pmax = ',rPmax/100.0,';      %% rPmax in N/m2 -->  mb'
    ...
    ca8 = 'p.plevs'
! ALL write_stringnice(ca8,raP,0.01,8,iNumLevs,-1)       !! raP in N/m2, convert to mb for rtp
