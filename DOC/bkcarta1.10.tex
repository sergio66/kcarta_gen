...
{\bf ABSTRACT}
\end{center}
\kc is a radiative transfer code for a non-scattering Earth's atmosphere.  It 
can be used to output monochromatic gas optical depths, layer-to-space 
transmittances and radiances.  The basic version can also do clear sky 
radiative transfer computations, as well as compute and  output analytic 
Jacobians with respect to temperature and gas amount.  
...
In addition to the main \kc source code, we provide many packages with the
code. One is $klayers$, which allows an user to input a radiosonde or 
retrieved point profile, and output a layer averaged profile that \kc can use.
We also provide the Fortran and Matlab source code that enables on to read
the output from \kc.

To maintain compatibilty with the AIRS products, \kc allows the user to 
completely or partially interface with the RTP file format, which can contain
the layer averaged profile and associated atmosphere definitions (and convolved
radiances). 
...
supplied in this \kc distribution, that changes a user supplied point profile
to a kCARTA layers averaged profile. The default behaviour is to use to 
...
developed, we are adding on the RTP file format. These files can contain the
AIRS profile and the atmosphere definitions (such as surface pressure and
...
structures. Currently Absoft and SGI do, but g77 does not}

\textcolor{red}
{The \kc Makefile produces three different executables. In this manual, we only
provide a description of the basic \kc package.
\begin{itemize}
\item $kcarta.x$ contains all the features of kCARTA (optical depths, clear 
sky radiative transfer, clear sky jacobians and fluxes, $DISORT$ and $RTSPEC$ 
...
very little memory.
\item $skcarta.x$ is the intermediate version, containing the most basic 
features of kCARTA (optical depths and clear sky radiative transfer), as well
as one of the scattering codes (the user has to tailor the $Makefile$ and the
file $kcartascat.f$ for this. 
\end{itemize}
}
...
\kc is comparatively easy to use. Running the program will mean that klayers.x
has to be run first, as this produces a set of ``reference profiles'' for \kc 
to use. \kc then has to be compiled, with the right array sizes being set.
Compile time options, which are set in file {\sf kcarta.param}, 
define paths to compressed data files and array sizes. In particular, this 
file sets the number of layers that \kc expects from the user profile. 
...
these combinations, radiance Jacobians can be computed and output.  In
...
version number. This will
contain the entire source code distribution (which includes the radiative
transfer code \kc, the point profile to layer average code %klayers$ and some
Matlab and Fortan readers). There are also some additional data files, and the
documentation. This is about 80Mb\\
...
\item \textcolor{blue} {klayers} Go to KCARTA/KLAYERS/Src\_1.10. Edit the 
...
To check that this executable file works, type 
\textcolor{red} {klayersDRY.sc USStandardProf\_NEW}
If everything is OK, a file named ``fort.30'' should appear in the subdir, of
about 900000 bytes in size. In addition, a number of files should appear in the
\\ KCARTA/SRCv.109/NewRefProfiles directory : about 6 param files and about 38
refgas files, containing the reference profiles for the gases (according to 
the HITRAN naming conventions).
...
Go to KCARTA/SRCv.109. Edit the ``Makefile'' 
so that the 
...
are set for the data files (ensure that the kCompPath and kWaterPath strings
point correctly to the database files, while kCKDPat,kSolarPath and kXsecFile
point to the correct be vs le files). Type ``make''. If everything compiles 
ok, file \textcolor{red}{bkcarta.x} will appear in the KCARTA/BIN directory
\begin{itemize}
\item $bkcarta.x$ contains only the most basic features of kCARTA 
(optical depths and clear sky radiative transfer). This version uses 
very little memory.
...
averaged profile $basic.prof$ in the KCARTA/WORK subdirectory. After that,
$kcarta.x$ runs, reading in namelist file $basic.nml$ (see below) and profile
$basic.prof$. The output of $kcarta.x$ is stored in a large binary file,
...
\begin{verbatim}
 ...
 !set this to v2.1
 kCKD      = 21
 !set kRTP = -1 so we use all oldstyle stuff
 kRTP = -1
 $end
...
$nm\_jacobn$ & optional section in \kc driver namelist file.  Once the user 
              ...
              scattering models, and then compute jacobians for a clear sky. 
...
$nm\_scattr$ & optional section in \kc driver namelist file.  Once the user 
               ...
               $disort.f$ or $twostream.f$ . If scattering is 
               turned on, then jacobians cannot be computed.\\ \hline
...
atmosphere is 
defined between layers 3 and 65, then the flux differences (at 0.0025 \cm 
resolution) between the top and bottom of each of layers 3-65 is output. In 
other words, the code does not sum over all the radiances across the infrared 
spectrum, but just outputs the monochromatic ``flux''.

If the user supplies a name $filename$ for the usual radiance 
results to be output, then the flux results are in $filename\_FLUX$; otherwise
they will stored in flux.dat. Note that if kFlux=1,2 and if a radiance for an
atmosphere is to be output, then the fluxes will be computed. This is 
independent of whether or not Jacobians are to be computed. 

If kFlux=1, the 
fluxes are simply an integral of radiance over $2 \pi \mu d\mu$, which means 
the units will be radiance units $\times$ steradians. If kFlux=2, the fluxes 
are an integral of 
radiance over $2\pi \mu d\mu$, and then divided by $-c_{p} \;\; \rho \delta z$ 
where $c_{p}$ is the specific heat at constant pressure, $\rho$ is the density
and $\delta z$ is the layer thickness. This means the units will be 
$Kelvin$ per $second$ per $cm^{-1}$. So to get the same type of plots that are
in Liou, ``An Introduction to Atmospheric Radiation'', pg 108, the user has to
first read in the computed ``fluxes''; then for each layer, sum across the
monochromatic ``fluxes'', multiply the result by 0.0025 $cm^{-1}$ (as this
is the same as integrating $I(\nu) d\nu$ to get the correct flux units, and 
then multiply the results by 86400 (seconds/day). A sample flux plot is shown
below, in Figure ~\ref{sampleflux}
...
The reader is referred to the documentation in the separate KLAYERS package.
The default layering is to use the 100 AIRS layers, but the savvy user
can change this.

\subsection{Compiling kcarta}
Once the user has successfully run klayers.x as described above, he/she should
now go to the kCARTA/SRC directory. A quick check of the subdirectory 
``NewRefProfiles'' should be done, to ensure that the recomputed reference 
profiles exist there. In addition, the first three lines of 
``outincLAY.param'' should be looked at, as they tell the user how 
klayers.x subdivided the original 100 AIRS layers, to produce MYNLAY layers.

The user now has to edit kcarta.param, and set kProfLayer to MYNLAY. Having 
...
\item {\sf calcon.f} : compute the water continuum (CKD v0 and v2.1)
...
\item {\sf kcoeffMAIN.f} : set up calls to compute absorption coefficients
\item {\sf kcoeffSPL.f}  : compute absorption coefficients
\item {\sf kcoeffSPLJAC.f}  : compute absorption coefficients and do Jacobians

\item {\sf n\_main.f} : main input file parsing routines 
\item {\sf n\_gas\_spectra.f} : MOLGAS,XSCGAS,SPECTRA parsing routines 
...
database and required files. A new requirement is that klayers.x must have 
already been run, to have produced reference profiles that have the requisite
number of layers, as well as some other files that contains information such
as where the sublayering begins, the pressure levels of the new layering
and so on.
...
gauss.param& contains Gauss-Legendre wieghts for angular integrations\\ \hline
airslevels.param& contains the AIRS pressure levels info\\ \hline
airsheights.param& contains the AIRS layer heights info\\ \hline
scatter.param& contains interface array dimensions kCARTA to rtrspec.f\\ 
\hline
\end{longtable}
\end{small}
...
In subdirectory NewRefProfiles, the following files should be present
before compiling kcarta : 
\begin{verbatim}
outlayers.param
outfilemisc.param     refgas21              refgas56
outheights.param      refgas22              refgas57
outincLAY.param       refgas23              refgas58
outpresslevels.param  refgas25              refgas59
refgas1               refgas26              refgas6
refgas10              refgas27              refgas60
refgas11              refgas3               refgas61
refgas12              refgas4               refgas62
refgas15              refgas5               refgas63
refgas16              refgas51              refgas7
refgas18              refgas52              refgas8
refgas19              refgas53              refgas9
refgas2               refgas54              summary.param
refgas20              refgas55
\end{verbatim}
refgas(n) contains the recomputed reference profiles, outincLAY.param
reproduces parameters M1000mb,M100mb,mSubLayer and MYNLAY (total number of 
layers), as well as the average pressures of the original AIRS 100 and the 
new (MYNLAY) layers. File outheights.param has the height structure of the 
levels that were computed. File outpresslevels.param contains the (MYNLAY+1) 
pressure levels. File outfilemisc.param contains information that is not
used by kcarta (layer thickness in meters, temperature in Kelvin and 
pressure in atm). File summary.param is another file that kcarta does not use,
but does tell the user the pressure levels that are created, and where the 
sublayering was started and where it finished.
...
gas profile & contains the path profile information (gas amounts,
            ...
            regression profile\\ \hline
continuum  & we are using our own set of lookup tables for the water
             continuum. These tables are based on the CKD tables \\ \hline
emissivity  & if radiances are to be computed, the user can vary the
            surface emissivity as a function of wavenumber\\ \hline
xsecdata  & if the absorption coefficients of the {\em CFC's} are to be
            included, a file containing this info must be present
            We supply file xsecdata.dat in {\sf DATA/General}.
            Note that this requirement will eventually be phased out, as we 
            will be moving towards using a kCompressed style format for
            the cross section gases\\ \hline
\end{longtable}
\end{small}
...
When the program starts to run, it checks to ensure that the
parameters set in {\sf kcarta.param} make sense.  It then parses in the
...
kCO2Path   & Path to the compressed data files for CO2.\\
...
kXsecParamFile & path to {\sf xsec.param}, which stores gas ID/frequency
           ...
           & \\
kWarnFile  & File where user wants warning messages to be dumped out to\\
           & Typically {\sf warning.msg}; if the user feels fairly confident, 
             use {\sf /dev/null}\\ 
           & \\
...
kXsecFormat   & tells the code if minor cross section gases data are in
             ...
             \hline
           & \\
caAuxiliaryPath   & tells the code where to look for certain auxuliary files,
                    such as the AIRS channel list ID\\
             \hline
           & \\
\end{longtable}
...
Here the user can prune or enlarge the arrays used by $DISORT$, $RTSPEC$ or 
$TWOSTREAM$. Note that this will have major consequences on the amount of 
memory used when
...
\item While reading this section, it might be helpful to look at the
  ...
  addition, the following points should be remembered :
   ...
   \item keywords MOLGAS, FRQNCY, PRFILE, OUTPUT are required 
   \item keywords PARAMS, XSCFIL, WEIGHT, RADNCE, JACOBN, SCATTR, SPECTRA are 
         optional 
...
2  & {\sf kCKD} & -1,0,21,23    & (integer) turn water continuum versions 
on/off. If turned on, specifies which model to use (CKDv0, 2.1, 2.3 or 2.4)\\
   &           &  -1  & no continuum\\
   &           &   0  & use water continuum CKDv0 \\
   &           & {\bf 21} & {\bf use water continuum CKDv2.1}\\
   &           &   23  & use water continuum CKDv2.3 \\ 
   &           &   24  & use water continuum CKDv2.4 \\ \hline
...
5 & {\sf kJacobOutput} & -1,0,1 & (integer) one of 3 output styles for 
    Jacobians.  They can 
be in raw $dR/ds_{m}$, $dR/ds_{m} \Delta s_{m}$ or $d(BT)/ds_{m} \Delta s_{m}$
where $R$ is radiance, $s_{m}$ is layer gas amount or temperature for layer 
$m$ and $BT$ is the brightness temperature.  $\Delta s_{m}$ is the amount
of gas in layer $m$, or a $1~K$ change in temperature \\
...
6 & {\sf kFlux}   & -1,1,2 & (integer) to calculate flux (+1,+2) or not (-1).
there are 2 different output styles for the computed ``fluxes.'' Note that the
user has to read in the computed ``flux'' for each layer, and then sum over
the computed fluxes, at the output resolution, to get the correct units.\\
   &  & {\bf -1}  & {\bf no flux computations} \\
   &             & 1 & flux computations : output units are radiance units\\
   &             & 2 & flux computations : output units are Kelvin/second\\ 
\hline
...
jacobians. This jacobian can be computed as a sum over 
d/dT(Planck $\times \tau$), or as d/dT(planck) or as d/dT($\tau$). Note 
that whether
...
8 & {\sf kRTP}   & -1,0,1   & (integer) whether or not to use an RTP file \\
  &              & {\bf +1}  & {\bf RTP style profile, RTP atmosphere defn}\\
  &              & {0}       & {\bf RTP style profile, user atmosphere defn}\\
  &              & {-1}      & {\bf old style profile, user atmosphere defn}\\
   \hline
\end{longtable}
\end{small}
...
This section specifies the frequency start/stop endpoints.  If kRTP = -1,0 the
user has to speciy the start/stop wavenumbers that kCARTA has to process. 
The format here is
...
If kRTP = +1, the RTP file contains the channelID list that \kc will process 
and then chunk through. \kc will automatically select start and stop endpoints
that include the extremeties of the channelID list (if head.nchan = -1, then 
kCARTA will go from 605.0 to 2830 cm-1). In the future the RTP file format 
might have a variable specifying which instrument to use, and \kc  should
automatically be able to set the start/stop freqs (we will probably look at
head.vchan for this info instead of head.ichan)
...
If kRTP = -1, this means the user wishes to use an old style (text) kLAYERS 
file for the profile. So in addition the user simply has to specify the name 
of the $kLayers$ profile file \\
{\sf 
\ttab caPfname = \\
}
...
\medskip
\noindent
where the set of weights is in one of the following possible formats

\medskip
\ttab {\sf iN list of weights }
...
\medskip
If $kRTP = -1,0$ this means the user will be defining the atmosphere, using the
specifications stated below. If $kRTP = +1$ then \kc will use the atmosphere
definitions specified in the RTP file that contains the profile; the 
documentation can be found in KLAYERS/Src\_1.10/Doc

\medskip
\subsubsection{kRTP = -1,0}
\noindent We now assume that $kRTP = -1,0$ and so the user needs to define the
atmosphere(s) in the namelist file, as follows. The first line has an 
...
degrees.  It has to be between 0.0 and +90.0, or it will be reset to
0.0 degrees.
...
As mentioned above, the documentation for this will be fouind in the 
KLAYERS/Src\_1.10/Doc directory. If $kRTP = 1$ then only $ONE$ radiance can be
computed for a single atmosphere. Parameter $kSurfTemp$ has to be set to -1, as
the RTP file specifies the surface temperature. From the $WEIGHT$ section, one
still needs to set IAMPSetForRad, to tell \kc which set of weights to use for 
the atmosphere. The start and stop pressures are defined by the pressure levels
in the RTP file (stop pressure can be assumed to be 0.0). Other variables are
set as necessary
\begin{verbatim}
                ...
                j) rakSolarAngle(iC) = abs(prof.sunang)  
                   IF (prof.sunang >=  0.0) & (prof.sunang <= 180.0)) THEN  
                     iakSolar(iC) = +1   
                   ELSE   
                     iakSolar(iC) = -1  
                    END IF  
...
\medskip
\noindent caComment is a 80 character string that the user should use to 
enter a short one line summary of the kCARTA run.
...
\item {\sf kcarta.x} is command line driven. Driver namelist input file and 
      ...
      automatically
      defaulted to stdin and stdout. See section on running the program.  
...
\item The reference profiles, and the PTHFIL profiles, must have kProfLayers
  layers for each gas.  We supply a program with the \kc distribution,
  klayers.f.  This program can take in (almost any) point profile and
  change it to a path averaged kProfLayers layer profile.
...
informative messages are saved in a file ``warning.msg''.  This
message filename can be changed or set to /dev/null in kcarta.param,
...
is saved to file $warning.msg$, so the user can easily view what \kc read in 
and how it did some set-ups. At present, this option cannot be used if fluxes 
and/or jacobians are to be output. 
...
{\sf integer}    & iTotal = total number of outputs per 10000 point chunk & \\
\end{longtable}
...
text version of this is saved to the warning.msg file, so the user
...
If kFlux == 1 then an auxiliary data file, with 'FLUX' appended to it,
...
subdirectory ``/KLAYERS/LookUp1.03.'' If your profile has non-water mixing 
ratios for dry air, use script ``klayersDRY.sc'', else use ``klayersWET.sc''
to run your sonde profile through $klayers.x$ : 
\begin{verbatim}
klayersZZZ.sc  sonde_profile  comment 
\end{verbatim}
where ``sonde\_profile'' contains your point profile and ``comment'' is 
a string that you can enter. The resulting profile is saved in fort.30, and
...
Jacobians.  In addition, the water continuum and cross-section
Jacobians are also obtained exactly, by analytic differentiation of
the terms used to compute these quantities in the first place.
...
\\
  ...
  5) Introduced a new parameter into *SCATTR, raExp(j), where j is
     the cloud under consideration. If set to less than 0 and a cloud is 
     ``expanded'' from ``one'' layer to layers (p1,p2), the IWP of 
     these layers is the same. If set to greater than 0 and a cloud is 
     ``expanded'' from ``one'' layer to layers (p1,p2), the IWP of 
...
We wish to thank Dave Edwards of NCAR for allowing us to use his {\sf GENLN2}
...
modifications of his subroutines in our code. We also wish to thank Dave Tobin
of U. of Wisconsin-Madison for help in the CO2 line mixing spectroscopy. 
Gyula Molnar of UMBC has provided a lot of useful feedback, regarding the meshing 
of kCARTA and kLAYERS. 

In addition, we wish to thank Istvan Laszlo of U. of Maryland, College Park for 
answering many questions regarding $DISORT$, and Frank Evans of U. of Colorado, 
for doing the same for $RTSPEC$. Thanks also go to Pat Arnott of the Desert 
Research Institute, for advice regarding the twostream code.
