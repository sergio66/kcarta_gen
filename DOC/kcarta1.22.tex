...
a) now uses the same uncompression and jacobian routines that are in the Matlab
   package. This makes the code cleaner. See iMatlabORf77 == iaaOverrideDefault(1,4)\\
...
\includegraphics[width=5.5in]{FIG/example.1.09}  
...
5 & {\sf kJacobOutput} & -1,0,1,2 & (integer) one of 3 output styles for 
  ...
  &      &           & \textcolor{red}{Note d(R)/d(ln(q)) = q d(R)/dq} \\    
...
\begin{verbatim}
 ...
 The surface temperature and satellite view angle are straightforward
  ...
  varying emissivity
   ...
   For constant background thermal angle, see n_rtp.f90 subroutine radnce4rtp
...
\item {\sf kcarta.x} is command line driven. Driver namelist input file and 
      binary output file names, and jacobian output file names, if not present on the command line, are 
      automatically defaulted to stdin and stdout. \\

      You can also specify the iRTP file as an option,
      that overrides iRTP in \emphPnm\_prfile}. So for example \\
        path_to_exec/kcarta.x  nmlfile outrad outjac -iXYZ \\
...
following forms, assuming you want to use iRTP in \emph{nm\_prfile}:

\begin{itemize}
  \item kcarta driver outfile jacfile
  \item kcarta driver outfile
  \item kcarta driver   (NOT RECOMMENDED)
  \item kcarta          (NOT RECOMMENDED)
\end{itemize}

Valid invocations of kcarta have one of the three following forms, assuming you want to override iRTP in \emph{nm\_prfile}:

\begin{itemize}
  \item kcarta driver outfile jacfile -XYZ
  \item kcarta driver outfile         -XYZ
  \item kcarta driver                 -XYZ    (NOT RECOMMENDED)
\end{itemize}
...
\subsection{Analytic vs Finite Difference Jacobians}

\noindent kCARTA has been checked quite thoroughtly, for
\begin{itemize}
\item single layer dr/dq, dBT/dq  analytic versus finite difference jacobians 
\item single layer q dr/dq, q dBT/dq  analytic versus finite difference jacobians
\item combined many layer dr/dq, dBT/dq  analytic versus finite difference jacobians 
\item combined many layer q dr/dq, q dBT/dq  analytic versus finite difference jacobians
\item See subroutine Set\_Ref\_Current\_Profs in kcartamisc.f
\item Also see ../MATLAB/compare\_sumlayer\_jac\_dRdq\_vs\_qdRdq.m
\end{itemize}

\noindent In order to check \emph{single layer dr/dq or dBT/dq via finite differences} you need to
\begin{itemize}
\item set kJacobOutput = -1 or +2 in nml file
\item run KCARTA and produce r0(v) and J(v,:) which is the raw TOA radiance and jacobain for eg L=1 to 97 layers  
\item go to Set\_Ref\_Current\_Profs, set iJax,iJax2,iGasJac as needed
\item uncomment the ``jacob test amount jacobian, new'' code
\item run KCARTA to produce r1(v), note what dq is for layer \emph{J}
\item finite difference $J_{finite} = \frac{(r1(v)-r0(v))}{dq}$ \emph{or use BT instead of r}
\item compare to $J_{analytic}(:,J)$
\end{itemize}

\noindent In order to check \emph{many adjacent layers dr/dq or dBT/dq via finite differences} you need to
\begin{itemize}
\item set kJacobOutput = -1 or +2 in nml file
\item run KCARTA and produce r0(v) and J(v,:) which is the raw TOA radiance and jacobain for eg L=1 to 97 layers
\item go to Set\_Ref\_Current\_Profs, set iJax,iJax2,iGasJac as needed
\item uncomment the ``jacob test amount jacobian, new'' code
\item run KCARTA to produce r2(v), note what cumulative dq(total) is for layer \emph{J1:JM}
\item finite difference $J_{finite} = \frac{(r2(v)-r0(v))}{dq(total)/length(J1:JM)}$  \emph{or use BT instead of r}
\item compare to sum($J_{analytic}(:,J1:JM),2$)
\end{itemize}

\noindent In order to check \emph{single layer q dr/dq or q dBT/dq via finite differences} you need to
\begin{itemize}
\item set kJacobOutput = 0 or +1 in nml file
\item run KCARTA and produce r0(v) and J(v,:) which is the raw TOA radiance and jacobain for eg L=1 to 97 layers  
\item go to Set\_Ref\_Current\_Profs, set iJax,iJax2,iGasJac as needed
\item uncomment the ``jacob test amount jacobian, new'' code
\item run KCARTA to produce r1(v), note what dq is for layer \emph{J}
\item Note what $\delta = dq/q$ is
\item finite difference $J_{finite} = \frac{(r1(v)-r0(v))}{log(1+\delta)}$  \emph{or use BT instead of r}
\item compare to $J_{analytic}(:,J)$
\end{itemize}

\noindent In order to check \emph{many adjacent layers q dr/dq or q dBT/dq via finite differences} you need to
\begin{itemize}
\item set kJacobOutput = 0 or +1 in nml file
\item run KCARTA and produce r0(v) and J(v,:) which is the raw TOA radiance and jacobain for eg L=1 to 97 layers
\item go to Set\_Ref\_Current\_Profs, set iJax,iJax2,iGasJac as needed
...
\item run KCARTA to produce r2(v), note what cumulative dq(total) is for layer \emph{J1:JM}
\item Note what $\delta = dq/q$ is
\item finite difference $J_{finite} = \frac{(r2(v)-r0(v))}{log(1+\delta)}$  \emph{or use BT instead of r}
\item compare to sum($J_{analytic}(:,J1:JM),2$)
\end{itemize}
...
\bibliographystyle{plain} \bibliography{/home/sergio/texmf/bibtex/bib/atmspec2017}
