...
kCARTA has been checked quite thoroughtly, for
\begin{itemize}
\item single layer dr/dq, dBT/dq  analytic versus finite differente jacobians 
\item single layer q dr/dq, q dBT/dq  analytic versus finite differente jacobians
\item combined many layer dr/dq, dBT/dq  analytic versus finite differente jacobians 
\item combined many layer q dr/dq, q dBT/dq  analytic versus finite differente jacobians
...
In order to do single layer dr/dq or dBT/dq via finite differences you need to
\begin{itemize}
...
\item run KCARTA to produce r1(v), note what dq is for layer <<J>>
\item finite difference Jfinite = (r1(v)-r0(v))/dq  [or use BT instead of r]
\item compare to Janalytic(:,J)
\end{itemize}

In order to do many adjacent layers dr/dq or dBT/dq via finite differences you need to
\begin{itemize}
\item set kJacobOutput = -1 or +2 in nml file
\item run KCARTA and produce r0(v) and J(v,:) which is the raw TOA radiance and jacobain for eg L=1 to 97 layers
\item go to Set\_Ref\_Current\_Profs, set iJax,iJax2,iGasJac as needed
\item uncomment the ``jacob test amount jacobian, new'' code
\item run KCARTA to produce r2(v), note what cumulative dq(total) is for layer <<J1:JM>>
\item finite difference Jfinite = (r2(v)-r0(v))/dq(total)/length(J1:JM)  [or use BT instead of r]
\item compare to sum(Janalytic(:,J1:JM),2)
\end{itemize}

In order to do single layer q dr/dq or q dBT/dq via finite differences you need to
\begin{itemize}
...
\item finite difference Jfinite = (r1(v)-r0(v))/log(1+$\delta$)  [or use BT instead of r]
\item compare to Janalytic(:,J)
\end{itemize}

In order to do many adjacent layers q dr/dq or q dBT/dq via finite differences you need to
\begin{itemize}
\item set kJacobOutput = 0 or +1 in nml file
\item run KCARTA and produce r0(v) and J(v,:) which is the raw TOA radiance and jacobain for eg L=1 to 97 layers
\item go to Set\_Ref\_Current\_Profs, set iJax,iJax2,iGasJac as needed
...
\item run KCARTA to produce r2(v), note what cumulative dq(total) is for layer <<J1:JM>>
\item Note what $\delta = dq/q$ is
\item finite difference Jfinite = (r2(v)-r0(v))/log(1+$\delta$)  [or use BT instead of r]  
\item compare to sum(Janalytic(:,J1:JM),2)
\end{itemize}
