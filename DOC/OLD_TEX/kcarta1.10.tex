...
written our own twostream scattering code {\em TWOSTREAM}. To run any 
of these three scattering  codes, one needs the MIE scattering tables 
...
{\sf INCLUDE/kcarta.param, INCLUDE/pre\_defined.param and Makefile}. 
...
are set for the data files (ensure that the kCompPath,kCO2Path and kWaterPath 
...
\begin{itemize}
\item $kcarta.x$ contains all the features of kCARTA (optical depths, clear 
sky radiative transfer, clear sky jacobians and fluxes, $DISORT$, $RTSPEC$  or
$TWOSTREAM$ radiances and fluxes. However, this version can use a $LOT$ of 
memory
\textcolor{red}
{\item $bkcarta.x$ contains only the most basic features of kCARTA 
(optical depths and clear sky radiative transfer). This version uses 
...
The original source code and documentation for all three packages is 
...
\item {\sf scatter\_simple\_main.f} : interface to TWOSTREAM calculations
\item {\sf scatter\_twostream\_solar.f} : twostream scattering code 
\item {\sf scatter\_simple\_flux.f} : flux code (totally incomplete!!!!)
...
pre\_defined.param& contains many interface array dimensions for kCARTA; DO NOT
TOUCH!!!!\\ 
\hline
\end{longtable}
\end{small}
...
MT\_CKD releases are 1\\
our modifications are 2 (derived from 1)\\
 51,55,60 (derived by Strow, Machado, Hannon from analysing RAL  + AIRSdata)\\
and various 12,13,50,52,56 which might be gotten rid of eventually\\
   ...
   &           &  {\bf 60} & {\bf use water continuum CKD-MSH v60}\\
...
8 & {\sf kRTP}   & -1,0,1   & (integer) whether or not to use an RTP file \\
  ...
  &              & {0}       & {\bf RTP style profile, user atmosphere, 
                                scatter defn}\\
  &              & {-1}      & {\bf old style profile, user atmosphere, 
                                scatter defn}\\
   \hline
\end{longtable}
\end{small}
...
documentation can be found in KLAYERS/Doc. Only $ONE$ atmosphere can
...
If $kRTP = -1,0$ this means the user will be defining the atmosphere and
...
one of three scattering codes, depending on the users needs. 
...
\subsubsection{kRTP = -1,0}
The user has to tell the code which of the above three scattering models 
...
model ($RTSPEC,DISORT,TWOSTREAM$), but always uses the $TWOSTREAM$ model to 
compute jacobians.

For the code to interface to $twostream$, $rtspec$ or $disort$, the user $has$ 
...
\noindent $kWhichScatterCode$ = +3 if DISORT is to be used, +2 if RTSPEC is 
to be used, +1 if $TWOSTREAM$. Note that RTSPEC and TWOSTREAM run very fast.
\vspace{0.25in} 
...
\subsection{RTP file}
\begin{itemize}
\item If kRTP = -1, then the driver namelist file needs to specify and define
