!same as to_f90.90 except it reads in file names from a external source

! https://jblevins.org/mirror/amiller/to_f90.f90
! ifort to_f90_loop.f90 -o to_f90_loop
...
!     SERGIO : keep variables as same case
...
CHARACTER (LEN=60)       :: f77_listname, f77_name, f90_name
...
INTEGER                  :: iostatus, iostatusN, pos, count, last, n_marks, pos1(20),  &
...
!! this is eg 
!!  ls -1 *.f > 'allnames.txt'
WRITE(*, *) ' Enter name of text file conatining all F77 Source files: '
READ(*,*) f77_listname
OPEN(10, file=f77_listname, status='old',access='sequential', form='formatted', action='read',IOSTAT=iostatus)
count = 0
iostatusN = 0
DO WHILE (iostatusN .EQ. 0)
  READ(10,110,iostat = iostatusN) f77_name  
  if (iostatusN .EQ. 0) then
    count = count + 1
    write(*,'(I5,I2,3X,A)'), count,iostatusN,f77_name  
  end if  
END DO
CLOSE(10)
110 FORMAT (A)
...
DO WHILE (iostatusN .EQ. 0)
  ...
  if (iostatusN .EQ. 0) then
    ...
    IF (iostatusN < 0) STOP               ! Halts gracefully when the names are
...
!                                      Date & time stamp
  ...
  IF (ch /= ' ') WRITE(9, *)
  WRITE(9, '("! Code converted using TO_F90_LOOP by Alan Miller")')
    ...
    IF(text(i1-1:i1-1) == '.') THEN
      DO j = 1, 6
        IF (text(i1-1:pos+1) == compare(j)) THEN
          ...
          text0(i1-1:pos+1) = ' ' // replacement(j) // ' '	  
          EXIT
        END IF
      END DO
...
! If text is not matched, convert to lower case, if necessary.
  IF (.NOT. matched) THEN
! comment this out and modify with lower loop SERGIO  
!    DO j = i1, pos
!      IF (text(j:j) >= 'A' .AND. text(j:j) <= 'Z')              &
!              text(j:j) = CHAR ( ICHAR(text(j:j)) - inc )     
!    END DO
     text(i1:pos) = text0(i1:pos)
  END IF
